...
const SCRATCH_DB = "mongodb://localhost:27017/reality_book_type";
process.env.PORT = "3806";
process.env.MONGODB_URI = SCRATCH_DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "booktype-secret-0123456789";
process.env.REALITY_KEY_DIR = path.join(os.tmpdir(), "booktype-keys-" + process.pid);
...
try {
  ...
  !err && typed?.ok ? ok(`typed line parsed + evaluated (laid ${typed.laid.length}: ${typed.laid.join(", ") || "—"})`)
                    : bad(`press failed`, err?.message || typed);
  typed?.laid?.some((l) => /create-space/.test(l)) ? ok(`the press laid a create-space fact (the new bead)`)
  ...
  const newAct = fresh.find((a) => /made the space notebook/.test(a.line) || a.landings.some((l) => /made the space notebook/.test(l.did)));
  newAct ? ok(`the book updated live, in the Word — "${newAct.line}" (past tense, not do:create-space)`)
  ...
  afterAll.length === before.length + 1
    ? ok(`append-only: ${before.length} → ${afterAll.length} acts, the past untouched, one new act at the edge`)
    : bad(`append-only`, { beforeActs: before.length, afterActs: afterAll.length });
