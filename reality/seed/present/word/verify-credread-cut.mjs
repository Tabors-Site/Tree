...
const SCRATCH_DB = "mongodb://localhost:27017/reality_word_credread_cut";
process.env.PORT = "3794";
process.env.MONGODB_URI = SCRATCH_DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "credread-secret-0123456789";
process.env.REALITY_KEY_DIR = path.join(os.tmpdir(), "credreadcut-keys-" + process.pid);
...
try {
  ...
  rd.result?.hasPlain === true && typeof rd.result?.plaintext === "string" && rd.result.plaintext === minted
    ? ok(`read @victim → hasPlain:true, plaintext === the reset's minted cleartext (read decrypts the stored blob)`)
