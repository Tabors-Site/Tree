...
const MONGOOSE = `${R}/node_modules/mongoose/index.js`;
{
...
try {
  ...
  if (!cherub) { console.log("  FATAL: genesis failed (no cherub)"); process.exit(1); }
  const arrival = await findByName("being", "arrival", branch);
  console.log(`  cherub=${String(cherub?.id).slice(0,10)} arrival=${String(arrival?.id).slice(0,10)}\n`);
  ...
  founder ? ok(`first human @founder registered (first-being path)`) : bad(`@founder registered`, "no row");
  ...
  const { res, deltaF } = await register({ name: "comer-being", password: "comerpw123", nameId: String(ownerName) });
  console.log(`  birthHandler laid ${deltaF.length} world fact(s); returned firstUser=${res?.firstUser}, name=${res?.name}\n`);

  // ── assertions ──
  res?.name === "comer-being" ? ok(`return names @comer-being`) : bad(`return name`, res?.name);
  res?.firstUser === false ? ok(`return firstUser=false (subsequent path taken)`) : bad(`firstUser=false`, String(res?.firstUser));
  res?.identityToken ? ok(`session strand minted an identity token`) : bad(`identity token`, "none");
  res?.homeSpaceId ? ok(`return carries homeSpaceId (portal lands at home)`) : bad(`homeSpaceId`, String(res?.homeSpaceId));

  const born = await poll(() => findByName("being", "comer-being", branch));
  born ? ok(`@comer-being materializes after seal (${String(born.id).slice(0, 10)}…)`) : bad(`@comer-being materializes`, "no row");

  const proj = born ? await loadProjection("being", String(born.id), branch) : null;
  String(proj?.state?.trueName) === String(ownerName)
    ? ok(`being is the arriving Name's own (trueName = the Name)`) : bad(`trueName = arriving Name`, String(proj?.state?.trueName));

  const lineage = proj?.state?.qualities?.lineage;
  (String(lineage?.mother) === String(cherub?.id) && String(lineage?.father) === String(arrival?.id))
    ? ok(`lineage: mother=Cherub, father=Arrival`) : bad(`lineage`, JSON.stringify(lineage));

  const home = res?.homeSpaceId ? await loadProjection("space", String(res.homeSpaceId), branch) : null;
  String(home?.state?.members?.owner ?? home?.state?.owner) === String(born?.id)
    ? ok(`home space owned by the new being`) : bad(`home owner`, JSON.stringify(home?.state?.members ?? home?.state?.owner));
