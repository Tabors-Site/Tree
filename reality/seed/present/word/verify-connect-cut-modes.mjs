...
async function connect({ address, identity = null, nameId = null }) {
  ...
  } catch (e) { if (e && (e.name === "IbpError" || e.code)) return { res: null, refused: e }; throw e; }
}
...
try {
  ...
  await register({ name: "myvessel", password: "vesselpw1234", nameId: String(tabor) });
  const daddy = await birth("daddy");
  await birth("childbeing", { parentBeingId: daddy });

  // ── Mode-owned (flow 2): signed-in tabor connects to @myvessel (no password) ──
  const owned = await connect({ address: `${D}/@myvessel`, nameId: String(tabor) });
  owned.res?.owned === true && owned.res?.identityToken && owned.res?.name === "myvessel"
    ? ok(`Mode-owned cut: tabor → @myvessel → owned:true, token, name`) : bad(`mode-owned`, owned.refused?.message || owned.res);
  ...
  const notOwned = await connect({ address: `${D}/@myvessel`, nameId: String(mallory), identity: { beingId: "mallory-being", name: "mallory" } });
  notOwned.refused ? ok(`Mode-owned not-owned → falls through → refused (not owner, not relative)`) : bad(`not-owned fallthrough`, notOwned.res);

  // ── Mode-3 (flow 3): daddy inherit-connects to @childbeing it birthed ──
  const inh = await connect({ address: `${D}/@childbeing`, identity: { beingId: String(daddy), name: "daddy" } });
  inh.res?.inherited === true && inh.res?.identityToken && inh.res?.name === "childbeing"
    ? ok(`Mode-3 cut: daddy → @childbeing → inherited:true, token, name`) : bad(`mode-3`, inh.refused?.message || inh.res);

  // ── Mode-3 not-relative: a stranger → refuse ──
  const stranger = await connect({ address: `${D}/@childbeing`, identity: { beingId: "stranger-being", name: "stranger" } });
  stranger.refused && /inhabit beings/i.test(stranger.refused.message)
    ? ok(`Mode-3 stranger → refused (you can only inhabit beings you birthed/descend from)`) : bad(`mode-3 stranger`, stranger.refused?.message || stranger.res);
