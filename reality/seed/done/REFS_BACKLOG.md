# Refs Migration Backlog (ARCHIVED)

## Status: ARCHIVED — Phase 1.6 over-reach, rolled back 2026-06-04

This document is preserved for forensic interest only. The Phase 1.6 migration it tracks was over-scoped. The substrate's own handlers always knew which fields were aggregate IDs via their own logic; making them strictly demand typed Refs added no information and created a permanent maintenance tax on every wire consumer (portal, future CLI, federation).

The rollback (same day) kept the actual federation primitive — the Ref type and the walker (`findRefs` / `remapRefs`) — and unwound:
- Handler-side strict validation that refused bare strings
- The plan to make Refs the wire contract (portal would have had to wrap every ID)
- The "wire dispatcher refuses bare strings" item

The doctrine now lives at `seed/REFS.md` in its narrow scope: Refs are a **content-walking primitive** used at three seams — replicate, clone, federation — where code traverses content without access to the schema. Everywhere else, schemas know what's an ID; bare IDs flow through.

For everything below: each "done" row records what migration landed AND will need to come back to bare-string handling. The handler-side strictness has been removed (or is being removed; see git log around 2026-06-04 for the rollback commits). Storage retains Refs in some places during lazy cleanup; consumers read defensively via `refId()` which tolerates both shapes.

## Status (historical): ACTIVE — Phase 1.6 in flight
...
- [x] **qualities.beings.\<role\>.beingId** (kind: being) — done 2026-06-04. Writers wrap into being-Refs: seedDelegates.js (place-root delegate registration) and harmony/danceFloor.js (drummer + dancer registrations, 2 sites) emit ref("being", id) inside the qualities.beings entry. Readers extract bare id via refId(): descriptor.js#beingsAtSpace inlines a Ref unwrap (sync function — no dynamic import) so the wire-shape `_beingId` stays bare-string through enrichBeings + activity lookups; summon.js's stance-resolution-by-role-shorthand extracts via refId() before loadOrFold. qualities namespace is already Mongoose-Mixed so no schema flip. No Mongo subpath queries on `state.qualities.beings.<role>.beingId` exist (it's iterated by name, not queried by id). Regression: 162/162 green.
- [x] **qualities.beings.\<role\>.parentBeingId** (kind: being) — done 2026-06-04 (vacuously). Surveyed all writers of `qualities.beings.<role>` entries (seedDelegates.js, harmony/danceFloor.js, future cherub paths): none write a `parentBeingId` subfield into the entry today. The Being's top-level parentBeingId is captured at birth (already migrated to Ref in the Being.parentBeingId row); the qualities.beings registry entry just carries `{ beingId, role, installedAt, installedBy }` — no lineage subfield. The backlog row was forward-looking; if a future writer adds parentBeingId to the entry shape, it must use ref("being", id) per doctrine. Same migration discipline as the beingId path above. Regression: 162/162 green (no behavioral surface to exercise).
- [x] **qualities.beings.\<role\>.homeSpace** (kind: space) — done 2026-06-04 (vacuously). Same survey as parentBeingId: no current writer populates `homeSpace` inside a qualities.beings entry. The being's top-level homeSpace is captured at birth (Being.homeSpace row, already Ref). The qualities.beings registry entry shape is `{ beingId, role, installedAt, installedBy }`; descriptor.js#beingsAtSpace doesn't read a homeSpace subfield. Forward-looking guarantee: any future writer must use ref("space", id) per doctrine. Regression: 162/162 green.
- [x] **qualities.connection.inhabitedBy** (kind: being) — done 2026-06-04. be.js connect handler wraps driverId via ref("being", id) into params.inhabitedBy; release handler emits null (no change). Reducer (reducerHelpers.js applyConnect/applyRelease) is value-agnostic — stores whatever the writer puts. identity/lookups.js#beingCognition's `if (connection?.inhabitedBy) return "human"` uses a truthy check that works on both Ref and bare-string values; no extraction needed. resetReels.js Mongo query is `$ne: null, $exists: true` — null/exists semantics unchanged by Ref storage. No descriptor.js / beingsCatalog.js consumer reads of `inhabitedBy` value — Sam's row hint was stale; confirmed by grep. Regression: 162/162 green.
- [x] **qualities.connection.inhabitsHomeSpace** (kind: space) — done 2026-06-04 (vacuously). Grep of seed/ + extensions/ returned zero code sites writing or reading `inhabitsHomeSpace`. The path is documented in the backlog row but no current code path populates or consumes it. Forward-looking: any future writer must use ref("space", id) per doctrine. Regression: 162/162 green (no behavioral surface).
- [x] **qualities.wakes.\<key\>.spaceId** (kind: space) — done 2026-06-04 (vacuously). Grep of seed/ + extensions/ returned zero code sites for a `qualities.wakes` namespace. The current wake mechanism stores wake state via facts on the being's reel (`do:wake-scheduled` / `do:wake-cancelled` on target = the being) plus an in-memory registry projection in wakeSchedule.js (`_byId`, `_byBeing` maps keyed by bare string). Fact params are `{ scheduleId, intervalMs, priority, content, skipIfBacklog }` — no spaceId, no beingId-in-params. Scheduler's `wake(beingId, spaceId)` runtime fn takes bare-string args (not fact data). Sam's row hint was a placeholder from an older design considering qualities.wakes; the actual design uses target-typed facts. No migration surface. Regression: 162/162 green.
- [x] **qualities.wakes.\<key\>.beingId** (kind: being) — done 2026-06-04 (vacuously, same rationale as qualities.wakes.spaceId above). No qualities.wakes namespace exists. The wake-scheduled fact's `target.id` IS the beingId, and target.id stays bare-string per substrate doctrine (structurally typed by the surrounding kind envelope). The in-memory registry's `beingId` field is extracted from `fact.target.id` and stays a bare string. Forward-looking: if a future design re-introduces qualities.wakes, any beingId subfield must use ref("being", id). Regression: 162/162 green.
