...
**Runtime machinery is wrapped as beings in heaven.** Each piece of the running process (the HTTP listener, the WebSocket pool, the Mongo connection) is a being with a role, homed in its own room under `./host`. The being's act-chain records its activity; operations at the boundary (handling a request, accepting a connection) produce facts on the being's chain or on matter the being owns. The machinery becomes observable through the same four verbs as everything else. This wrapping is fully fact-backed — the opposite of `./source`'s disk-fold exception — and it is observability, not rerouting: hot paths (the stamper writing facts, Mongo queries) stay direct, because wrapping the write path in a being would recurse.

**One being per boundary.** Don't split machinery by sub-category — no being per HTTP verb, per query type, per connection direction. The natural unit is the boundary itself: `@http-server` for HTTP, `@websocket-pool` for sockets, `@mongo` for the database. Verbs, routes, and query shapes are FIELDS in fact content, not separate identities; to see all POSTs, filter the request-log's facts by verb. Per-verb beings would fracture one thing into N chains, duplicate roles and lifecycle, and buy nothing the data didn't already give.
...
**Outages are recorded as gap-facts on reconnect, never pretended away.** While Mongo is down the reality cannot stamp, and pretending otherwise would be a lie on the chain. The disconnect is held in memory; the reconnect stamps ONE fact carrying the whole gap ({disconnectedAt, reconnectedAt, gapMs}). The chain is honest about its own discontinuities. This generalizes: any wrapped dependency that can go away records what happened when it CAN stamp, including the gap itself.

**Names are one namespace per kind per branch — and roles mirror as spaces.** The projection index makes a name collision break the fold, not just look odd. Two consequences bitten in practice: connection matter names carry the FULL socket id (truncated prefixes collide), and a role name must not collide with any space name, because the registry mirrors every role as a space under `./roles` (the `@mongo` being carries the `mongo-connection` role precisely so the role-mirror doesn't collide with the `./host/mongo` space).
...
- `host.js` — resolved ids, readiness, per-being serial act lanes, the WebSocket connection lifecycle, the Mongo event hooks, the boot reconcile sweep.
