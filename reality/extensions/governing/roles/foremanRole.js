...
import { runTurn } from "../../../seed/factory/voices/llm/runTurn.js";
...
async function dispatchLeafBatch({ group, executionSpaceId, rulerSpaceId, ctx, allBranchNames, planEmission }) {
  ...
  const foremanStance = `${realityDomain}/${executionSpaceId}@${ctx.toBeing?.name || "foreman"}`;
...
async function ensureWorkerBeing({ executionSpaceId, workerRoleName, ctx }) {
  ...
  try {
    ...
    if (being?._id) {
      // Stamp qualities.beings.<workerRoleName> via do.set so the
      // worker-registration lands on the execution space's reel as a
      // Fact (the foreman is acting; the substrate records it).
      // Deep-path set writes the per-worker entry without touching
      // the rest of the beings namespace.
      ...
      if (space) {
        ...
        const opts = identity
          ? { identity, summonCtx: ctx?.summonCtx || null }
          : { scaffold: true };
        await doVerb(
          space,
          "set",
          {
            field: `qualities.beings.${workerRoleName}`,
            value: {
