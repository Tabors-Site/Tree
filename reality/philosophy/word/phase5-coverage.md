...
When an authenticated being (the caller) connects to inherit a descendant being:
  ...
  Otherwise (inherited, not as father):
    The driver trueName is the target being's trueName (the vessel keeps its own signer).
...
---
*Lane split: parser + .word slices = Claude; engine + the live-diff gate = the other agent. This map is the shared backlog.*
