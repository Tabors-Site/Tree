...
// Run: node --test place/test/sessionKeys.test.js

import { test, describe } from "node:test";
import { strict as assert } from "node:assert";
import { resolvePipelineKey } from "../seed/factory/intake/session.js";
...
describe("resolvePipelineKey", () => {
  ...
  test("scope=place + purpose builds pipeline:place key", () => {
    const { key } = resolvePipelineKey({
      scope: "place", purpose: "digest",
      makeEphemeral,
    });
    assert.equal(key, "pipeline:place:digest");
  });
  ...
  test("scope=place without purpose throws", () => {
    assert.throws(
      () => resolvePipelineKey({ scope: "place", makeEphemeral }),
      /scope='place' requires purpose/,
    );
  });
