...
export function createView() {
  ...
  function mount(rootEl, portalCtx) {
    ...
    wrap.innerHTML = `
      <div id="tv-bar">
        <div class="tv-scope" data-el="scope"></div>
        <span class="tv-title" data-el="title"></span>
        <div class="tv-bar-spacer"></div>
        <button class="tv-tool tv-now" data-el="now" style="display:none" title="return every view to the present">⏵ return to now</button>
        <button class="tv-tool" data-el="refresh" title="re-read the chain">↻</button>
      </div>
      <div id="tv-feed"></div>`;
    ...
    els = {
      ...
      feed:    wrap.querySelector("#tv-feed"),
    };
  ...
  function paintAnchor() {
    ...
    for (const el of els.feed.querySelectorAll(".tv-entry")) {
  ...
  function renderFeed(emptyText = null) {
    ...
    if (!exhausted) {
      ...
      more.className = "tv-tool tv-more";
  ...
  function renderEntry(e) {
    ...
    if (e.facts?.length) {
      ...
      f.className = "tv-facts";
      const shown = e.facts.slice(0, 4).map((x) => `⛓ ${x}`).join("  ");
      f.innerHTML = `<span class="tv-fact-chip"></span>`;
