...
**BE** happens in the place scene itself. The cherub stands somewhere in the place as a real figure the user encounters on arrival (the architectural commitment that the cherub is a being made literal). Staring at the cherub triggers a hazy glare effect (sensory cue: you are entering an identity moment) and a login menu opens overlaid in 3D space. The user picks register or claim, fills in credentials, the BE call goes out, the token comes back, the glare clears, and they are in the world as their established stance. Switching identity later is the same flow: walk back to the cherub, stare, the glare returns, switch. The flat Portal's BE surface still exists for users who prefer it, but the native 3D flow is gaze-on-cherub.
