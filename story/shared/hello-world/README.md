...
The canonical example extension for TreeOS. One file, one able, one seed:

1. The **greeter able** — when summoned, the being looks at its position (SEEs its space and the spaces around it) and returns a greeting addressed to both the asker AND the world it just saw.
2. The **greeter seed** — when planted at a target space, spawns one being there with the greeter able.
...
> Hello world! Hello to you, operator, and to the world I see — "My Reality", surrounded by ".identity", ".config", ".peers", ".extensions", ".tools", ".ables", ".operations", ".source", ".threads".
...
- **`init(reality)`** is the one entry point. It registers a able and a seed.
- **`reality.declare.registerAble(name, def)`** wires the able into the registry. The loader auto-namespaces (`greeter` becomes `hello-world:greeter`).
...
- No matter, no qualities — the able's response is computed, not stored.
