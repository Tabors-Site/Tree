...
//      eligibility into the being). For cross-story summons the
...
// story for the first time. Different entry point, different able.)
//
...
// the being. First scripted able in the repo that turns a see entry
// into a real perception-aware decision.

import log from "../../../seedStory/log.js";

const DEFAULT_BIRTHER_NAME_PREFIX = "being";

export const birtherAble = Object.freeze({
  name: "birther",
  description:
    "Mints children from authenticated callers. Click @birther to give birth to a child being — the new being's parent is you. Cross-world: foreign actors can summon:mate against @birther to commission a being-child on this story.",
  ...
  // else in the substrate. See seed/AblesAreAuth.md + FEDERATION.md.
  can: [
    { verb: "see", word: "place" },
    {
      verb: "call",
      ...
      as: "receiver",
      description:
        "Auto-accepts mate requests. The summoner becomes father; this birther becomes mother; child is birthed on this story.",
    },
    { verb: "be", word: "birth" },
  ],
  ...
  async call(message, ctx) {
    ...
    // for this moment (orientation + able + position + capabilities +
    // able.can see-entry-resolved blocks). Scripted ables read it as data:
    //   ctx.innerFace.blocks . [{ key, source, label, payload }, ...]
    // Same shape the LLM mouth reformats and the human portal renders.
    // This birther able uses it as a perception-aware pre-flight gate
    // for the "mate" intent: before commissioning a being, it looks
    // at the "place" block (resolved from the see:"place" entry) and
    // refuses when an occupant already carries the prospective being name.
    ...
    if (intent === "mate") {
      // Perception-aware pre-flight. Compute the prospective being
      ...
      if (collision) {
        ...
        return (
          ctx.failure?.(
            "refused",
            `being name "${prospectiveName}" already at home space`,
          ) || {
            kind: "failure",
            ok: false,
            shape: "refused",
            reason: `name collision: ${prospectiveName}`,
          }
        );
      }
...
// being name. Suggested (from message.name) wins; otherwise the
// `being-<story>-<short>` fallback. Story is the asker's when
...
async function resolveBeingName(message, ctx) {
  ...
  const content =
    messageObj.content && typeof messageObj.content === "object"
      ...
      : null;
  const suggested =
    content && typeof content.name === "string" && content.name.length
      ? content.name.trim()
      : typeof messageObj.name === "string" && messageObj.name.length
  ...
  return `${DEFAULT_BIRTHER_NAME_PREFIX}-${fatherStory.replace(
    /[^a-z0-9]/gi,
    "",
...
// (Same-story being-children are rarer than cross-story, but the
...
async function handleMateRequest(message, ctx, precomputedName = null) {
  ...
  if (!askerBeingId) {
    ...
    return (
      ctx.failure?.(
        "internal",
        "summon:mate received without asker identity",
      ) || {
        kind: "failure",
        ok: false,
        shape: "internal",
        reason: "no asker identity",
      }
    );
  }
  ...
  // Resolve the homeSpace for the new being. Default: the story
  // root (the new being lives at the top of this story). Operators
  // who want beings parented elsewhere can override via
  // message.homeSpaceId.
  const messageObj =
    typeof message === "object" && message !== null ? message : {};
  ...
  if (!homeSpaceId) {
    return (
      ctx.failure?.("internal", "cannot resolve homeSpace for new being") || {
        kind: "failure",
        ok: false,
        shape: "internal",
  ...
  // being name. Pre-resolved by the summon dispatcher via
  ...
  if (!name) {
    return (
      ctx.failure?.(
        "internal",
        "being name unresolvable (no suggestion and no asker identity)",
      ) || {
        kind: "failure",
        ok: false,
        shape: "internal",
  ...
  const birtherBeingId = ctx?.toBeing?._id || ctx?.toBeing?.id || null;
  if (!birtherBeingId) {
    return (
      ctx.failure?.("internal", "birther beingId unresolved in ctx") || {
        kind: "failure",
        ok: false,
        shape: "internal",
        reason: "no birther id",
      }
    );
  }
  ...
  const summary =
    `being-child "${result?.name || name}" born on ${localStory} ` +
