...
//
// ableComposer.js — compose a able STACK into a single able-shaped spec
...
// able; every stacked clause (`stack: true`) whose `when` matches
...
//   - summon — the PRIMARY able's summon function. Scripted-cognition
...
//   - name — PRIMARY's name. Stamped into Act.activeAble; the modifier
...
//
// ── Why "one able-shaped spec" ────────────────────────────────────
//
// Every existing consumer (buildPrompt, momentum, llmMoment, summon
// verb) reads able.name / able.prompt / able.canSee etc. Composing
...
// currently in this mode" frame naming the modifier able, so the LLM
// composes them as layered context rather than reading the `---`
// divider as a discontinuity / topic change. This matches the
// guidance in able-manager.md verification §3: name each modifier so
...
/**
 * Resolve a list of able names into a single composed able spec.
 *
 * @param {object} args
 * @param {string[]} args.stack        ordered [primary, ...modifiers] able names
 * @param {object}   args.toBeing      recipient being (for requiredCognition filtering)
 * @returns {object|null}              composed able spec, or null when the primary is unresolvable
 *
 * Modifiers whose ables aren't registered, or whose requiredCognition
...
export function composeStack({ stack, toBeing }) {
  ...
  for (let i = 0; i < stack.length; i++) {
    ...
    const able = getAble(name);
    if (!able) continue;
    if (able.requiredCognition && able.requiredCognition !== beingCog) {
  ...
  // Capability unions. Each able's can*-arrays are entry lists (strings
  ...
  // Prompt composition. Each able's prompt may be a function (the
  // canonical seed/extension shape — registry.js's makeLazyDefaultCall
  // wraps it) or a string (live ables authored via set-able). The
  ...
  return Object.freeze({
    ...
    // Identity-flag preservation: ables that need a scripted-cognition
...
// Wraps able.prompt regardless of source shape into one (ctx) => string.
function normalizePrompt(able) {
  if (typeof able?.prompt === "function") {
    return (ctx) => {
      try { return String(able.prompt(ctx) || ""); }
      catch { return ""; }
    };
  }
  if (typeof able?.prompt === "string") {
    const literal = able.prompt;
    return () => literal;
  }
  return () => "";
}
