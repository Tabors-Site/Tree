...
//
// arrival. The able every unauthenticated visitor carries.
//
...
// register/claim exception). Under ables-are-auth (seed/AblesAreAuth.md)
// the arrival able's `can` list IS the gate; the be:birth/connect/release
// entries are the only BE surface anonymous callers reach.
//
// Cognition: scripted, no-op. Arrival doesn't receive SUMMONs (no
// triggerOn entry for "message"). The able exists as the receptive
// label for the @arrival stance, not as a being that processes
// inbox traffic.

export const arrivalAble = Object.freeze({
  name: "arrival",
  description:
  ...
  // addresses, which only the human/angel ables carry).
  //
  // summon: The doctrinal path for an anonymous visitor is summon
  // @cherub:mate. Cherub mints the new being (with the human able and
   ...
   * this handler exists only so the able has a callable summon when
