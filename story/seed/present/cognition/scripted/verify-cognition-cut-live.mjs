...
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "cogcut-keys-" + process.pid,
);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "cogcut-src");
...
const { setCurrentSpace, getCurrentSpace } = await import(
  `${R}/seed/materials/being/position.js`
);
...
try {
  ...
  if (!getCurrentSpace(String(being))) {
    await withIAmAct("place reactor being", async (m) => {
      await setCurrentSpace(String(being), String(position), m);
    });
    await new Promise((r) => setTimeout(r, 400));
  }
  const here = getCurrentSpace(String(being));
  here
    ? ok(`being stands at a position (${String(here).slice(0, 8)})`)
  ...
  const triggers = [
    { when: () => true, then: () => "I make notebook.\nI make journal." },
  ];
  const innerFace = {
    position: { name: "home" },
    able: "global",
    capabilities: { canDo: ["create-space"] },
    blocks: [],
  };
  const res = await runReactorMoment(triggers, {
    able: { name: "global" },
    moment: { innerFace },
    beingId: String(being),
    username: "reactorbeing",
    history: "0",
  });
  ...
  res?.kind === "see"
    ? ok(
        `the cognition returned SEE — the cognition-moment seals nothing of its own`,
      )
  ...
  actsAfter - actsBefore === 2
    ? ok(
        `the chain GREW BY 2 acts (${actsBefore} → ${actsAfter}) — one moment per deed, no run-on`,
      )
    : bad(`chain grew by 2`, { actsBefore, actsAfter });
  facetsAfter - facetsBefore === 2
    ? ok(
        `2 create-space facts landed under the being (the deeds reached store)`,
      )
  ...
  actIds.length >= 2
    ? ok(
        `the deeds carry DISTINCT actIds — each its own moment, the spacebar held`,
      )
    : bad(`distinct actIds`, { actIds });

  const nb = await findByName("space", "notebook", "0");
  const jr = await findByName("space", "journal", "0");
  nb && jr
    ? ok(`both spaces (notebook, journal) exist in store`)
    : bad(`spaces in store`, { nb: !!nb, jr: !!jr });
