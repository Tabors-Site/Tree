...
// cherub/connect verifiers; this pins the fold + bootstrap + the special-branch ops. (I_AM's genesis
// be:birth is a raw emitFact in sprout.js, never beVerb, so it predates + grounds this fold.)
import fs from "fs"; import os from "os"; import path from "path"; import { fileURLToPath } from "url";
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const R = path.resolve(__dirname, "../../..");
const DB = "mongodb://localhost:27017/story_beopsfold";
process.env.PORT = "3833"; process.env.MONGODB_URI = DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "beopsfold-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "beopsfold-keys-" + process.pid);
...
const { bindWord, registerHostHandler, resolveBeOpFromFold, getWordSync } = await import(`${R}/seed/present/word/wordStore.js`);
...
try {
  ...
  const expected = ["birth", "connect", "release", "switch", "death", "truename"];

  // (1) all six BE ops folded as kind:"beop" words "be:<op>" (incl switch/death/truename)
  const folded = expected.filter((op) => { const w = getWordSync(`be:${op}`); return w && w.kind === "beop" && w.do?.ref; });
  (folded.length === expected.length)
    ? ok(`all 6 BE ops folded as kind:"beop" words (be:${expected.join(", be:")})`)
  ...
  const resolved = expected.filter((op) => typeof resolveBeOpFromFold(op)?.handler === "function");
  (resolved.length === expected.length)
    ? ok(`resolveBeOpFromFold returns a handler for all 6 (incl the special-branch switch/death/truename)`)
    : bad(`all 6 resolve a handler`, { resolved });

  // (3) the BE-only `bootstrap` flag rides the fold, matching the buffer for each op
  const bootstrapOk = expected.every((op) => !!getBeOp(op).bootstrap === !!resolveBeOpFromFold(op).bootstrap);
  const bootstrapTrue = expected.filter((op) => resolveBeOpFromFold(op).bootstrap);
  (bootstrapOk)
    ? ok(`bootstrap flag carried through the fold (true for: ${bootstrapTrue.join(", ") || "none"})`)
    : bad(`bootstrap carry`, expected.map((op) => ({ op, buf: !!getBeOp(op).bootstrap, fold: !!resolveBeOpFromFold(op).bootstrap })));
  ...
  const agree = buffer.length === expected.length && buffer.every((op) => typeof resolveBeOpFromFold(op)?.handler === "function" && typeof getBeOp(op)?.handler === "function");
  (agree)
    ? ok(`BE_OPS object (${buffer.length}) agrees with the fold (the demoted registration buffer)`)
  ...
  await bindWord("be:test-be-op", { ownerExtension: "seed", kind: "beop", do: { ref: "be-op:test-be-op" }, bootstrap: true });
  ...
  (typeof synth?.handler === "function" && synth.bootstrap === true)
    ? ok(`resolveBeOpFromFold resolves the synthetic op from the fold (with its bootstrap)`)
