...
const SCRATCH_DB = "mongodb://localhost:27017/story_word_grantable_cut";
process.env.PORT = "3797";
process.env.MONGODB_URI = SCRATCH_DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "grantable-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "grantablecut-keys-" + process.pid);
...
const grant = async (target, params) => {
  ...
  try {
    const res = await doVerb({ kind: "being", id: String(target) }, "grant-able", params, { identity: ident, moment: sc, currentHistory: "0" });
...
try {
  ...
  resolveAbleWord("being", "grant-able") ? ok(`grant-able.word resolves through the bridge`) : bad(`resolves`);

  // pick a registered able to grant
  const able = ["human", "global", "member"].find((r) => getAble(r)) || "global";
  ...
  g.result?.granted === true && g.result?.able === able && g.result?.grantedBy === I_AM
    ? ok(`grant "${able}" @ root → granted:true, grantedBy = I_AM (the caller)`)
  ...
  gf && gf.params?.grantedBy === I_AM && typeof gf.params?.grantedAt === "string"
    ? ok(`the grant-able fact carries grantedBy + grantedAt (the cut's param-enrichment reached the auto-fact)`)
    : bad(`fact params`, gf?.params);

  // ── 3. the grantee's ablesGranted folds the grant ──
  const slot = await loadOrFold("being", String(grantee), "0");
  const granted = (slot?.state?.qualities?.ablesGranted || []).find((r) => (r.able || r) === able && (r.anchorSpaceId === anchor || !r.anchorSpaceId));
  granted
    ? ok(`@grantee ablesGranted folds {able:"${able}", anchorSpaceId, grantedBy} — the adjective applied`)
  ...
  n1.refused && /able is required/i.test(n1.refused.message) && n1.refused.code === "INVALID_INPUT" ? ok(`no able → refuse "able is required" [INVALID_INPUT]`) : bad(`no able`, n1.refused?.message || n1.result);
  const n2 = await grant(grantee, { able: "definitely-not-a-able", anchorSpaceId: anchor });
  n2.refused && /not registered/i.test(n2.refused.message) ? ok(`unknown able → refuse "not registered"`) : bad(`unknown able`, n2.refused?.message || n2.result);
  const n3 = await grant(grantee, { able, anchorSpaceId: anchor, anchorBeingId: String(cherub.id) });
