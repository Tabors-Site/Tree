...
const SCRATCH_DB = "mongodb://localhost:27017/story_word_creatematter_cut";
process.env.PORT = "3799";
process.env.MONGODB_URI = SCRATCH_DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "creatematter-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "createmattercut-keys-" + process.pid);
...
try {
  ...
  const c = await create({ kind: "space", id: rootSpace }, { name: "hello.txt", content: "hello world\n" });
  c.result?.matterId && String(c.result?.spaceId) === rootSpace
    ? ok(`create-matter under root → matterId returned, spaceId = root (the .word resolve-birth-spec + emitBirth ran)`)
  ...
  cf && String(cf.of?.id) === String(c.result?.matterId) && String(cf.through) === String(I_AM)
    ? ok(`do:create-matter fact lands: of.id = the content-addressed matterId, through = the caller (I_AM)`)
  ...
  String(slot?.state?.spaceId) === rootSpace && slot?.state?.name === "hello.txt" && !!slot?.state?.type
    ? ok(`@matter folds: name "hello.txt", type "${slot?.state?.type}", spaceId = root`)
    : bad(`fold`, slot?.state);

  // ── 4. unique-name floor: a second nameless text matter gets a generated unique name ──
  const c2 = await create({ kind: "space", id: rootSpace }, { content: "two\n" });
  const slot2 = c2.result?.matterId ? await loadOrFold("matter", String(c2.result.matterId), "0") : null;
  c2.result?.matterId && slot2?.state?.name && slot2.state.name !== "hello.txt"
    ? ok(`second matter (no name) → generated unique name "${slot2.state.name}" (resolveMatterName floor ran in the .word)`)
    : bad(`unique name`, slot2?.state?.name);

  // ── 5. the no-actor gate refuses (the .word's "requires an identified actor") ──
  const n = await create({ kind: "space", id: rootSpace }, { content: "x" }, {});
  n.refused
