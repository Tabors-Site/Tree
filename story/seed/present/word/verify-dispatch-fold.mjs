...
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "dispatchfold-keys-" + process.pid,
);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "dispatchfold-src");
...
try {
  ...
  registerHostHandler("seed:test-fold-op", async (ctx) => {
    ranFromFold = true;
    return { ok: true, echo: ctx.params?.x ?? null };
  });

  // bind the word: a coin fact whose do-answer points at the ref. NOT in the operations Map.
  await bindWord("test-fold-op", {
    ownerExtension: "seed",
    do: { ref: "seed:test-fold-op" },
    targets: ["being"],
    factAction: "test-fold-op",
  });
  await new Promise((r) => setTimeout(r, 500));

  // it is absent from the operations Map, so a successful DO proves the fold path ran
  !getOperation("test-fold-op")
    ? ok(`"test-fold-op" is absent from the operations Map (fold-only)`)
  ...
  ranFromFold === true
    ? ok(`doVerb ran the handler resolved from the FOLD (not the Map)`)
    : bad(`handler ran from fold`, { ranFromFold });
  result && result.ok === true && result.echo === 42
    ? ok(`the fold handler got ctx + returned its result (echo=42)`)
    : bad(`result`, result);
  ...
  f
    ? ok(`the fold-driven op laid its do:test-fold-op fact (same audit path)`)
    : bad(`fact laid`, "no test-fold-op fact");
