...
const DB = "mongodb://localhost:27017/story_book_receive";
process.env.PORT = "3847"; process.env.MONGODB_URI = DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "bookrcv-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "bookrcv-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "bookrcv-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
try {
  ...
  let book = makeBook({
    ...
    body: { words: [{ name: WORD, kind: "op", binding: { do: { ref: "noop" } }, ownerExtension: "test-book" }] },
  });
  ...
  (v.ok && !v.unsigned && v.signers?.length === 1)
    ? ok(`book sealed + verifyColophon ok (signed by ${String(v.signers[0]).slice(0, 12)}…)`)
    : bad("seal/verify failed", v);

  // 2. The word does NOT exist yet.
  const before = await ws.getWord(WORD, "0");
  !before ? ok(`"${WORD}" is unknown before receive`) : bad("word already existed", before);

  // 3. receive() the book → receiveWords binds the coin (as I_AM on "0").
  const got = await receive(book, { history: "0" });
  (got.words === 1 && got.kind === "language")
    ? ok(`receive landed ${got.words} word(s), kind="${got.kind}", root=${String(got.root).slice(0, 12)}…`)
    : bad("receive did not land the word", got);

  // 4. The word now resolves (the coin folded).
  const after = await poll(() => ws.getWord(WORD, "0"), (w) => !!w);
  (after && after.word === WORD) ? ok(`"${WORD}" resolves after receive (coin folded)`) : bad("word did not resolve after receive", after);

  // 5. A TAMPERED book is refused before any plant (the seal catches it; nothing leaks).
  const book2base = makeBook({ title: "tampered", body: { words: [{ name: "tampered-word", kind: "op", binding: { do: { ref: "noop" } } }] } });
  const book2 = sealColophon(book2base);
  book2.body.words[0].name = "EVIL-SWAP"; // mutate AFTER sealing → root no longer matches
  let refused = false;
  try { await receive(book2, { history: "0" }); } catch (e) { refused = /colophon verification failed|root mismatch/i.test(e.message); }
  const leaked = await ws.getWord("EVIL-SWAP", "0");
  (refused && !leaked) ? ok("a tampered book is REFUSED before any word plants (no leak)") : bad("tamper not refused / leaked", { refused, leaked: !!leaked });
