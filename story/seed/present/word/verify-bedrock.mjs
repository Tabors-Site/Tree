...
process.env.PORT = "3845"; process.env.MONGODB_URI = DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "bedrock-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "bedrock-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "bedrock-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
console.log("\n  verify-bedrock (the I_AM genesis guard in wordStore)\n");
try {
  await poll(() => findByName("being", "cherub", "0"), (v) => !!v);
  // I_AM declares a "0" word (actorBeingId null => I_AM) — it becomes bedrock.
