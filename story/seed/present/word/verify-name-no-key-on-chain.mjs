...
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "namenokey-keys-" + process.pid);
...
try {
  ...
  await bindWord("name:leak-probe", { ownerExtension: "seed", kind: "nameop", do: { ref: "name-op:leak-probe" } });
  await new Promise((r) => setTimeout(r, 400));

  let ret;
  await withIAmAct("leak probe", async (moment) => {
    ret = await nameVerb("leak-probe", {}, { moment, identity: { name: "i-am", beingId: String(I_AM), nameId: String(I_AM) }, currentHistory: "0" });
  });

  const f = await pollFor(() => Fact.findOne({ verb: "name", act: "leak-probe" }).lean(), (v) => !!v);
  if (!f) { bad("the name:leak-probe fact was laid", "no fact"); }
  else {
    // 1. the fact carries NO result field at all (the structural omit)
    (!("result" in f) || f.result == null)
      ? ok(`the name-op fact has NO result field (the structural omit, not a redaction)`)
    ...
    const leaked = [PEM, RAW, PHRASE, "wordleak"].filter((s) => blob.includes(s));
    (leaked.length === 0)
      ? ok(`no key material anywhere in the stamped fact (pem / raw key / mnemonic all absent)`)
    ...
    (f.verb === "name" && f.act === "leak-probe" && f.of?.kind === "name")
      ? ok(`the fact still records the act (verb:name, act:leak-probe, of:{kind:name})`)
  ...
  (ret && ret.reveal && ret.reveal.privateKey === RAW)
    ? ok(`the verb RETURN still carries the reveal to the asker (key delivered off-chain)`)
    : bad(`asker receives reveal`, { reveal: ret?.reveal ? "present-but-wrong" : "missing" });
