...
const { emitWordFact, stampsFact } = await import(
  `${R}/seed/ibp/factResult.js`
);
...
try {
  ...
  Array.isArray(keep) &&
  keep.length === 2 &&
  keep.includes("beingAddress") &&
  keep.includes("note")
    ? ok(
        `resolveBeOpFromFold('connect').resultPolicy.keep = [beingAddress, note] (the production binding)`,
      )
    : bad(`real connect binding carries the allowlist`, {
        resultPolicy: binding?.resultPolicy,
      });
  ...
  const richResult = stampsFact(
    {
      beingAddress: "story/@y",
      note: "welcome",
      beingId: "B-target",
      name: "y",
      seatHistory: "0",
      firstUser: true,
      futureLeakField: "MUST-NOT-LAND",
  ...
  await withIAmAct("connect curate probe", async (moment) => {
    await emitWordFact(
      binding,
      { through: String(I), actId: moment?.actId || null, history: "0" },
      richResult,
      moment,
    );
  });
  ...
  if (!f) {
    bad("the be:connect fact was laid", "no fact");
  } else {
    const rk = f.result ? Object.keys(f.result).sort() : [];
    rk.length === 2 && rk[0] === "beingAddress" && rk[1] === "note"
      ? ok(
          `stamped fact.result has EXACTLY {beingAddress, note} (rich fields dropped)`,
        )
    ...
    const leaked = [
      ...
      "B-target",
      "firstUser",
    ].filter((s) => f.result && JSON.stringify(f.result).includes(s));
    leaked.length === 0
      ? ok(
          `no rich/secret field (futureLeak, token, beingId, firstUser) reached fact.result`,
        )
    ...
    f.params?.inhabitedBy === "B-target" && f.of?.kind === "being"
      ? ok(
          `the act is still fully recorded (params.inhabitedBy, of:{kind:being})`,
        )
