...
// .words, do-ops) onto the chain as I coin facts in one moment. After boot, asking for a
...
const DB = path.join(os.tmpdir(), "story_seedfold-" + process.pid);
process.env.PORT = "3835";
...
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "seedfold-keys-" + process.pid,
);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "seedfold-src");
...
const pollFor = async (fn, pred, t = 20000, e = 300) => {
  const t0 = Date.now();
  while (Date.now() - t0 < t) {
    const v = await fn();
    if (pred(v)) return v;
...
try {
  ...
  if (!cherub) {
    console.log("  FATAL: genesis failed (boot did not reach the delegates)");
    process.exit(1);
  }
  ok(
    `genesis completed past seedFold (boot reached the delegates, the fold did not crash it)`,
  );

  // 1. a CONCEPT word folds back from the chain as {kind:"concept", says}
  const being = await pollFor(
    () => getWord("being"),
    (v) => v && v.kind === "concept",
  );
  being && being.kind === "concept" && /being/i.test(being.says || "")
    ? ok(
        `concept "being" declared at genesis, folds back {kind:"concept", says}`,
      )
  ...
  word && word.kind === "concept"
    ? ok(`the root "word" declared too (the descent folded)`)
    : bad(`root word`, word);
  ...
  story.includes("spoke the word being") && /A being is a presence/.test(story)
    ? ok(
        `the story reads the seed: "spoke the word being: A being is a presence..."`,
      )
    : bad(
        `story renders the seed`,
        story.includes("spoke the word")
          ? "name shown but body missing"
          : "no coin rendered in the story",
      );
  ...
  for (const c of CONCEPTS) {
    const w = await getWord(c);
    if (w && w.kind === "concept") found++;
  }
  found === CONCEPTS.length
    ? ok(
        `all ${CONCEPTS.length} concept words declared (the full descent folded)`,
      )
    : bad(`all concepts`, `${found}/${CONCEPTS.length}`);

  // 6. the do-ops declared into the fold alongside the concepts
  const { factCount } = await import(
    `${R}/seed/present/word/_factStoreTest.mjs`
  );
  const opCount = factCount({
    verb: "do",
    act: "coin",
    history: "0",
    "params.binding.kind": "op",
  });
  opCount > 30
    ? ok(
        `${opCount} do-ops declared into the fold (the op set, beside the concepts)`,
      )
    : bad(`op count`, opCount);
