...
process.env.JWT_SECRET = process.env.JWT_SECRET || "createspace-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "createspacecut-keys-" + process.pid);
...
const create = async (target, params, who = ident) => {
  ...
  try {
    const res = await doVerb(target, "create-space", params, { identity: who, moment: sc, currentHistory: "0" });
    if (sc.deltaF.length) await sealFacts(sc.deltaF);
    // Mimic the real moment-seal: fire afterSeal (the parent-lock release) post-seal.
    for (const cb of (sc.afterSeal || [])) { try { await cb(); } catch { /* ignore */ } }
...
try {
  ...
  resolveAbleWord("space", "create-space") ? ok(`create.word resolves through the bridge (self-registered)`) : bad(`resolves`, "null");

  const rootSpace = String(getSpaceRootId());

  // ── 1. create a space under the place root → spaceId returned ──
  const c = await create({ kind: "space", id: rootSpace }, { name: "alpha", type: "2d" });
  c.result?.spaceId
    ? ok(`create-space under root → spaceId returned (the .word resolve-birth-space ran)`)
  ...
  cf && String(cf.of?.id) === spaceId && String(cf.through) === String(I_AM)
    ? ok(`do:create-space fact lands: of.id = spaceId, through = the caller (I_AM)`)
  ...
  p.name === "alpha" && String(p.parent) === rootSpace && nullish.length === 0
    ? ok(`birth params: name "alpha", parent = root, owner present (root), NO null terms`)
  ...
  dup.refused
    ? ok(`duplicate sibling name → refuse "${dup.refused.message?.slice(0, 40)}..."`)
  ...
  c2.result?.spaceId
    ? ok(`2nd create under root (name "beta") succeeds → the afterSeal lock-release works (no RESOURCE_CONFLICT)`)
    : bad(`lock-release`, c2.refused?.message || c2.result);

  // ── 7. the no-actor gate refuses (the .word's "requires an identified actor") ──
  const n = await create({ kind: "space", id: rootSpace }, { name: "gamma" }, {});
  n.refused
