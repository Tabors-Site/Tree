...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_call-" + process.pid);
...
try {
  ...
  // I calls @owner with a able-request — a hand-built call node (the parser surface lands later)
  const sc = {
    actId: randomUUID(),
    actorAct: { history: "0", by: "i-am" },
    identity: { beingId: I, name: "i-am", nameId: "i-am" },
    deltaF: [],
    foldedSeqs: new Map(),
    afterSeal: [],
  };
  ...
  const node = {
    ...
    being: { ref: "owner" },
    intent: "able-request",
    content: { able: "warrior", from: "i-am" },
    bind: "sent",
  };
  ...
  !err
    ? ok(`call @owner dispatched through callVerb (no throw) → reached`)
    : bad(`call dispatch`, err.message || err);
  // the reach is a RECORD: a fact landed (summon writes the reach through moment)
  const summonFact = (sc.deltaF || []).find(
    (f) => f.verb === "call" || /summon|reach|inbox/i.test(String(f.act)),
  );
  summonFact
    ? ok(
        `the reach laid a RECORD fact (verb:${summonFact.verb} action:${summonFact.act}) — through the stamper, not a bare emit`,
      )
    : bad(
        `reach record`,
        (sc.deltaF || []).map((f) => `${f.verb}:${f.act}`),
      );
  // attribution: the reach is the CALLER's act (i-am here)
  summonFact && String(summonFact.by || summonFact.through)
    ? ok(`reach attributed to the caller (a Name through a being)`)
    : bad(`attribution`, summonFact);
