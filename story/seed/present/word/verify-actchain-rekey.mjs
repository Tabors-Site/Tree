...
process.env.PORT = "3846"; process.env.MONGODB_URI = DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "actchain-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "actchain-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "actchain-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
try {
  ...
  heads.length > 0 ? ok(`genesis produced ${heads.length} act-chain head(s)`) : bad("no act-chain heads after genesis");
  let shapeOk = heads.length > 0, fieldsOk = true;
  for (const h of heads) {
    const seg = String(h._id).split(":");
    if (seg.length !== 3 || seg[0] !== story) shapeOk = false;
    if (h.story !== story || typeof h.history !== "string" || !h.beingId) fieldsOk = false;
    if (`${h.story}:${h.history}:${h.beingId}` !== String(h._id)) fieldsOk = false;
  }
  shapeOk ? ok(`every ActHead _id is "<story>:<history>:<being>" (story="${story}")`) : bad("an ActHead _id is not 3-segment story-prefixed", heads.map((h) => h._id).slice(0, 6));
  fieldsOk ? ok("every ActHead carries story/history/beingId fields that reconstruct its _id") : bad("an ActHead field disagrees with its _id", heads.slice(0, 3));

  // 2. VERIFY — verifyActChain(story, history, being) walks every chain clean.
  let allOk = true; const broken = [];
  for (const h of heads) { const v = await verifyActChain(h.story, h.history, h.beingId); if (!v.ok) { allOk = false; broken.push({ id: h._id, reason: v.reason, at: v.brokenAt }); } }
  allOk ? ok(`verifyActChain(story, history, being) walks all ${heads.length} chain(s) end-to-end clean`) : bad("a chain failed verifyActChain", broken.slice(0, 6));
  ...
  await ws.bindWord("actchain-probe-word", { kind: "op", do: { ref: "noop" } }, {}); // I_AM act on "0"
  const after = await ActHead.find({}).lean();
  const changed = after.filter((h) => before.get(String(h._id)) !== String(h.headHash));
  let advOk = changed.length > 0;
  for (const h of changed) { const v = await verifyActChain(h.story, h.history, h.beingId); if (!v.ok) advOk = false; }
  advOk ? ok(`a new act advanced ${changed.length} chain(s); each re-verifies clean (no fork, no interleave)`) : bad("no chain advanced / advanced chain failed verify", { changed: changed.map((h) => h._id) });
