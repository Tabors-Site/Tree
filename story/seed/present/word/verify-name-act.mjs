...
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "nameact-keys-" + process.pid,
);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "nameact-src");
...
try {
  ...
  const nameId = "i-am"; // the I name (loadSigningKey maps it to the story key)
  ...
  v.ok && v.count >= 1
    ? ok(`verifyActChain(story, "5d", name) walks ${v.count} name-act(s) clean`)
