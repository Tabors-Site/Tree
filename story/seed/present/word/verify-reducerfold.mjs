...
// ensureIAm to fold I_AM itself, before any reducer could be declared — the Map is the backstop.)
import fs from "fs"; import os from "os"; import path from "path"; import { fileURLToPath } from "url";
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const R = path.resolve(__dirname, "../../..");
const DB = "mongodb://localhost:27017/story_reducerfold";
process.env.PORT = "3843"; process.env.MONGODB_URI = DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "reducerfold-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "reducerfold-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "reducerfold-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
const { resolveReducerFromFold } = await import(`${R}/seed/present/word/wordStore.js`);
...
try {
  ...
  (types.length === 4 && types.every((t) => resolveReducerFromFold(t)))
    ...
    : bad(`fold coverage`, types.map((t) => [t, !!resolveReducerFromFold(t)]));
  const b = resolveReducerFromFold("being");
  (b && b.reduce === beingR.reduce && b.initial === beingR.initial)
    ? ok(`being-reducer resolves to the SAME host functions (ref identity, not a copy)`)
    : bad(`being ref identity`, { reduceMatch: b?.reduce === beingR.reduce, initialMatch: b?.initial === beingR.initial });
  (resolveReducerFromFold("matter").isGone === matterR.isGone)
    ? ok(`matter's isGone rides the fold (the optional field is carried)`)
    : bad(`matter isGone`, resolveReducerFromFold("matter").isGone === matterR.isGone);
  (resolveReducerFromFold("being").isGone === undefined)
    ? ok(`being has no isGone (optional field absent, matching the module)`)
    : bad(`being isGone should be absent`, resolveReducerFromFold("being").isGone);
  const r = reducers.get("matter");
  (typeof r.initial === "function" && typeof r.reduce === "function" && typeof r.isGone === "function")
    ? ok(`reducers.get("matter") fold-first returns a working {initial, reduce, isGone}`)
    : bad(`get fold-first`, { initial: typeof r.initial, reduce: typeof r.reduce, isGone: typeof r.isGone });
