...
const optsFor = (story, beingId, beingName, ctx) => ({
  address: `${story}/.@${beingName}`,
  addressKind: "stance",
  identity: { beingId, name: beingName },
  moment: ctx,
  currentHistory: "0",
});
console.log(
...
try {
  ...
  w?.kind === "ableword" && !!ir
    ? ok(
        `cherub:switch folds as kind:"ableword" + resolveAbleWord returns its IR (the word is the live path)`,
      )
  ...
  await withIAmAct("be:switch", async (ctx) => {
    res = await beVerb(
      "switch",
      { history: "0" },
      optsFor(story, beingId, beingName, ctx),
    );
  });
  res?.switched === true && res?.seatHistory === "0" && res?.toHistory === "0"
    ? ok(
        `beVerb returns switched:true + seatHistory="0" + toHistory (the seat inputs the transport reads)`,
      )
    : bad(`switch result`, {
        switched: res?.switched,
        seatHistory: res?.seatHistory,
        toHistory: res?.toHistory,
      });
  ...
  f &&
  f.params?.fromHistory === "0" &&
  f.params?.toHistory === "0" &&
  f.of?.kind === "being" &&
  f.result?.switched === true
    ? ok(
        `be:switch fact: params {fromHistory,toHistory}, of:{kind:"being"}, result.switched (the WORD authored the params; the dispatcher stamped on toHistory)`,
      )
    : bad(`the fact's authored params`, {
        params: f?.params,
        of: f?.of,
        switched: f?.result?.switched,
      });
  ...
  await withIAmAct("be:switch no-history", async (ctx) => {
    try {
      await beVerb("switch", {}, optsFor(story, beingId, beingName, ctx));
    } catch (e) {
      threw3 = true;
      code3 = e?.code || null;
    }
  ...
  threw3 && code3 === IBP_ERR.INVALID_INPUT
    ? ok(
        `gate: no history → ${IBP_ERR.INVALID_INPUT} (the .word If-no-history gate)`,
      )
    : bad(`no-history gate`, {
        threw: threw3,
        code: code3,
        want: IBP_ERR.INVALID_INPUT,
      });
