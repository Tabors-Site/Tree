...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_askable_cut-" + process.pid);
...
async function askAble(caller, able, space) {
  ...
  const sc = {
    actId: randomUUID(),
    actorAct: { history: branch, by: "i-am" },
  ...
  try {
    const res = await doVerb(
      ...
      "ask-able",
      { able },
      { identity: { beingId: String(caller) }, moment: sc },
    );
...
try {
  ...
  ir
    ? ok(`ask-able.word resolves through the bridge (self-registered)`)
    : bad(`ask-able resolves`, "null");
  ...
  await withIAmAct("create arena", async (ctx) => {
    const res = await doVerb(
      ...
      "create-space",
      { name: "askarena", type: "generic" },
      { identity: I, moment: ctx },
    );
    arena = String(res.spaceId);
  });
  const installAble = (name, acquisition) =>
    withIAmAct(`install ${name}`, async (ctx) => {
      await doVerb(
        { kind: "space", id: arena },
        "set-space",
        {
          field: `qualities.ables.${name}`,
          value: { canSee: [], canDo: [], canSummon: [], acquisition },
          merge: false,
        },
        { identity: I, moment: ctx },
      );
  ...
  await withIAmAct("set arena owner", async (ctx) => {
    await doVerb(
      { kind: "space", id: arena },
      "set-space",
      { field: "owner", value: String(owner), merge: false },
      { identity: I, moment: ctx },
    );
  });
  const asker = await birth("asker");
  arena
    ? ok(`arena: greeter(auto) + member(queue) + sage(false), owner @askowner`)
    : bad(`arena`, "no space");
  console.log(
    `  arena=${arena.slice(0, 10)} asker=${String(asker).slice(0, 10)} owner=${String(owner).slice(0, 10)}\n`,
  );
  ...
  a.result?.granted === true && a.result?.path === "auto"
    ? ok(`ask greeter (asked:auto) → granted:true, path:auto`)
    : bad(`auto granted`, a.refused?.message || a.result);
  const askFact = (a.deltaF || []).find(
    (f) => f.act === "ask-able" && f.params?.able === "greeter",
  );
  askFact && askFact.params?.grantedBy
    ? ok(
        `a do:ask-able fact laid carrying the grant record (applyAbleGrants folds the grant from the ask)`,
      )
    : bad(
        `ask fact + grant record`,
        a.deltaF?.map((f) => f.act),
      );
  const slot = await loadOrFold("being", String(asker), "0");
  (slot?.state?.qualities?.ablesGranted || []).some(
    (r) => (r.able || r) === "greeter",
  )
    ? ok(`@asker now HOLDS the greeter able`)
    : bad(`holds`, slot?.state?.qualities?.ablesGranted);
  ...
  const reAsk = (a2.deltaF || []).find(
    (f) => f.act === "ask-able" && f.params?.able === "greeter",
  );
  a2.result?.already === true && reAsk && !reAsk.params?.grantedBy
    ? ok(
        `re-ask → already:true; the ask IS stamped (do:ask-able, outcome:already) but no grant record → nothing re-folds`,
      )
    : bad(`idempotent re-ask`, {
        already: a2.result?.already,
        fact: reAsk?.params,
      });
  ...
  const queueAsk = (a3.deltaF || []).find(
    (f) => f.act === "ask-able" && f.params?.able === "member",
  );
  a3.result?.granted === false &&
  a3.result?.path === "queue" &&
  /Requested/i.test(a3.result?.message || "") &&
  queueAsk &&
  !queueAsk.params?.grantedBy
    ? ok(
        `ask member (asked:queue) → granted:false, path:queue, owner summoned; the ask IS stamped (do:ask-able, outcome:queue) but folds no grant`,
      )
  ...
  a4.refused && /not ask-acquirable/i.test(a4.refused.message)
    ? ok(`ask sage (asked:false) → refuse "not ask-acquirable"`)
    : bad(`asked false`, a4.refused?.message || a4.result);

  // ── 5. a able NOT installed → refuse "not installed" ──
  const a5 = await askAble(asker, "ghostable", arena);
  a5.refused && /not installed/i.test(a5.refused.message)
    ? ok(`ask an uninstalled able → refuse "not installed"`)
    : bad(`not installed`, a5.refused?.message || a5.result);
