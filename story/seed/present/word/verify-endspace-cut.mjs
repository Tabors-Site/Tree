...
process.env.PORT = "3842"; process.env.MONGODB_URI = SCRATCH_DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "endspace-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "endspacecut-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "endspacecut-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
try {
  ...
  const c = await runOp({ kind: "space", id: rootSpace }, "create-space", { name: "doomedspace", parent: rootSpace });
  const sId = String(c.result?.spaceId ?? c.result?.id ?? (await poll(() => findByName("space", "doomedspace", "0")))?.id);
  sId && sId !== "undefined" ? ok(`seeded a space (doomedspace)`) : bad(`seed`, c.refused?.message || c.result);
  ...
  (endFacts.length === 1 && setFacts.length === 0)
    ? ok(`end-space lays ONE do:end-space fact; zero set-space facts (the two-field false shape is gone)`)
    : bad(`one fact`, { acts: facts.map((f) => f.act) });

  const ef = endFacts[0];
  (ef && ef.verb === "do" && ef.of?.kind === "space" && String(ef.of?.id) === sId && String(ef.through) === String(I_AM))
    ? ok(`the do:end-space fact: of {space, spaceId}, through = the deleter (I_AM)`)
  ...
  (slot?.state?.parent === DELETED && String(slot?.state?.owner) === String(I_AM))
    ? ok(`the space folds: parent=DELETED (hidden) + owner=the deleter (revival audit) — derived from the one fact`)
    : bad(`folds deleted`, { parent: slot?.state?.parent, owner: slot?.state?.owner, position: slot?.state?.position });

  // ── it drops out of parent-query (hidden from children) ──
  const { listSpaceChildren } = await import(`${R}/seed/materials/space/spaces.js`).catch(() => ({}));
  if (typeof listSpaceChildren === "function") {
    ...
    (!stillChild) ? ok(`ended space no longer appears as a child of root (parent=DELETED hides it)`) : bad(`hidden`, { stillChild });
  } else { ok(`(listSpaceChildren not directly testable here — parent=DELETED fold asserted above)`); }
