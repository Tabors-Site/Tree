...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_renamematter_cut-" + process.pid);
process.env.PORT = "3840";
process.env.TREEOS_STORE_BASE = SCRATCH_DB;
fs.rmSync(SCRATCH_DB, { recursive: true, force: true });
process.env.JWT_SECRET =
  process.env.JWT_SECRET || "renamematter-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "renamemattercut-keys-" + process.pid,
);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "renamemattercut-src");
...
try {
  ...
  resolveAbleWord("matter", "rename-matter")
    ? ok(`rename-matter.word resolves through the bridge (self-registered)`)
    : bad(`resolves`, "null");
  const rootSpace = String(getSpaceRootId());

  // seed two matters in the root folder
  const a = await runOp({ kind: "space", id: rootSpace }, "create-matter", {
    name: "hello.txt",
    content: "a\n",
  });
  const b = await runOp({ kind: "space", id: rootSpace }, "create-matter", {
    name: "taken.txt",
    content: "b\n",
  });
  const aId = String(a.result?.matterId),
    bId = String(b.result?.matterId);
  aId && bId
    ? ok(`seeded two matters (hello.txt, taken.txt)`)
    : bad(`seed`, { a: a.refused?.message, b: b.refused?.message });
  ...
  r.result?.name === "renamed.txt" && slot?.state?.name === "renamed.txt"
    ? ok(
        `rename-matter via the .word → the row FOLDS the new name "renamed.txt" (the fix lands)`,
      )
    : bad(`rename`, {
        result: r.result,
        refused: r.refused?.message,
  ...
  collide.refused && /already in use/i.test(collide.refused.message || "")
    ? ok(
        `rename to a sibling's name → refuse "already in use" (the host uniqueness read ran)`,
      )
  ...
  const replace = await runOp(matterA, "rename-matter", {
    name: "taken.txt",
    allowReplace: true,
  });
  ...
  repf?.params?.value === "taken.txt"
    ...
    : bad(`allowReplace`, {
        ...
        refused: replace.refused?.message,
        factParams: repf?.params,
      });
