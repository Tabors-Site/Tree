...
try {
  ...
  resolveAbleWord("being", "add-llm-connection")
    ? ok(`add-llm-connection.word resolves through the bridge`)
    : bad(`resolves`, "null");
  ...
  const cA = await poll(
    async () => (conns(await being())[idA] ? conns(await being())[idA] : null),
    8000,
  );
  idA && cA && cA.name === "A"
    ? ok(
        `add A → connection folds (deed 1: do set-being): "${cA.name}", id ${String(idA).slice(0, 8)}…`,
      )
  ...
  m1
    ? ok(
        `main auto-assigned to A (deed 2 ran as its OWN moment — the multi-moment composite)`,
      )
  ...
  const cB = await poll(
    async () => (conns(await being())[idB] ? "y" : null),
    8000,
  );
  cB && idB !== idA
  ...
  mainSlot(await being()) === idA
    ? ok(
        `main STILL A after adding B (isFirst=false → the conditional assign deed skipped — not a run-on)`,
      )
