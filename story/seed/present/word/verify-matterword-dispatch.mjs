...
process.env.JWT_SECRET = process.env.JWT_SECRET || "matterworddispatch-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "matterworddispatch-keys-" + process.pid);
...
const { bindWord, resolveDoOpFromFold } = await import(`${R}/seed/present/word/wordStore.js`);
...
try {
  ...
  const ref = await putContent(BLOB, { mimeType: "application/javascript", name: "double.js" });
  const hash = ref?.hash || ref;
  hash ? ok(`stored the matter blob in CAS (hash ${String(hash).slice(0, 10)}…)`) : bad(`CAS put`, ref);

  // (1) bind a do-op whose BODY is matter — NO do.ref handler, just matter:{hash,type}
  await bindWord("matterword-double", {
    ownerExtension: "seed", kind: "op",
    matter: { hash: String(hash), type: "js", entry: "run" },
    targets: ["being"], factAction: "matterword-double",
  });
  await new Promise((r) => setTimeout(r, 500));

  // (2) it is absent from the operations Map (fold-only) AND resolveDoOpFromFold surfaces the matter body
  (!getOperation("matterword-double")) ? ok(`"matterword-double" is absent from the operations Map (fold-only)`) : bad(`should be Map-absent`, "found in Map");
  const spec = resolveDoOpFromFold("matterword-double");
  (spec && spec.matter && spec.matter.hash === String(hash) && spec.matter.type === "js" && !spec.handler)
    ? ok(`resolveDoOpFromFold surfaces the matter body {hash,type:js} and NO host handler`)
    : bad(`matter body surfaced`, { matter: spec?.matter, hasHandler: !!spec?.handler });
  ...
  await withIAmAct("run the matter word", async (moment) => {
    result = await doVerb(targetId, "matterword-double", { x: 21 }, { moment, identity: { name: "i-am", beingId: iam, nameId: iam }, currentHistory: "0" });
  });
  (result && result.computed === 42)
    ? ok(`the matter blob RAN through runMatterWord (computed 21*2=42, no host handler exists)`)
  ...
  const f = await pollFor(() => Fact.findOne({ verb: "do", act: "matterword-double" }).lean(), (v) => !!v);
  (f && f.params && f.params.computed === 42)
    ? ok(`the native word laid its do:matterword-double fact (params.computed=42, the word authored it)`)
    : bad(`fact laid with authored params`, f ? { params: f.params } : "no fact");
