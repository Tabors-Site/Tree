...
const SCRATCH_DB = "mongodb://localhost:27017/story_word_setrender_cut";
process.env.PORT = "3843"; process.env.MONGODB_URI = SCRATCH_DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "setrender-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "setrendercut-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "setrendercut-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
try {
  ...
  const c = await runOp({ kind: "space", id: rootSpace }, "create-matter", { name: "drum.txt", content: "tick\n" });
  ...
  (renderFacts.length === 1 && innerSetFacts.length === 0)
    ? ok(`set-render lays exactly ONE do:set-render fact; zero inner set-matter facts (the sugar is gone)`)
    : bad(`one fact`, { acts: facts.map((f) => f.act) });

  const rf = renderFacts[0];
  (rf && rf.verb === "do" && rf.of?.kind === "matter" && String(rf.of?.id) === mId && rf.params?.field === "qualities.render")
    ? ok(`the do:set-render fact: of {matter, id}, params.field = qualities.render (the qualities set the reducer folds)`)
    : bad(`fact shape`, rf ? { verb: rf.verb, of: rf.of, params: rf.params } : "no render fact");
  ...
  (slot?.state?.qualities?.render?.model === "seed:drum" && slot?.state?.qualities?.render?.scale === 2)
    ? ok(`qualities.render folds from the one fact (model="seed:drum", scale=2) — applySetQualities derived it`)
