...
const SCRATCH_DB = "mongodb://localhost:27017/story_word_takeable_live";
process.env.PORT = "3789";
process.env.MONGODB_URI = SCRATCH_DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "takeable-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "takeable-keys-" + process.pid);
...
try {
  ...
  takeFact ? ok(`a real do:take-able fact laid (the dispatcher's ONE auto-Fact, the .word self-emits nothing)`) : bad(`take fact`, t.deltaF?.map((f) => f.act));
  ...
  (slot?.state?.qualities?.ablesGranted || []).some((r) => (r.able || r) === "warrior") ? ok(`@taker now HOLDS the warrior able (ablesGranted) after seal`) : bad(`holds able`, slot?.state?.qualities?.ablesGranted);
  ...
  const reTake = (t2.deltaF || []).find((f) => f.act === "take-able" && f.params?.able === "warrior");
  (t2.result?.already === true && reTake && !reTake.params?.grantedBy) ? ok(`re-take → already:true; the take IS stamped (do:take-able, outcome:already) but no grant record → no duplicate fold`) : bad(`idempotent re-take`, { already: t2.result?.already, fact: reTake?.params });
