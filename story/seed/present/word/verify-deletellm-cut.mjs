...
process.env.JWT_SECRET = process.env.JWT_SECRET || "deletellm-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "deletellmcut-keys-" + process.pid);
...
process.env.CUSTOM_LLM_API_SECRET_KEY = process.env.CUSTOM_LLM_API_SECRET_KEY || "deletellm-llm-encryption-key-0123456789";
...
try {
  ...
  resolveAbleWord("being", "delete-llm-connection") ? ok(`delete-llm-connection.word resolves through the bridge`) : bad(`resolves`, "null");
  ...
  idA ? ok(`seeded conn A (auto-assigned to main): ${String(idA).slice(0, 8)}…`) : bad(`add A`, addA.refused?.message || addA.result);
  ...
  await did("set-being", T, { field: `qualities.beingLlm.slots.main`, value: idA });
  (mainSlot(await being()) === idA) ? ok(`main → A (seeded directly)`) : bad(`set main`, mainSlot(await being()));
  ...
  del.result && sf.length === 1
    ? ok(`delete → exactly ONE do:set-being fact (slot-clears run-on dropped; no 2nd slot fact)`)
    : bad(`one-fact`, del.refused?.message || { facts: sf.length, result: del.result });

  // ── 2. the fact unsets the connection (value:null at the right path) ──
  const f = sf[0];
  f && f.params?.field === `qualities.llmConnections.${idA}` && f.params?.value === null && String(f.through) === String(I_AM)
    ? ok(`do:set-being: field = llmConnections.<id>, value = null (unset), through = caller`)
    : bad(`fact`, f ? { field: f.params?.field, value: f.params?.value } : "no fact");
  ...
  (mainSlot(after) === idA)
    ? ok(`main still points at the deleted A (dangling — slot-clears run-on dropped, the spacebar)`)
    : bad(`dangling`, mainSlot(after));

  // ── 5. isFirst-liveness: the dangling main folds to empty → resolveConnectionSpec.isFirst = true ──
  const { resolveConnectionSpec } = await import(`${R}/seed/present/cognition/llm/connect.js`);
  const spec = await resolveConnectionSpec(String(I_AM), { name: "C", baseUrl: "https://example.com/c", model: "m", apiKey: "sk-c" }, { moment: { actorAct: { history: "0" } } });
  spec.isFirst === true
    ? ok(`isFirst-liveness: dangling main folds to empty (so a re-add auto-assigns to main)`)
