...
process.env.PORT = "3823"; process.env.MONGODB_URI = DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "statement-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "statement-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "statement-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
const callType = (socket, text) => new Promise((resolve) => handleType(socket, { id: "t", verb: "type", payload: { text } }, (resp) => resolve(resp)));
...
try {
  ...
  await withIAmAct("birth typist", async (m) => {
    const b = await birthBeing({ spec: { name: "typist", parentBeingId: cherub.id, homeId: cherub.state?.homeSpace, cognition: "scripted", defaultAble: "global" }, identity: I_AM, moment: m, history: "0" });
    typist = b.beingId;
  });
  ...
  r1?.status === "ok" ? ok(`valid Word accepted ("make notebook" → "I make notebook.")`) : bad(`valid accepted`, r1);
  const made = await poll(async () => (await Fact.countDocuments({ act: "create-space" })) > before, 8000);
  made ? ok(`a create-space fact landed under the typist`) : bad(`fact landed`, { before });
  const sp = await Fact.findOne({ act: "create-space", through: String(typist) }).lean();
  sp ? ok(`the new space is attributed to the typist (the "I")`) : bad(`attributed to typist`, "no create-space through typist");
  ...
  (r2?.status === "error" && r2?.error?.message) ? ok(`invalid Word REJECTED with a hint: "${r2.error.message}"`) : bad(`invalid rejected`, r2);
  await new Promise((r) => setTimeout(r, 600));
  const total2 = await Fact.countDocuments({});
  total2 === total ? ok(`nothing laid on rejection (the press failed clean, no fact forced)`) : bad(`no fact on reject`, { total, total2 });

  // 3. no being → refused
  const r3 = await callType({ beingId: null }, "make X");
  (r3?.status === "error" && /being/i.test(r3?.error?.message || "")) ? ok(`no being → refused (a Name needs a being to speak the Word)`) : bad(`no-being refused`, r3);
