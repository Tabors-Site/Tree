...
// word lays a permanent do:coin fact (every fact needs an ACTOR — I for the seed
...
try {
  ...
  declared
    ? ok(
        `boot auto-declared the seed vocabulary: ${declared} do:coin facts on the chain (no manual call)`,
      )
    : bad(`boot declare`, "no coin facts on the chain");

  // 2. a known word's declaration is a REAL fact, and it has an actor (I)
  const d1 = (await wordFacts()).find(
    (f) =>
      f.act === "coin" &&
      f.params?.word === WORD &&
      f.params?.binding?.kind === "ableword",
  );
  d1 && String(d1.through) === String(I)
    ? ok(`do:coin for ${ABLE}:${OP} on the chain, actor = I`)
    : bad(`declare fact`, d1);

  // 3. resolveAbleWord reads the projection → an IR (declared + backed). Stays synchronous.
  reg.resolveAbleWord(ABLE, OP)
    ? ok(`resolveAbleWord(${ABLE},${OP}) → IR (declared + backed, sync)`)
    : bad(`resolve`, "null");
  ...
  const disFact = (await wordFacts()).find(
    (f) => f.act === "retire" && f.params?.word === WORD,
  );
  !afterDisable && disFact
    ? ok(`disableWord → do:retire fact on the chain + resolveAbleWord → null`)
    : bad(`disable`, { resolved: !!afterDisable, disFact: !!disFact });
  ...
  !reg.resolveAbleWord(ABLE, OP)
    ? ok(
        `rehydrateWordsFromFacts → the disable persists (folded from the chain)`,
      )
    : bad(`rehydrate persists disable`, "resolved");

  // 6. RE-ENABLE → resolveAbleWord restored (a fresh coin, the fold's last action wins)
  await reg.enableWord(ABLE, OP, {});
  reg.resolveAbleWord(ABLE, OP)
    ? ok(`enableWord → resolveAbleWord restored (fresh coin, last action wins)`)
    : bad(`enable`, "null");

  // 7. the whole STORY is permanent on the chain: declare → disable → declare (nothing removed)
  const story = (await wordFacts())
    .filter((f) => f.params?.word === WORD)
    .map((f) => f.act);
  story.join(",") === "coin,retire,coin"
    ? ok(
        `the vocabulary's whole story is permanent: declare → disable → declare (you DISABLE, never delete)`,
      )
    : bad(`story`, story);
  ...
  after8 === before8
    ...
    : bad(`idempotent`, { before: before8, after: after8 });
  ...
  !moveOnBranch && moveOnMain
    ? ok(
        `move:move disabled on branch "${BR}" only → null on "${BR}", IR on main (on in one branch, off in another)`,
      )
    : bad(`per-branch disable`, {
        onBranch: !!moveOnBranch,
        onMain: !!moveOnMain,
      });

  // 10. rehydrate folds the per-branch state from the chain (by the disable fact's branch)
  await reg.rehydrateWordsFromFacts();
  !reg.resolveAbleWord("move", "move", BR) &&
  reg.resolveAbleWord("move", "move", "0")
    ? ok(
        `rehydrate preserves per-branch state: "${BR}" off, main on (folded by the fact's branch)`,
      )
    : bad(`per-branch rehydrate`, {
        onBranch: !!reg.resolveAbleWord("move", "move", BR),
        onMain: !!reg.resolveAbleWord("move", "move", "0"),
      });

  // 11. re-enable on the branch → restored there too
  await reg.enableWord("move", "move", { history: BR });
  reg.resolveAbleWord("move", "move", BR)
    ? ok(`enable move:move on "${BR}" → restored on "${BR}"`)
    : bad(`per-branch enable`, "null");
