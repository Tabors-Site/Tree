...
process.env.PORT = "3793"; process.env.MONGODB_URI = DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "closestory-0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "closestory-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "closestory-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
process.kill = (pid, sig) => { if (sig === "SIGTERM" && pid === process.pid) return true; return _kill(pid, sig); };
...
const bad = (l, d) => { fail++; console.log("  ✗ " + l); if (d !== undefined) console.log("      " + JSON.stringify(d).slice(0, 240)); };
const poll = async (fn, t = 30000, e = 250) => { const t0 = Date.now(); while (Date.now() - t0 < t) { const v = await fn(); if (v) return v; await new Promise((r) => setTimeout(r, e)); } return null; };
...
const did = async (op, target, params, who = ident) => {
  ...
  try { const res = await doVerb(target, op, params, { identity: who, moment: sc, currentHistory: "0" }); if (sc.deltaF.length) await sealFacts(sc.deltaF); return { result: res?.result ?? res, deltaF: sc.deltaF, refused: null }; }
...
try {
  ...
  c.result?.closing === true && doFacts.length === 0
    ? ok(`close-story → closing:true, ZERO do-facts in the op moment (ranAsMoments; no skipAudit double-stamp)`)
    : bad(`close`, c.refused?.message || { result: c.result, doFacts: doFacts.map((f) => f.act) });

  // ── 2. the close-story NAME-ACT landed on the library reel (verb:name, bodiless, by the Name) ──
  const cf = await poll(() => Fact.findOne({ "of.kind": "library", "of.id": story, act: "close-story", verb: "name" }).lean());
  cf && cf.through == null && cf.by && String(cf.params?.closedBy) === String(I_AM)
    ? ok(`close-story name-act on the library reel: verb=name, through=null, by=${cf.by}, params.closedBy=I_AM`)
    : bad(`name-act`, cf ? { verb: cf.verb, through: cf.through, by: cf.by, params: cf.params } : "no fact on library reel");
  ...
  const g1 = await did("create-space", { kind: "space", id: rootSpace }, { name: "after-close" });
  g1.refused && /closed/i.test(g1.refused.message || "")
    ? ok(`post-close act REFUSED (in-process latch): "${g1.refused.message?.slice(0, 50)}..."`)
  ...
  const { _resetStoryClosedLatchForTest } = await import(`${R}/seed/storyLifecycle.js`);
  _resetStoryClosedLatchForTest();
  const g2 = await did("create-space", { kind: "space", id: rootSpace }, { name: "after-restart" });
  g2.refused && /closed/i.test(g2.refused.message || "")
    ? ok(`post-close act REFUSED via the library-reel fact read (restart path): "${g2.refused.message?.slice(0, 40)}..."`)
