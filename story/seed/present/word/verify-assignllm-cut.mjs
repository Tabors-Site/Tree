...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_assignllm_cut-" + process.pid);
process.env.PORT = "3794";
process.env.TREEOS_STORE_BASE = SCRATCH_DB;
fs.rmSync(SCRATCH_DB, { recursive: true, force: true });
delete process.env.MONGODB_URI;
process.env.JWT_SECRET =
  process.env.JWT_SECRET || "assignllm-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "assignllmcut-keys-" + process.pid,
);
...
process.env.CUSTOM_LLM_API_SECRET_KEY =
  process.env.CUSTOM_LLM_API_SECRET_KEY ||
  "assignllm-llm-encryption-key-0123456789ab";
...
try {
  ...
  resolveAbleWord("being", "assign-llm-slot")
    ? ok(`assign-llm-slot.word resolves (being)`)
    : bad(`resolves`, "null");
  ...
  const addB = await did("add-llm-connection", T, {
    name: "B",
  ...
  const asg = await did("assign-llm-slot", T, {
    slot: "main",
    connectionId: idB,
  });
  const m = await poll(
  ...
  asg.result && m
    ? ok(
        `assign main → B: being-branch deed fired (do set-being at beingLlm.slots.main), main now B`,
      )
    : bad(
        `assign`,
        asg.refused?.message || { main: mainSlot(await being()), want: idB },
      );
  ...
  const cleared = await poll(
    async () => (mainSlot(await being()) == null ? "y" : null),
    8000,
  );
  clr.result && cleared
