...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_endmatter_cut-" + process.pid);
process.env.PORT = "3841";
process.env.TREEOS_STORE_BASE = SCRATCH_DB;
fs.rmSync(SCRATCH_DB, { recursive: true, force: true });
process.env.JWT_SECRET =
  process.env.JWT_SECRET || "endmatter-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "endmattercut-keys-" + process.pid,
);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "endmattercut-src");
