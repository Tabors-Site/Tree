...
// .histories-resolved gates) is .word; the heaven read + the lone pointer-map set-space
// stay host. Proves: a pointer is removed (the .histories map drops it), an absent pointer
...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_delpointer_cut-" + process.pid);
...
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "delpointercut-keys-" + process.pid,
);
...
try {
  ...
  resolveAbleWord("history-manager", "delete-pointer")
    ? ok(`delete-pointer.word resolves through the bridge`)
    : bad(`resolves`);
  ...
  d.result?.deleted === true && d.result?.name === "release-x"
    ? ok(`delete release-x → deleted:true`)
    : bad(`delete`, d.refused?.message || d.result);
  const map = await readPointers();
  !Object.prototype.hasOwnProperty.call(map || {}, "release-x")
    ? ok(`.histories map no longer has release-x`)
    : bad(`map`, map);

  // ── 2. delete an ABSENT pointer → no-op, alreadyAbsent (no fact) ──
  const d2 = await drive("delete-pointer", { name: "ghostptr" });
  d2.result?.deleted === false &&
  d2.result?.alreadyAbsent === true &&
  !(d2.deltaF || []).some((f) => f.act === "set-space")
    ? ok(
        `delete an ABSENT pointer → deleted:false, alreadyAbsent:true, NO set-space fact`,
      )
  ...
  d3.refused &&
  /reserved/i.test(d3.refused.message) &&
  d3.refused.code === "FORBIDDEN"
    ? ok(`delete "main" (reserved) → refuse "reserved" [FORBIDDEN]`)
    : bad(`reserved`, d3.refused?.message || d3.result);

  // ── 4. delete a structurally-invalid name → refuse, exact code ──
  const d4 = await drive("delete-pointer", { name: "1bad" });
  d4.refused &&
  /invalid/i.test(d4.refused.message) &&
  d4.refused.code === "INVALID_INPUT"
    ? ok(`delete "1bad" (invalid) → refuse "invalid" [INVALID_INPUT]`)
    : bad(`invalid`, d4.refused?.message || d4.result);
