#!/usr/bin/env node
// set-pointer (history-manager.word), LIVE through the bridge with ZERO stubs. The CONTROL
// strand (caller gate, name/canonical validation, .histories resolution) is .word; the two
// regex validators + the heaven reads (readPointers / findPointersSpaceId) + the lone
// set-space onto .histories qualities.pointers are host: escapes wired by historyManagerHost.
...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_setpointer_cut-" + process.pid);
process.env.PORT = "3791";
process.env.TREEOS_STORE_BASE = SCRATCH_DB;
fs.rmSync(SCRATCH_DB, { recursive: true, force: true });
delete process.env.MONGODB_URI;
process.env.JWT_SECRET =
  process.env.JWT_SECRET || "setpointer-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "setpointercut-keys-" + process.pid,
);
...
async function setPointer(name, canonical) {
  ...
  try {
    const res = await doVerb(
      { kind: "being", id: String(cherub.id) },
      "set-pointer",
      { name, canonical },
...
try {
  ...
  a.result?.set === true &&
  a.result?.name === "release-v2" &&
  a.result?.canonical === "1a2" &&
  a.result?.previous === null
  ...
  (a.deltaF || []).some(
    (f) =>
      f.act === "set-space" &&
      f.params?.field === "qualities.pointers" &&
      f.params?.value?.["release-v2"] === "1a2",
  )
    ? ok(
        `one do:set-space at qualities.pointers carrying release-v2→1a2 (the lone WORLD fact)`,
      )
    : bad(
        `fact`,
        (a.deltaF || []).map((f) => `${f.act}:${f.params?.field}`),
      );

  // ── 3. the .histories heaven map folds the pointer ──
  const map = await readPointers();
  map?.["release-v2"] === "1a2"
    ? ok(`.histories pointers folds release-v2 → 1a2`)
    : bad(`fold`, map);

  // ── 4. re-point → previous is the prior canonical ──
  const b = await setPointer("release-v2", "7b3");
  b.result?.set === true && b.result?.previous === "1a2"
    ? ok(
        `re-point release-v2 → 7b3 → previous:"1a2" (the merge read the prior)`,
      )
    : bad(`re-point`, b.refused?.message || b.result);

  // ── 5. invalid name → refuse, exact code (refuse-code form flowing) ──
  const c = await setPointer("1bad", "0");
  c.refused &&
  /invalid/i.test(c.refused.message) &&
  !(c.deltaF || []).some((f) => f.act === "set-space")
    ? ok(`set "1bad" → refuse "invalid" [code ${c.refused.code}], NO fact`)
    : bad(`refuse name`, c.refused?.message || c.result);
  c.refused?.code === "INVALID_INPUT"
    ? ok(`refuse carries code INVALID_INPUT (refuse-code form end-to-end)`)
    : bad(`refuse code`, c.refused?.code);
