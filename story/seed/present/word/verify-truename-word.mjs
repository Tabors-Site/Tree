...
try {
  ...
  w?.kind === "ableword" && !!ir
    ? ok(
        `cherub:truename folds as kind:"ableword" + resolveAbleWord returns its IR (the word is the live path)`,
      )
  ...
  await withIAmAct("declare a name", async (ctx) => {
    const r = await nameVerb(
      "declare",
      { soulType: "scripted" },
      { identity: I, moment: ctx, currentHistory: "0" },
    );
    nameId = r.nameId;
  });
  await pollFor(
    () => loadProjection("name", nameId, "0"),
    (s) => !!s?.state?.privateKeyEnc,
  );
  let beingId = null;
  ...
  const after = await pollFor(
    () => loadProjection("being", beingId, "0"),
    (s) => s?.state?.trueName === nameId,
  );
  after?.state?.trueName === nameId
    ? ok(
        `be:truename folded @tnbeing.trueName to the declared Name (applyTrueName ran)`,
      )
  ...
  f &&
  f.params?.trueName === nameId &&
  f.of?.kind === "being" &&
  f.result?.trued === true
    ? ok(
        `be:truename fact: params.trueName === resolved nameId, of:{kind:"being"}, result.trued (the WORD authored the params; the dispatcher stamped)`,
      )
    : bad(`the fact's authored params`, {
        params: f?.params,
        of: f?.of,
        resultTrued: f?.result?.trued,
        wantName: nameId,
      });
  ...
  const r2 = await expectRefusal(
    "truename to a ghost being",
    { trueName: nameId },
    `${story}/.@ghostbeing404`,
  );
  r2.threw && r2.code === IBP_ERR.BEING_NOT_FOUND
    ? ok(
        `gate: missing target being → ${IBP_ERR.BEING_NOT_FOUND} (the .word If-no-targetId gate, be.js-equivalent)`,
      )
