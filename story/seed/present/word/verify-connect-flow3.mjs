...
// being-takeover attack the JS guards, which the .word's old conds would have allowed),
...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_connect_flow3-" + process.pid);
process.env.PORT = "3792";
process.env.TREEOS_STORE_BASE = SCRATCH_DB;
fs.rmSync(SCRATCH_DB, { recursive: true, force: true });
process.env.JWT_SECRET =
  process.env.JWT_SECRET || "connectflow3-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "connectflow3-keys-" + process.pid,
);
...
async function inherit(targetName, caller) {
  ...
  const moment = {
    actId: randomUUID(),
    actorAct: { branch },
    identity: { beingId: caller?.beingId || "x" },
    deltaF: [],
    foldedSeqs: new Map(),
    afterSeal: [],
  };
  ...
  try {
    ...
    if (e && e.__wordRefusal)
      return { result: null, deltaF: moment.deltaF, refused: e };
    throw e;
  }
}
...
try {
  ...
  inheritFlow
    ? ok(`selectConnectFlow("inherit") finds flow 3`)
    : bad(`flow 3 selection`, "null");

  const daddy = await birth("daddy");
  const descendant = await birth("childbeing", { parentBeingId: daddy });
  const localbeing = await birth("localbeing", {
    father: { story: localDomain, beingId: daddy },
  });
  const FOREIGN = "did:web:foreign.example.com";
  const crossbeing = await birth("crossbeing", {
    father: {
      story: FOREIGN,
      beingId: "F-foreign-being",
      nameId: "z6MkVictimName",
    },
  });
  console.log(
    `  daddy=${String(daddy).slice(0, 10)} descendant/localbeing/crossbeing born\n`,
  );
  ...
  anc.result?.inherited === true && !anc.result?.asFather && anc.result?.token
    ? ok(
        `ancestor: daddy inherits @childbeing → inherited:true, asFather:false, token`,
      )
    : bad(`ancestor`, anc.refused?.message || anc.result);

  // ── 2. LOCAL FATHER: daddy is the local father of the being (beingId is the credential) ──
  const lf = await inherit("localbeing", {
    beingId: daddy,
    story: localDomain,
  });
  lf.result?.asFather === true && lf.result?.token
    ? ok(`local father: daddy admits @localbeing → asFather:true, token`)
    : bad(`local father`, lf.refused?.message || lf.result);

  // ── 3. LEGIT CROSS-STORY FATHER: proven NAME + verified sig ──
  const xf = await inherit("crossbeing", {
    beingId: "F-foreign-being",
    story: FOREIGN,
    nameId: "z6MkVictimName",
    beingSigVerified: true,
  });
  xf.result?.asFather === true
    ? ok(`cross father (proven name + verified sig) → asFather:true`)
    : bad(`cross father legit`, xf.refused?.message || xf.result);
  ...
  const attack = await inherit("crossbeing", {
    beingId: "F-foreign-being",
    story: FOREIGN,
    nameId: "z6MkAttackerName",
    beingSigVerified: true,
  });
  attack.refused
    ? ok(
        `SECURITY: cross caller with victim's beingId but wrong name → REFUSED (no beingId-takeover)`,
      )
    : bad(`SECURITY HOLE`, `attacker got in: ${JSON.stringify(attack.result)}`);

  // ── 5. NEGATIVE: a stranger (neither ancestor nor father) → refuse ──
  const stranger = await inherit("localbeing", {
    beingId: "nobody-unrelated",
    story: localDomain,
  });
  stranger.refused
    ? ok(`stranger (not ancestor, not father) → refused`)
    : bad(`stranger refused`, stranger.result);
