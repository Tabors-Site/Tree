...
console.log(
  `the acts, with the actor surfaced (by the NAME, through the being):\n`,
);
for (const e of flow.effects) {
  const thr = e.through ? ` through ${e.through}` : "";
  const note = e.params?.trueName
    ? `  [being's trueName = ${e.params.trueName === "$ownerName" ? ctx.bindings.ownerName : e.params.trueName}]`
    : e.params?.value?.mother
      ? `  [mother ${e.params.value.mother}, father ${e.params.value.father}]`
      : "";
...
for (const f of facts)
  console.log(
    `  ${f.verb}:${f.action} -> ${f.target ? f.target.kind + ":" + (f.target.id ?? "") : ""}`,
  );
console.log("");
