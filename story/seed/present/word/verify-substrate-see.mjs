...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_substratesee-" + process.pid);
process.env.PORT = "3799";
process.env.TREEOS_STORE_BASE = SCRATCH_DB;
fs.rmSync(SCRATCH_DB, { recursive: true, force: true });
process.env.JWT_SECRET =
  process.env.JWT_SECRET || "substratesee-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "substratesee-keys-" + process.pid,
);
...
try {
  ...
  cherub
    ? ok(`genesis booted with the 4 converted .words loaded (cherub present)`)
    : bad(`genesis`, "no cherub — a converted .word failed to load");
  if (!cherub) {
  ...
  const cm = await drive({ kind: "space", id: root }, "create-matter", {
    name: "note-see",
    content: "hello from the see-sweep",
  });
  cm.result?.matterId && cm.result?.spaceId === root
    ? ok(
        `create-matter → matterId ${String(cm.result.matterId).slice(0, 12)}…, spaceId=root (see:resolve-birth-spec dispatched, emitBirth laid)`,
      )
  ...
  m?.state?.name
    ? ok(
        `@matter folds: name="${m.state.name}" (the birth spec resolved + wrote through)`,
      )
    : bad(`matter fold`, m?.state);

  console.log(`\n  ${pass} passed, ${fail} failed`);
  console.log(
    `  (move/set-render/form-portal: construction-verified — parse→see:<op>, env key renamed to match,`,
  );
  console.log(
    `   same see-op runner; dedicated behavior cuts are a 7.md follow-up.)`,
  );
