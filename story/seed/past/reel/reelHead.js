// TreeOS Seed . AGPL-3.0 . https://treeos.ai . Tabor Holly
//
// ReelHead. The per-aggregate, per-history seq counter.
//
// Every aggregate (being, space, matter) has a reel — its own
// append-only chain of facts — in every history where it has been
// written to. The reel needs a head: the highest seq number stamped
// on it in THAT history. `allocSeq(type, id, { history })` is the only
// path that advances a head, and it advances atomically (single-doc
// `$inc` with upsert on first allocation).
//
// Per-history heads are how divergent worlds share history without
// stepping on each other. Each history (#0 = main, #1, #1a, #1a1...)
// has its own ReelHead doc per aggregate. The first fact in history X
// for reel Y allocates seq = branchPoint[Y] + 1 — picking up where
// the parent's reel left off at branch time.
//
// Why a separate collection and not a field on the aggregate row:
// per [STAMPER.md](../factory/stamper/STAMPER.md) the projection cache
// (the Space/Being/Matter row) is read-side, written only by `fold`.
// Putting the write-side counter on the same row couples writer and
// reader. Dedicated collection keeps the projection pure.
//
// `_id` shape: `<history>:<type>:<id>` for ALL histories including main
// (so `0:being:xyz` for tabor on main, `1a:being:xyz` for tabor on
// history #1a). Pre-history legacy rows used `<type>:<id>` without the
// history prefix; a one-shot boot migration rewrites them to the new
// shape with `history: "0"`. See seed/seedStory/migrations/.

import mongoose from "mongoose";

const ReelHeadSchema = new mongoose.Schema({
  _id:    { type: String },           // "<history>:<type>:<id>"
  history: { type: String, required: true, default: "0", index: true },
  type:   { type: String, required: true, enum: ["being", "space", "matter", "library"] },
  id:     { type: String, required: true },
  head:   { type: Number, required: true, default: 0 },
  // The reel's ROOT HASH: the head fact's identity (`_id`), which
  // commits to every prior fact on the reel. Written by the stamper
  // in the same locked append as the fact insert. Null until the
  // first post-CAS fact lands (allocSeq never touches it). History /
  // story roots roll up from these — see past/fact/chainRoots.js.
  headHash: { type: String, default: null },
});

// Sparse compound index for the common history-scoped lookup. The _id
// shape already supports this query, but the explicit index makes
// queries that filter by (history, type, id) without _id (e.g.
// migration scans) fast.
ReelHeadSchema.index({ history: 1, type: 1, id: 1 }, { unique: true });

const ReelHead = mongoose.model("ReelHead", ReelHeadSchema, "reelHeads");
export default ReelHead;
