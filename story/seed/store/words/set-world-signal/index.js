...
import {
  ...
  resolveAbleWord,
  runAbleWord,
} from "../../../present/word/ableWordRegistry.js";
...
// publisher, not I. The op no longer self-emits — the `.word` lays no fact and
...
registerOperation("set-world-signal", {
  ...
  handler: async ({ params, identity, moment }) => {
    ...
    const viaWord = await _setWorldSignalViaWord({
    ...
    if (!key) {
      throw new IbpError(
        IBP_ERR.INVALID_INPUT,
        "set-world-signal: `key` is required",
      );
    }
    ...
    if (!rootId) {
      throw new IbpError(
        IBP_ERR.INTERNAL,
        "set-world-signal: story root not initialized",
      );
    }
