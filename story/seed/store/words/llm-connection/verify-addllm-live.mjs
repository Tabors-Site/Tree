...
process.env.PORT = "3847"; process.env.MONGODB_URI = DB;
process.env.JWT_SECRET = process.env.JWT_SECRET || "addllm-0123456789";
process.env.CUSTOM_LLM_API_SECRET_KEY = process.env.CUSTOM_LLM_API_SECRET_KEY || "addllm-secret-0123456789abcdef0123456789";
process.env.STORY_KEY_DIR = path.join(os.tmpdir(), "addllm-keys-" + process.pid);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
const SRC = path.join(os.tmpdir(), "addllm-src"); fs.rmSync(SRC, { recursive: true, force: true }); fs.mkdirSync(SRC, { recursive: true }); fs.writeFileSync(path.join(SRC, "x.txt"), "x\n");
...
const addConn = async (being, name) => {
  await withBeingAct(String(being), "add llm connection", "0", async (m) => {
    await doVerb({ kind: "being", id: String(being) }, "add-llm-connection",
      { name, baseUrl: "https://api.openai.com/v1", apiKey: "sk-secret-" + name, model: "gpt-4" },
      { identity: { beingId: String(being), name: "llmbeing" }, moment: m, currentHistory: "0" });
  });
...
try {
  ...
  await withIAmAct("birth llm being", async (m) => {
    const b = await birthBeing({ spec: { name: "llmbeing", parentBeingId: cherub.id, homeId: cherub.state?.homeSpace, cognition: "scripted", defaultAble: "global" }, identity: I_AM, moment: m, history: "0" });
    being = b.beingId;
  });
  ...
  (a1 - a0 === 2)
    ? ok(`first add → chain GREW BY 2 (${a0}→${a1}): the connection + the auto-assign, each its own moment`)
    : bad(`first add → 2 moments`, { a0, a1 });

  // the deeds carry distinct actIds (two moments, not one run-on)
  const facts1 = await Fact.find({ through: String(being) }).select("actId act").lean();
  ...
  (Object.keys(conns).length === 1) ? ok(`one connection folded onto the being`) : bad(`one connection`, { conns: Object.keys(conns) });
  (beingLlm?.slots?.main) ? ok(`main slot auto-assigned (the If $conn.isFirst deed fired)`) : bad(`main slot assigned`, { beingLlm });

  // no plaintext key on the chain (encrypt rode the first deed; redact is separate)
  const leak = facts1.some((f) => JSON.stringify(f).includes("sk-secret-openai"));
  !leak ? ok(`no plaintext apiKey on the chain (encrypted in the deed)`) : bad(`apiKey leak`, "plaintext on a fact");
  ...
  (b1 - b0 === 1)
    ? ok(`second add → chain GREW BY 1 (${b0}→${b1}): the conditional deed did NOT fire (isFirst false)`)
