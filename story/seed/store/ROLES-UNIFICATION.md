...
| | wordStore.js | roleWordRegistry.js |
|---|---|---|
| fact params | `{ word, ownerExtension, binding }` | `{ role, op, source }` |
| projection | `_projection` (Map, heaven "0" only, sync) | `REGISTRY` (Map) + `_historyDisabled` (per-branch, sync) |
| read | `getWord` (async, per-branch) / `getWordSync` ("0" only) | `resolveRoleWord` (sync, per-branch, returns the parsed IR) |
| declare | `bindWord` (+ `declareOpsToFold` etc.) | `registerRoleWord` (buffer) + `declareWordsToChain` |
| disable | `disableWord(name)` | `disableWord(role, op)` / `enableWord(role, op)` |
...
| host extras | host-handler table (ref -> fn) | `irCache` + `wordOf`, `runRoleWord`, `bornBeingFrom` |

The duplication is the declare/disable/rehydrate/projection machinery. roleWordRegistry has THREE
things wordStore does not: a SYNC per-branch enabled-state overlay, I_AM bedrock protection, and the
role-specific host layer (IR cache + the evaluator runner + the birth-cut reader).

## Caller surface (what must keep working)

- `registerRoleWord(role, op, url)` — ~13 store-word bundles self-register at module load
  (create-matter, move, credential x4, portal, set-world-signal, grant-role, key, model, acquisition,
  set-render, branch-pointers, cherub). Pre-genesis (the chain is not writable yet).
- `resolveRoleWord(role, op, branch)` + `runRoleWord(ir, {...})` — every bundle's `_viaWord` handler
...
- `assemble.js` (the book view) imports roleWordRegistry — AUDIT its exact usage (WP-6).

DESIGN LOCK: keep this public API byte-for-byte. The ~13 bundles + the bridge do NOT change; only
roleWordRegistry's BACKEND swaps from its own REGISTRY/declare/rehydrate to wordStore's fold.

## The unified model

A role-word becomes a wordStore word, `kind:"roleword"`:

- word NAME = the role:op key (`"cherub:birth"`, `"being:grant-role"`, `"matter:create-matter"`, ...).
- binding = `{ kind:"roleword", role, op, source }` (source = the `.word` file URL string).
- wordStore is the ONE fold: `bindWord` declares, `disableWord` disables, `getWord`/`getWordSync` read.
- roleWordRegistry becomes a thin ROLE-LAYER over wordStore: registration buffer + IR resolution +
  the evaluator runner + the birth reader. It no longer owns a fact-emission path or a REGISTRY Map.

`resolveRoleWord(role, op, branch)` then = read wordStore's sync projection for `role:op` (existence +
enabled-on-branch) -> the `source` -> `wordOf(source)` (IR cache, unchanged) -> the parsed IR, or null.
`runRoleWord` / `bornBeingFrom` / `irCache` are HOST (the bottom turtle); they stay verbatim.

## The gap, and the two ways to close each half

### Gap A — the SYNC per-branch overlay
`resolveRoleWord` is synchronous and per-branch (a word can be disabled on branch X but live on "0").
wordStore's `getWordSync` is "0"-only; `getWord` is per-branch but async. Two options:

- A1 (smaller, recommended first): roleWordRegistry KEEPS `_historyDisabled` as a thin sync overlay.
  EXISTENCE (declared + backed) comes from `wordStore.getWordSync(role:op)`; the per-branch
...
- A2 (full): wordStore grows a sync per-branch read — `getWordSync(name, branch)` backed by a
  per-branch disabled overlay in the projection. Then resolveRoleWord is pure wordStore, and the
...
(project_iam_genesis_immutable), not a roleword quirk. Recommended: MOVE it into `wordStore.bindWord`
...
- WP-1 — roleword binding + key scheme. Define `kind:"roleword"` + the `role:op` name. VERIFY no
  ...
  authorize ext-scope gate (it reads `args.action`, an op name, never a roleword key — confirm).
- WP-2 — declareRoleWordsToFold (wordStore). Mirror declareOpsToFold: read roleWordRegistry's pending
  registrations, `bindWord(role:op, {kind:"roleword", role, op, source})`. Wire in seedFold beside
  the ops/types/reducers (the role bundles register at module load, so they are present early) +
  the boot-end pass for extension role-words. `declareWordsToChain` becomes a thin call to this.
- WP-3 — resolveRoleWord reads wordStore. Existence from `getWordSync(role:op)` (the binding's
  `source`), enabled-state from Gap-A1's overlay. `wordOf`/`irCache` unchanged. Delete REGISTRY.
- WP-4 — disable/enable + rehydrate delegate. `disableWord(role,op)` -> `wordStore.disableWord(role:op)`
  + flip the overlay; `enableWord` -> `bindWord` re-declare. `rehydrateWordsFromFacts` reads
  wordStore's roleword facts (existence) + builds `_historyDisabled` from the per-branch disable facts.
- WP-5 — bedrock to wordStore (Gap B). Move `_assertMayChange` into `wordStore.bindWord`/`disableWord`
  as a general I_AM-genesis-on-"0" guard; delete roleWordRegistry's `_genesisWords`/`_assertMayChange`.
- WP-6 — audit assemble.js + the book view. Confirm/repoint its roleWordRegistry usage onto the
  unified read (listFoldedRolewords or getWord). Any other inspector too.
- WP-7 — verification (the hot-path gate). Full boot. Then the bridge verifiers green: verify-cherub-*
  (birth), verify-connect-*, verify-take-role-*, verify-grantrole-cut, verify-credread/credreset-cut,
  verify-setpointer/deletepointer-cut, verify-setworldsignal-cut, verify-setmodel-cut,
  verify-creatematter-cut, verify-bridge-live. Plus: auth role-walk (a granted role still authorizes),
  a per-branch DISABLE test (a roleword off on branch X, on at "0"), and a bedrock test (a non-I_AM
  disable of an I_AM genesis word on "0" is refused, but shadowing on a branch is allowed).

## Risks

- THE BRIDGE/STAMPER HOT PATH: `resolveRoleWord` runs on every BE op and every wired do-op. A wrong
  null breaks birth/connect/auth. WP-7's bridge verifiers are the gate; run them after EVERY WP.
- SYNC timing: resolveRoleWord must stay synchronous (no await) — the overlay read is in-memory only.
...
- registerRoleWord is PRE-GENESIS (buffer); the bindWord flush must wait for the writable chain
  (seedFold), exactly as declareOpsToFold already does.

## Recommended phasing

1. WP-1 + WP-2 (declare path unifies; both folds coexist, roleword facts now wordStore-shaped). Boot.
...
Outcome: ONE fold system (wordStore) for every word — op, type, reducer, concept, roleword — with one
declare path, one projection, one bedrock guard. roleWordRegistry shrinks to the role HOST layer:
the IR cache, the evaluator runner (runRoleWord), the birth reader (bornBeingFrom). The bottom turtle,
and nothing else.
