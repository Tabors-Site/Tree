...
export function showRoleManagerPanel({ state, beingEntry, onClose }) {
  ...
  el.className = "role-manager-modal";
  el.style.cssText = `
  ...
  `;
  closeBtn.onclick = () => hideRoleManagerPanel();
  ...
  renderRoleManagerPanel(body, beingEntry, {
    ...
    branch:     state.descriptor?.address?.history || "0",
...
export function hideRoleManagerPanel() {
  if (!_panelEl) return;
  _panelEl.remove();
  _panelEl = null;
...
export function isRoleManagerPanelOpen() {
  return !!_panelEl;
}
