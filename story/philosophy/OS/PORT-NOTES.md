...
- Writes are already branch-local: reel chains, seq counters, and act-chains are all keyed per (branch, …). The JS append lock is now keyed (branch, kind, id) to match (appendLock.js, 2026-06-11) — the same aggregate on sibling branches appends in parallel, exactly what the model licenses.
