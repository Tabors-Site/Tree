...
The TreeOS Browser cannot be opened anonymously. Every session begins **signed in as a being at a land**. Identity is the root of every action; without it, no Perspective Address can form.
...
1. **Every action has an actor.** TreeOS's accountability chain (contributions, flags, plans, contracts, executions) requires knowing who initiated each event. The browser making requests on a user's behalf carries identity through every call. There is no "anonymous browse" because there are no anonymous actions.
...
The web's anonymous-by-default model breaks all three. The TreeOS Browser inverts: signed-in-by-default, anonymous-never.

## What a being is

A **being** is an actor that can sit on either side of a Perspective Address — i.e. it occupies a Stance (`land/path@embodiment`). The `@<label>` portion of the Stance names the being at that position. The label resolves to one of two kinds:
...
The Position Descriptor's `beings[]` field tells the browser which kind each invocable label resolves to.
...
The browser's first screen — opened before any address is loaded — is the sign-in surface. Options:
...
2. **Register a new being on a known land.** If registration is open, the browser collects username + auth method + LLM config (if bring-your-own-LLM).
...
After sign-in, the browser has a **session identity**: which human being on which land. This is the left side of every PA typed in the address bar until the user explicitly switches.

## The identity panel

Always visible in the browser chrome. Shows:
...
Every HTTP/WS request the browser makes carries identity headers:
...
These map onto TreeOS's existing JWT auth — the browser just makes them explicit and persistent across tabs. Lands validate via existing middleware and either:
...
The browser surfaces `FORBIDDEN` either as an inline "switch to a different being you have" hint OR a re-auth prompt.
...
1. **The user has a being on that land too.** Browser switches identity (with consent) and continues.
2. **The user doesn't have one but the land allows guests.** Browser opens with a guest being that lets the user browse the land zone (no write actions).
...
The browser keeps a roster of human beings the user has signed into. Each entry:

```
(land, username, displayName, lastUsed, authMaterial)
```

`authMaterial` is the refresh-token-or-equivalent the browser uses to re-sign-in transparently. Stored encrypted at rest where the OS provides secure storage (macOS Keychain, Windows Credential Manager, Linux Secret Service); falls back to a passphrase-protected store otherwise.

The browser does NOT store passwords in cleartext. Refresh tokens / signed keys only.

## Being creation flow

When the user creates a new being on a land:

1. Browser asks: which land?
2. Land returns its registration policy (`open` / `invite-only` / `closed`).
3. If open: browser collects username + password. The land's discovery declares whether LLM is land-provided, user-required, or optional-augment.
4. Browser sends create-user request to the land. Land validates uniqueness, returns auth material.
5. If the land requires (or allows) bring-your-own-LLM, browser walks the user through configuring (proxy URL, API key, model preferences). Otherwise the land's LLM is used by default.
6. Browser stores the being in the roster. Auto-signs-in.
...
When a tab has no being attached (which shouldn't happen because the browser refuses to open without one), the address bar refuses input and shows the sign-in surface inline.
...
## Failure modes the browser handles
...
- **Being revoked.** Land rejects auth with REVOKED. Browser removes the being from active state and prompts re-sign-in.
- **Token expired.** Browser refreshes via stored refresh token. Transparent to the user.
- **No LLM available in the resolution chain.** Being can browse positions but can't invoke AI beings — happens only if neither user, tree, nor land has an LLM configured. Browser shows an inline hint at chat-panel time pointing the user to either configure a personal LLM or use a land that provides one.
- **Multiple beings on same land.** Allowed. Roster lists both; user picks per tab.

## Relationship to existing TreeOS auth

The current Land server uses JWT bearer auth + WebSocket auth tokens against the User model. The TreeOS Browser carries these through the identity layer unchanged:
...
- The browser doesn't bypass or extend any of the existing auth — it makes the identity layer explicit, multi-being-aware, and visible in the UI.

No backwards-incompatible changes to the Land's auth model. The browser is a more explicit client over the same surface.
