...
A browser for the inhabitable internet. Not documents at URLs — beings addressing beings across lands, via perspective addresses.
...
The TreeOS Browser assumes **beings at positions, addressing other beings at other positions**. Every address names where interaction occurs AND which being is acting there. The browser renders based on what's at the position; the embodiment shapes how the user (or AI) at the address acts on it.

Two architectural commitments make this different from the web:

1. **Identity-first.** You can't open the browser anonymously. Every session starts signed in as a being at some land. Every action has an actor. The accountability chain TreeOS depends on starts in the browser's root surface.

2. **Stance-aware addressing.** Web URLs locate resources. Perspective Addresses (PAs) locate STANCES (positions interpreted through embodiments). The same node viewed @ruler vs @historian renders differently — same data, different lens, different tools.
...
| **Perspective Address (PA)** | stance -> stance (full bridge) | `tabor -> treeos.ai/flappybird@ruler` |
...
See [docs/perspective-address.md](docs/perspective-address.md) for the full grammar.

## What the browser renders

Lands no longer return HTML pages for each URL. They return **Position Descriptors** — structured JSON describing what's at a position. The browser knows how to render TreeOS-shaped data:
...
Rendering is consistent across lands because the browser owns the visual language — every land's positions look like TreeOS positions because the browser draws them that way.
...
- **Node zone** (`server/<zone>/<path>`) — specific position inside a tree. Governance, artifacts, children, beings invocable here.

The zone determines the chrome the browser renders; the position determines the content.

See [docs/zones.md](docs/zones.md).

## The browser's main surfaces
...
- **Tabs** — many Perspective Addresses open at once. No refresh button (everything live via WS).

See [docs/surfaces.md](docs/surfaces.md).

## Coexistence with the existing web

The TreeOS Browser doesn't replace the web. It wraps it.
...
Lands migrate surfaces from HTML to JSON over time. Browser supports both during transition.

For domains outside TreeOS (regular HTTP sites), the browser can act as a wrapper that presents the domain's *being-side* — every domain can publish an AI-being layer the TreeOS Browser knows how to invoke. Instead of MCP servers stitched onto a website, a full being can know and act on the platform. The legacy HTML is still reachable; the new being-layer is the preferred surface.
...
This browser sits opposite the Land server. It speaks a new protocol — the Position Descriptor format — and renders accordingly. The Land server gets new route handlers that recognize PA-shaped requests and return Position Descriptors instead of HTML. Old routes keep working; new routes coexist.

Future: the same browser opens any TreeOS-speaking land. Federation (Canopy) means a browser session can navigate across lands. A bridge can connect a stance on one land to a stance on another.

## What gets built first

Three pieces, in order:

1. **Position Descriptor format** — locked-in JSON contract between server and browser. The shape land servers return per zone type.
2. **Server-side handlers** — Land server recognizes PA-shaped routes and returns Position Descriptors.
3. **The browser app itself** — Electron or Tauri shell. Address bar, identity panel, JSON-position rendering, basic chat panel. Start with home-zone rendering since that's where most user time lives.
...
├── README.md                  this file
├── docs/
│   ├── perspective-address.md PA grammar + parser semantics
│   ├── position-descriptor.md JSON shape lands return per zone
│   ├── zones.md               land / home / node zone rendering rules
...
└── lib/
    └── perspective-address.js parser + formatter for the PA grammar
```
