...
import { currentOrd } from "../../past/fileStore.js";
...
export async function enqueueIntake(spaceId, beingId, entry) {
  ...
  // ARRIVAL ORDINAL (clock-free FIFO key). This self-summon is stamped OUTSIDE
  // a moment (enqueueIntake kicks one off), so commitMoment gets no act and
  // stamps no ord on the call fact . the fold would then have no order key. So
  // we read the current append head ord (the position this enqueue arrives just
  // after) and thread it through params; the inbox fold prefers fact.ord but
  // falls back to this params.ord for exactly this moment-less path. currentOrd
  // peeks, it never consumes a position, so it cannot collide with the next
  // real commit. This is the documented "no triggering fact . use the append
  // head ordinal" choice (the keystroke is not an act of its own).
  const arrivalOrd = currentOrd();
  // INERT display witness only. Kept so a UI can show "received at"; NEVER
  // sorted/compared/folded (arrivalOrd above is the order). entry.sentAt may be
  // a caller-supplied display string; null otherwise (no fresh wall-clock fold).
  const sentAt = entry.sentAt || null;
  ...
  if (existing) {
    return {
      ...
      ord:         existing.ord ?? arrivalOrd,
      // Inert display witness on the dedup path; never used for ordering.
  ...
  await emitFact({
    ...
    params:  {
      ...
      // ORDER KEY threaded for the fold. The self-summon fact is moment-less
      // (no act -> commitMoment stamps no fact.ord), so the inbox fold reads
      // this params.ord as the arrival ordinal. Clock-free FIFO position.
      ord:             arrivalOrd,
      // Inert display witness only (never sorted/compared/folded).
      sentAt,
  ...
  return { correlation, ord: arrivalOrd, sentAt };
}
...
export async function pickNextIntake(spaceId, beingId, opts = {}) {
  ...
  // pick before HUMAN). Within a rank, the LOWEST append ordinal wins
  // (clock-free FIFO): lowest ord = arrived earliest = longest-waiting =
  // picked first, the exact semantics sentAt gave, now keyed on the
  // clock-free order the fold stamped (fact.ord / arrival ord), never on
  // a wall-clock. TIME is never a dependency (order by sequence, not a
  // timestamp).
  ...
  return {
    entry: {
      ...
      // Clock-free FIFO position (the key the scheduler sorted on).
      ord:             row.ord ?? null,
      // Inert display witness only; never sorted/compared/folded.
