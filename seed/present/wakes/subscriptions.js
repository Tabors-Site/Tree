...
async function _emitOne({
  ...
  await callByResolved({
    ...
    message: {
      ...
      priority,
      // No envelope wall-clock. callByResolved stamps the subscription's
      // summon downstream and the inbox fold orders on that fact's append
      // ordinal (ord), not on a timestamp. The fact's own `date` is the lone
      // inert display witness; a fresh new Date() here would be a second
      // wall-clock the doctrine bars.
    },
  });
}
