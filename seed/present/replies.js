...
export function buildResponseEntry({ result, handoff, originalEntry }) {
  ...
  return {
    ...
    // No envelope wall-clock. The reply summon is stamped downstream (call.js /
    // enqueueIntake) and its FIFO order is the fact's append ordinal (ord),
    // which the inbox fold reads. The fact's own `date` is the lone inert
    // display witness; a fresh new Date() here would be a second wall-clock the
    // doctrine forbids. (ord is never carried on an envelope; the stamp lays it.)
  ...
  try {
    ...
    await callByResolved({
      ...
      message: {
        ...
        // No envelope wall-clock. callByResolved stamps the summon downstream;
        // its FIFO order is the fact's append ordinal (ord) which the inbox
        // fold reads, and the fact's own `date` is the lone display witness. A
        // fresh new Date() here would be a second wall-clock the doctrine bars.
      },
      identity: fromBeing
