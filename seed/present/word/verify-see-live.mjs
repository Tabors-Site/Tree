...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_see_live-" + process.pid);
...
try {
  ...
  console.log(
    `  alice=${String(alice).slice(0, 10)} child=${String(child).slice(0, 10)}\n`,
  );
  ...
  Array.isArray(found) &&
  found.length === 1 &&
  String(found[0]._id) === String(alice)
    ? ok(`QUERY: see beings where name="alice" → [@alice]`)
    : bad(`query`, found);
  (q.deltaF || []).length === 0
    ? ok(`SEE lays NO fact`)
    : bad(`no fact`, q.deltaF);
  ...
  q1.bindings.b && q1.bindings.b.name === "bob"
    ? ok(`QUERY one: see the being named "bob" → @bob`)
    : bad(`query one`, q1.bindings.b);
  ...
  "tn" in rd.bindings && rd.bindings.tn != null
    ? ok(
        `READ: see @alice's trueName (fresh) → ${String(rd.bindings.tn).slice(0, 10)}…`,
      )
    : bad(`read`, rd.bindings.tn);
  ...
  const p1 = await see(
    {
      kind: "see",
      of: { ref: "child" },
      descendsFrom: { ref: "ancestor" },
      bind: "isDesc",
    },
    { child: { _id: child }, ancestor: { beingId: alice } },
  );
  p1.bindings.isDesc === true
    ? ok(`PREDICATE: @alicechild descends from @alice → true`)
    : bad(`descends true`, p1.bindings.isDesc);

  const p2 = await see(
    {
      kind: "see",
      of: { ref: "alice" },
      descendsFrom: { ref: "child" },
      bind: "isDesc",
    },
    { alice: { _id: alice }, child: { beingId: child } },
  );
  p2.bindings.isDesc === false
    ? ok(`PREDICATE: @alice does NOT descend from @alicechild → false`)
    : bad(`descends false`, p2.bindings.isDesc);
  ...
  sq.bindings.sp &&
  sq.bindings.sp.kind === "space" &&
  String(sq.bindings.sp._id) === arena
    ? ok(`QUERY of a SPACE: see the space named "arena" → tagged kind:"space"`)
    : bad(`space query`, sq.bindings.sp);
  // READ a space's quality fresh → uses kind="space" → loadProjection("space"), not "being"
  const sr = await see(
    { kind: "see", of: { ref: "sp" }, read: "name", fresh: true, bind: "nm" },
    { sp: sq.bindings.sp },
  );
  sr.bindings.nm === "arena"
    ? ok(
        `READ a SPACE's quality (fresh, kind="space"): see the space's name → "arena"`,
      )
    : bad(`space read`, sr.bindings.nm);
  ...
  const sr2 = await see(
    ...
    { subj: { kind: "space", id: arena } },
  );
  sr2.bindings.nm2 === "arena"
    ? ok(
        `READ via a {kind,id} bind (move's subject shape) → resolves the space`,
      )
    : bad(`kind-id read`, sr2.bindings.nm2);
  ...
  sr3.bindings.tn != null
    ? ok(
        `READ a BARE id string (key-export's target shape): see the <id>'s trueName → ${String(sr3.bindings.tn).slice(0, 8)}…`,
      )
    : bad(`bare-id read (key-export)`, sr3.bindings.tn);
  ...
  ap1.bindings.auth === true
    ? ok(`AUTHORITY: @alice has credential authority over @alice (self) → true`)
    : bad(`auth self`, ap1.bindings.auth);
  const ap2 = await see(
    {
      kind: "see",
      of: { ref: "a" },
      hasAuthorityOver: { ref: "b" },
      credential: true,
      bind: "auth",
    },
    { a: { beingId: alice }, b: { beingId: bob } },
  );
  typeof ap2.bindings.auth === "boolean"
    ? ok(
        `AUTHORITY: @alice over @bob resolves via the credential walk → ${ap2.bindings.auth}`,
      )
    : bad(`auth resolves`, ap2.bindings.auth);
  const ap3 = await see(
    {
      kind: "see",
      of: { ref: "I" },
  ...
  const ap4 = await see(
    ...
    { n: { nameId: "z6MkUnrelatedNameXyz" }, a: { beingId: alice } },
  );
  ap4.bindings.auth === false
    ? ok(
        `AUTHORITY (general, name->being): an unrelated name has NO authority over @alice → false`,
      )
    : bad(`auth general false`, ap4.bindings.auth);
