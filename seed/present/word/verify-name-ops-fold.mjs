...
// twin of verify-dispatch-fold. (I's own bootstrap name:declare is a raw emitFact in sprout.js,
...
const DB = path.join(os.tmpdir(), "story_nameopsfold-" + process.pid);
process.env.PORT = "3832";
process.env.TREEOS_STORE_BASE = DB;
fs.rmSync(DB, { recursive: true, force: true });
process.env.JWT_SECRET = process.env.JWT_SECRET || "nameopsfold-0123456789";
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "nameopsfold-keys-" + process.pid,
);
fs.rmSync(process.env.STORY_KEY_DIR, { recursive: true, force: true });
// (scratch file store fresh-wiped above; no DB to drop)
await import(`${R}/begin.js`);
const { findByName } = await import(`${R}/seed/materials/projections.js`);
const { bindWord, registerHostHandler, resolveNameOpFromFold, getWordSync } =
...
try {
  ...
  const folded = expected.filter((op) => {
    const w = getWordSync(`name:${op}`);
    return w && w.kind === "nameop" && w.do?.ref;
  });
  folded.length === expected.length
    ? ok(
        `all 5 NAME ops folded as kind:"nameop" words (name:${expected.join(", name:")})`,
      )
  ...
  const resolved = expected.filter(
    (op) => typeof resolveNameOpFromFold(op)?.handler === "function",
  );
  resolved.length === expected.length
    ? ok(
        `resolveNameOpFromFold returns a handler for all 5 (the dispatch's read)`,
      )
  ...
  const agree =
    ...
    buffer.every(
      (op) =>
        typeof resolveNameOpFromFold(op)?.handler === "function" &&
        typeof getNameOp(op)?.handler === "function",
    );
  agree
    ? ok(
        `NAME_OPS object (${buffer.length}) agrees with the fold (the demoted registration buffer)`,
      )
  ...
  registerHostHandler("name-op:test-name-op", async () => {
    ranFromFold = true;
    return { nameId: String(I) };
  });
  await bindWord("name:test-name-op", {
    ownerExtension: "seed",
    kind: "nameop",
    do: { ref: "name-op:test-name-op" },
  });
  ...
  const f = await pollFor(
    () => factFindOne({ verb: "name", act: "test-name-op" }),
    (v) => !!v,
  );
  f
    ? ok(
        `the fold-driven NAME op laid its name:test-name-op fact (same audit path)`,
      )
    : bad(`fact laid`, "no test-name-op fact");
