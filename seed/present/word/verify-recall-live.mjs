...
// record, on a FRESH being (idle chain) so the verdict doesn't fork I's busy genesis chain.
...
const SCRATCH_DB = path.join(os.tmpdir(), "story_recall-" + process.pid);
...
try {
  ...
  // a FRESH being — its chain is idle, so its verdict won't fork I's busy genesis chain
  let sageId = null,
    sageName = null;
  ...
  const ident = {
    beingId: String(sageId),
    nameId: String(sageId),
    name: sageName || "sage",
  };
  const reader = () => ({
    dryRun: false,
  ...
  const c1 = reader();
  await evaluate({ kind: "recall", of: String(sageId), as: "mine" }, c1);
  Array.isArray(c1.bindings.mine) &&
  c1.bindings.mine.length > 0 &&
  c1.bindings.mine[0]?.line &&
  c1.deltaF.length === 0
    ? ok(
        `recall my own thread → recalled (${c1.bindings.mine.length} acts, woven in the Word), NO fact laid`,
      )
    : bad(`recall own`, {
        len: c1.bindings.mine?.length,
        line: c1.bindings.mine?.[0]?.line,
        laid: c1.deltaF.length,
      });

  // 2. recall the WORLD → saw; the whole branch, woven; NO fact
  const c2 = reader();
  await evaluate({ kind: "recall", of: "world", as: "w" }, c2);
  Array.isArray(c2.bindings.w) &&
  c2.bindings.w.length > 0 &&
  c2.deltaF.length === 0
    ? ok(`recall the world → saw (${c2.bindings.w.length} acts), NO fact laid`)
    : bad(`recall world`, {
        len: c2.bindings.w?.length,
        laid: c2.deltaF.length,
      });

  // 3. recall a FOREIGN thread (I's) → saw (all public, no gate); NO fact
  const c3 = reader();
  await evaluate({ kind: "recall", of: String(I), as: "f" }, c3);
  Array.isArray(c3.bindings.f) &&
  c3.bindings.f.length > 0 &&
  c3.deltaF.length === 0
    ? ok(
        `recall a foreign thread → saw (${c3.bindings.f.length} acts, all public), NO fact laid`,
      )
    : bad(`foreign saw`, {
        len: c3.bindings.f?.length,
        laid: c3.deltaF.length,
      });

  // 4. recall my LINEAGE → recalled; the family story (sage + its descendants); NO fact
  const c4 = reader();
  await evaluate(
    { kind: "recall", of: { lineage: String(sageId) }, as: "fam" },
    c4,
  );
  Array.isArray(c4.bindings.fam) &&
  c4.bindings.fam.length > 0 &&
  c4.deltaF.length === 0
    ? ok(
        `recall my lineage → recalled (${c4.bindings.fam.length} acts, the family story), NO fact laid`,
      )
    : bad(`recall lineage`, {
        len: c4.bindings.fam?.length,
        laid: c4.deltaF.length,
      });

  // 5. recall a MOMENT → saw; one act's cross-section (a WHEN view); NO fact
  const someAct = (c2.bindings.w || []).find((a) => a.actId)?.actId;
  const c5 = reader();
  await evaluate({ kind: "recall", of: { moment: someAct }, as: "mom" }, c5);
  Array.isArray(c5.bindings.mom) &&
  c5.bindings.mom.length > 0 &&
  c5.deltaF.length === 0
    ? ok(
        `recall a moment → saw (${c5.bindings.mom.length} act, the cross-section), NO fact laid`,
      )
    : bad(`recall moment`, {
        len: c5.bindings.mom?.length,
        act: someAct,
        laid: c5.deltaF.length,
      });
  ...
  verdict &&
  verdict.params?.mode === "saw" &&
  verdict.params?.that === "it was good" &&
  verdict.params?.because === "it served its purpose"
    ? ok(
        `the verdict wrote ONE memory fact (mode:saw, that + the declared why) — conclusion AND reason recorded`,
      )
    : bad(`verdict`, verdict);
  ...
  const line = world.find((a) =>
    /saw the world that it was good \(because it served its purpose\)/.test(
      a.line,
    ),
  );
  line
    ? ok(`the book reads the verdict-memory in the Word: "${line.line}"`)
    : bad(`verdict render`, world.map((a) => a.line).slice(-3));
