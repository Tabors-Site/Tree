...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_updatellm_cut-" + process.pid);
process.env.PORT = "3797";
process.env.TREEOS_STORE_BASE = SCRATCH_DB;
fs.rmSync(SCRATCH_DB, { recursive: true, force: true });
process.env.JWT_SECRET =
  process.env.JWT_SECRET || "updatellm-secret-0123456789";
process.env.STORY_KEY_DIR = path.join(
  os.tmpdir(),
  "updatellmcut-keys-" + process.pid,
);
...
process.env.CUSTOM_LLM_API_SECRET_KEY =
  process.env.CUSTOM_LLM_API_SECRET_KEY ||
  "updatellm-llm-encryption-key-0123456789";
...
const readConn = async (connId) => {
  const slot = await loadOrFold("being", String(I), "0");
...
try {
  ...
  resolveAbleWord("being", "update-llm-connection")
    ? ok(`update-llm-connection.word resolves through the bridge`)
    : bad(`resolves`, "null");

  // Seed: add a connection (exercises E6 resolveConnectionSpec on the add path too).
  const add = await did(
    "add-llm-connection",
    { kind: "being", id: String(I) },
    {
      name: "conn-a",
  ...
  connId
    ...
    : bad(`add`, add.refused?.message || add.result);
  ...
  up.result && sf.length === 1
    ? ok(
        `update → exactly ONE do:set-being fact (the spacebar lift; no skipAudit double-stamp)`,
      )
    : bad(
        `one-fact`,
        up.refused?.message || { facts: sf.length, result: up.result },
      );

  // ── 2. the fact carries the merged connection at the right path, attributed to the caller ──
  const f = sf[0];
  f &&
  f.params?.field === `qualities.llmConnections.${connId}` &&
  f.params?.value?.model === "m2" &&
  String(f.through) === String(I)
    ? ok(
        `do:set-being: field = llmConnections.<id>, value.model = "m2" (merged), through = caller`,
      )
    : bad(
        `fact`,
        f
  ...
  c.model === "m2" &&
  c.name === "conn-a2" &&
  c.baseUrl === "https://example.com/v2"
  ...
  c && typeof c.encryptedApiKey === "string" && c.encryptedApiKey.includes(":")
    ? ok(
        `encryptedApiKey preserved as ciphertext in the merged value (redact strips it on the wire)`,
      )
  ...
  const n = await did(
    "update-llm-connection",
    { kind: "being", id: String(I) },
    { connectionId: connId, baseUrl: "https://example.com/v2", model: "m2" },
    {},
  );
  n.refused
