...
const SCRATCH_DB = path.join(os.tmpdir(), "story_word_setmodel_cut-" + process.pid);
...
const { ensureSkinsSpace } = await import(
  `${R}/seed/store/words/model/index.js`
);
...
try {
  ...
  resolveAbleWord("render", "set-model", "0")
    ? ok(`model.word resolves through the bridge (self-registered)`)
    : bad(`resolves`, "null");

  // ── the skins catalog space (ensureSkinsSpace ran at genesis; resolve/ensure it) ──
  const skins = await ensureSkinsSpace("0", null);
  skins
    ? ok(`skins space resolves: ${String(skins).slice(0, 8)}…`)
    : bad(`skins`, "no skins space");

  // ── upload a model matter: putContent .glb-ish bytes → cas ref, then create-matter ──
  const bytes = Buffer.from(
    "glTF\x02\x00\x00\x00setmodel-cut-test-bytes",
    "binary",
  );
  ...
  const cm = await doOp({ kind: "space", id: String(skins) }, "create-matter", {
    type: "model",
    content: ref,
  });
  ...
  modelMatterId
    ? ok(
        `upload: create-matter {type:model} → matterId ${String(modelMatterId).slice(0, 8)}… in /skins`,
      )
  ...
  s.result?.set === true
    ? ok(
        `set-model {modelMatterId} → set:true (the .word assert-may-set-model + resolve-model-block + write-model ran)`,
      )
  ...
  sf &&
  sf.params?.field === "qualities.render" &&
  sf.params?.value?.model?.matterId === String(modelMatterId) &&
  sf.params?.merge === true
    ? ok(
        `the set-model fact carries field="qualities.render", value.model + merge:true (the .word param-enrichment reached the auto-fact)`,
      )
  ...
  model &&
  String(model.matterId) === String(modelMatterId) &&
  typeof model.hash === "string" &&
  model.url === `/api/v1/content/${model.hash}`
    ? ok(
        `@being qualities.render.model folds {matterId, hash, url:"${model.url.slice(0, 24)}…"} — the body is worn`,
      )
  ...
  c.result?.cleared === true
    ? ok(
        `set-model {clear:true} → cleared:true (the .word clear fork + clear-model ran)`,
      )
    : bad(`clear result`, c.refused?.message || c.result);
  const cf = (c.deltaF || []).find((f) => f.act === "set-model");
  cf &&
  cf.params?.field === "qualities.render.model" &&
  cf.params?.value === null &&
  cf.params?.merge === false
    ? ok(
        `the clear fact carries field="qualities.render.model", value:null, merge:false`,
      )
    : bad(`clear fact`, cf?.params);
  const slot2 = await loadOrFold("being", String(me), "0");
  slot2?.state?.qualities?.render?.model == null
    ? ok(
        `@being qualities.render.model cleared (folded to ${String(slot2?.state?.qualities?.render?.model)})`,
      )
  ...
  const strangerIdent = {
    beingId: String(stranger),
    name: "stranger",
    nameId: "stranger",
  };
  const n = await doOp(
    { kind: "being", id: String(other) },
    "set-model",
    { modelMatterId },
    strangerIdent,
  );
  n.refused && n.refused.code === "FORBIDDEN"
    ? ok(
        `non-owner set-model on another's being → refuse [FORBIDDEN] "${(n.refused.message || "").slice(0, 48)}…"`,
      )
    : bad(
        `owner gate`,
        n.refused
          ? { code: n.refused.code, message: n.refused.message }
          : n.result,
      );
