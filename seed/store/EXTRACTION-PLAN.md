...
in `ibp/operations.js`, the ableWordRegistry bridge) STAYS factory and holds no words.
...
the moved module imported at boot, and verify `registerOperation` + `registerAbleWord` both fired
(op in the Map AND `resolveAbleWord(able,op)` returns non-null). Op names + factAction strings stay
...
per-cut harnesses pass. The ableWordRegistry engine now holds ZERO built-in words. Demo programs
...
- [ ] 5. **grant-able** → `store/words/grant-able/` (grant-able.word + grantHost.js; carve grantAbleHandler/_grantAbleViaWord out of being/ops.js into a new index.js). ADD services.js import beside :194. LOW.
...
- [ ] 8. **set-world-signal** → `store/words/set-world-signal/` (carve from able-manager/ops.js's mixed 3-op file; disjoint helpers parseSignalValue/NS_SEGMENT_RE move too). ADD genesis.js boot import. MEDIUM.

## Stay co-located (do NOT move — flagged)

- **matter/create-matter** — cut is UNWIRED (handler runs pure JS, never resolveAbleWord; matterHostEnv has no consumers); shares private `assertMatterCoordInBounds` with setOnMatterHandler; ops.js co-owns 4 non-word siblings. Wire the cut first (add `_createMatterViaWord`), then revisit.
- **acquisition (ask-able/take-able)** — `emitInternalGrant` is shared by both handlers, acquisitionHost, AND the core SEE verb (see.js:718). A word bundle holding it forces SEE to reach into a word. Refactor emitInternalGrant to a shared module first.
- **cherub (birth/connect)** — birthHandler/connectHandler are 2 of 6 in the frozen `cherubBeOps` object feeding the closed BE_OPS table; the .word is control-strand only (real logic in connectHost + JS fallbacks); and cherub is the ONLY word hardcoded in ableWordRegistry's built-in map (engine-relative paths). Whole-dir move only, last, separate session.

## Shared-reader notes

- **ableWordRegistry built-in map (L32-35):** NO edit for any moved word — they self-register via `registerAbleWord` at module-load; `new URL("./x.word", import.meta.url)` resolves against the moved module's own dir. The only built-in-map entries are cherub (which stays).
...
- **services.js side-effect imports:** repoint one line per bundle (key:204, set-render:235, portal:193, move:182, credential:195); ADD a new import for grant-able beside :194 (being/ops.js stays for revoke-able/set-being/LLM ops).
- **root genesis.js dynamic imports:** history-pointers + set-world-signal register through genesis.js's `await import()` of the able-dir ops.js (history-manager 754-756, able-manager 742-744). After carving, ADD an explicit `await import()` of the new store bundle index.js beside those, or the .word silently goes dark.
- **declareWordsToChain / rehydrate:** no edit — fresh dev DB, params.source path changes are fine (no migration); resolveAbleWord keys on (able,op), path-agnostic.
...
registration), assert `resolveAbleWord(able,op,"0")` is non-null. Full boot-verify + the per-word
