...
fn the_lookups_land() {
    ...
    assert!(treeseed::op_word("makespace", Some("space")).is_some(), "makespace via its noun bundle (the M1C rename)");
    assert!(treeseed::op_word("makespace", None).is_some(), "makespace by coined name (the unique stem)");
    assert!(treeseed::op_word("makematter", Some("matter")).is_some(), "makematter via its noun bundle (the M1C rename)");
