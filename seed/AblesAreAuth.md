...
**Ables are auth. Every able-in-effect lives on a space's `qualities.ables[<name>]`; grants on beings reference the able by name + the space it's anchored at; authorize walks the grant's anchor up the qualities ancestor chain to find the spec, then applies the able's canX after a reach check.**

### Single-gate doctrine (pinned)

**The able-walk is THE gate. Every action is gated by the caller's granted ables. There is no bypass mechanism.**

When an action needs to be universally available — like "every being can request acquisition" — the substrate expresses that as a `canDo` entry on an open able that every being holds (today: `global`, granted to every being at birth). The substrate's gating IS the able system; they are not separate.
...
### Hosting (where ables live)

A able lives on the `qualities.ables` of the space where it was authored. That's the **host**. The able naturally reaches the host + all descendants via the qualities ancestor inheritance — no extra storage needed.
...
By default a able reaches its host + all descendants. The optional `reach` field on the able spec adjusts this. Single ordered list, bash-style `!` prefix for exclusions:
...
### Ownership is foundational; ables are delegated

Two parallel authority mechanisms work together:

- **`members.owner` of space S → implicit authority over S + descendants.** No able grant needed. You own the space, you can do anything in it: author ables, install/grant/revoke, edit qualities, manage members, etc.
- **Granted ables → delegated authority.** Owners grant slices of their authority to others via `grant-able`; grantees can only do what the able's canX permits within the able's reach.

The auth walk has an owner-check step BEFORE the able-walk:

```
authorize:
  1. I-Am bypass                                (bootstrap axiom)
  2. SEE on .discovery                          (pre-identity surface)
  3. Anonymous → arrival floor                  (implicit arrival able)
  4. Owner-check: actor in members.owner of      ← step 4
     target's space or any ancestor → ALLOW
  5. Able-walk (qualities.ablesGranted)         (delegated authority)
  6. Deny
```

When a human is birthed, cherub sets `members.owner = [<newBeingId>]` on their home space — they own their home and everything under it. Outside their home they need granted ables (the `human` able gives broad access by default; operators narrow it as the story matures).

### The `@public` being — implicit commons-able for visitors
...
The space becomes a forever-public commons. **Every visitor walking into that subtree is treated as if they hold an implicit `public-commons` able.** Authorize's able-walk has a Public-commons step AFTER explicit grants:
...
authorize:
  ...
  5. Explicit able-walk over qualities.ablesGranted
  6. Public-commons: target sits in a public-owned subtree → APPLY
     the seed-shipped commons able's canX (see + move + create-space +
     create-matter + call @cherub + release). MATCH → ALLOW.
  7. Deny
```

The commons able is **not granted as a fact** — it's a derived, implicit grant for every visitor at every public-owned space. Visible in the authorize decision (returned `able: "public-commons"`); not stored on the being.
...
Operators wanting a richer commons surface author a real able on the public-owned space's `qualities.ables` (those flow through the able-walk above; the public-commons step is the floor).
...
Per the seed/AblesAreAuth.md flow + the be:birth refactor: there are TWO ways a new being comes into existence.
...
The arrival able's only outward capability is `call @cherub:mate`. Cherub's `canSummon` declares `as: "receiver"` for `intent: "mate"` — that's the contract that says "this able accepts call:mate from anonymous arrivals." Cherub's handler mints the new being with the human able + the visitor's chosen credentials, grants `global` + `human` at the place root, and binds the session.
...
A being with `canBe: ["*"]` or `canBe: ["birth"]` calls `be:birth` directly on their own identity. The target IS the actor — "I birth a being." Available only to humans and angels by default; other ables use Path A.
...
`human` (the root-founder temporary able) and `angel` (super-sudo) carry `["*"]`. Other ables use named ops (the canonical `place` op covers most navigation needs).
...
async function authorize({identity, verb, target, action, intent, operation, branch}):
  ...
  for each grant in identity.ablesGranted:
    // Look up the able spec by walking the grant's anchor up the qualities chain.
    spec = await getAbleSpec(grant.able, grant.anchorSpaceId, branch)
    if !spec: continue                               // able doesn't exist where anchored

    // Did the host find an ancestor that reaches target?
    if !ableReachesTarget(spec, host, target, branch): continue
...
async function getAbleSpec(name, anchorSpaceId, branch):
  // Walk anchorSpaceId up; first ancestor with qualities.ables[name] is the host.
  for ancestor in walkAncestors(anchorSpaceId, branch):
    if ancestor.qualities.ables?.[name]:
      return { spec: ancestor.qualities.ables[name], host: ancestor._id }
  return null

function ableReachesTarget(spec, host, target):
...
A grant is a fact on the being's reel: `do:grant-able` with `{able, anchorSpaceId, grantedBy, grantedAt}`. The being's reducer folds these into `qualities.ablesGranted`. The grant persists across movement; it just becomes inert at positions outside its able's reach.
...
`revoke-able` emits `do:revoke-able`; the reducer drops the matching `(able, anchor, grantor)` tuple.

### flow integrates cleanly

A `flow` clause picks a held able given moment context. Each clause is now filtered by three checks:

1. `when` condition matches (today's behavior)
2. The being has a grant of that able (in `qualities.ablesGranted`)
3. The grant's able spec reaches the being's current position

Failing any of the three → skip the clause. Position changes naturally rotate the active able. No re-authoring needed.

If no clause produces a held + reaching able, the moment fails loud (`no granted able applies at this position`). No silent fallback.

### Authoring a able — `set-able` writes to a space

`set-able(targetSpace, name, {canSee, canDo, canSummon, canBe, reach?, description?})` writes the spec into `targetSpace.qualities.ables[name]`. The targetSpace IS the host.
...
Today's `REGISTRY` map in `seed/present/ables/registry.js` retires from the authorize path. It becomes a **template shelf** — a library of able specs operators or extensions can install onto spaces. Nothing about this registry gates anything by itself; it's just curated content.

`getAble(name)` from the registry stays for cognition-frame use (the LLM prompt builder reads able specs by name). The authorize path uses `getAbleSpec(name, anchorSpaceId, branch)` which walks space qualities.

For extension-shipped able TEMPLATES, the extension manifest declares `provides.ables[]` as today; the loader registers them as templates. Operators decide which spaces to install them at via `set-able` (or a new `install-able`) ops.

### Foundational seed ables — genesis installs them onto spaces

Genesis sequence change:

```js
// Today: ables live in REGISTRY only
...
`installAbleOnSpace(space, name, spec, identity)` emits `do:set-able` on the target space.

The bootstrap grants (I-Am grants angel to seed delegates) stay the same — the grant references the able by name + anchorSpaceId (now heaven for angel-anchored grants).
...
- `scope: "global" | "anchored"` field on able spec → goes away. Every able just has a host.
- Top-level `reach` validation in registry that demands scope=global → goes away. Any able can declare reach.
...
- Implicit arrival floor (anonymous callers run under arrival able's canX).
- Extension scope gate (orthogonal: refuses ext:op at blocked positions).
- `qualities.ablesGranted` on Being (the grant list, fact-folded).
- `qualities.flow` on Being (active-able pick at moment-time, now filtered by grants + reach).
- The able registry/template shelf for cognition-frame use and operator-discovery of installable ables.

### Pinned principles (for FACTORY.md / PERMISSIONS.md)

> **Ables are auth.** A being acts under a able; the able's canX IS the permission gate. There is no parallel `qualities.permissions` namespace.
>
> **Ables live where they're authored.** Every able-in-effect is hosted on a space's `qualities.ables`. Inheritance flows down via the normal qualities ancestor walk.
>
> **Reach adjusts default coverage.** Default = host + descendants. The optional `reach` field on the able can ADD lateral extensions or EXCLUDE specific subtrees. One field, two directions.
>
> **Grants travel with the being.** A grant fact lives on the being's reel. Movement never strips a grant — just makes it contextually inert when the able's reach doesn't cover the current position.
>
> **The space tree IS the authority tree.** Author a able at the space whose subtree you want it to govern. Higher hosts override lower (a parent's able reaches all children); children can stack additions or local-shadow the same name, but can't erase ancestor authority.
>
...
1. **Stance auth** — `seed/ibp/authorize.js` walks `qualities.permissions.<verb>.<keyParts>` on the target's ancestor chain. The `requires:` block matches against stance properties (`arrival`, `owner`, `contributor`, `memberClasses`, etc.) derived from the caller's relationship to the position. The keyParts INCLUDE able-name strings (e.g. `call:@cherub:birth`) but those strings are just names — authorize never looks them up in the able registry.

2. **The able registry** — `seed/present/ables/registry.js`. Each able declares `canSee` / `canDo` / `canSummon` / `canBe`. These are used at moment-time to build the LLM frame (the prompt's capability list) and to dispatch the call handler. They are NOT consulted by authorize.

This is the load-bearing smell. A able's `canDo: ["set-config"]` is documentation for the LLM — it tells the model "you have this affordance" — but it doesn't actually grant the underlying permission. Authorization gates on whether the caller is a `contributor` of the target space, not on whether their able has `set-config` in its canDo list. Add a new able with canDo containing "set-config" and nothing changes downstream.
...
```
I-Am  →  Angel able (cansee:*, cando:*, cansummon:*, canbe:*)
  │
  │  grants angel able to:
  ↓
[cherub, birther, llm-assigner, able-manager, ...]  (seed delegates at heaven)
  │
  │  cherub anoints new humans into angel able at heaven (their parents in being-tree)
  ↓
[human operators]
  │
  │  humans author ables, grant them to children they birth, or to peers who travel
  ↓
[domain-specific ables: coder, factory-worker, dancer-llm, ...]
```

The I-Am's authority is bootstrap-axiomatic: it carries the implicit "I am that I am" angel able over the place root + everything below. Every other being's authority derives from a recorded grant chain rooted at the I-Am.

### Ables ARE permissions

A able's `canSee / canDo / canSummon / canBe` is no longer "documentation for the LLM frame" — it is the actual gate. The able registry becomes the single source of truth for what a being can do. Authorize replaces the layered stance-properties check with a single able-walk check.
...
1. **Find the actor's granted ables whose anchor is an ancestor of the target space** (or the actor's able is global-scope).
2. **For each candidate able, check if any `canDo` entry matches `set-matter`.**
3. **If at least one able grants the action, allow.** Otherwise deny.

No more `requires: { contributor: true }` rules in `qualities.permissions`. The able's `canDo: ["set-matter"]` IS the rule.

### ablesGranted on Being

```js
Being.qualities.ablesGranted = [
  {
    able:          "coder",
    ...
    // harmony beats), enforced at the able-walk like everything else.
  },
  { able: "human", anchorSpaceId: "<placeRootId>", grantedBy: "i-am", grantedAt: "..." },
  ...
]
```
...
Entries are unique by `(able, anchorSpaceId, grantedBy)`. If Alice and Bob both grant you `coder` at `/coders/`, you get TWO entries — each separately revocable. The being holds the able as long as at least one grant survives.

- `revoke-able({able, anchorSpaceId, grantedBy})` removes the matching tuple. Caller must be that `grantedBy` OR hold a strictly-broader able at the same anchor.
...
#### Active-able resolution at moment-time

Today's `resolveActiveStack()` walks `qualities.flow` and picks any able from the registry. After this change:

- Each clause's `when` is evaluated as today.
- If the `when` passes, the resolver checks: **does the being hold `clause.able` granted at the current position (or globally)?** Look up `ablesGranted` and walk the position's ancestors against each grant's `anchorSpaceId`.
- If granted: the clause wins.
- If not granted: SKIP — same as if the `when` failed. The next clause is tried.
- If the terminal fallback clause (the one with no `when`) also references an ungranted able: the resolver returns `null` and the moment fails loudly with `no granted able applies at this position`. No silent fallback.

The `defaultAble` field stays for now (drives the terminal clause for legacy realities) but no longer auto-grants anything. In the migration step, every being's `defaultAble` becomes an explicit grant at `homeSpace` so the existing flows continue working.
...
The scope is set when the able is authored (the `set-able` DO op gains a `scope` arg). Existing ables default to "anchored" with the place root as anchor — same as today's stance auth where heaven gates on `memberClasses: angel`.

### Anchor kind: space OR being

A grant anchors to one of two things — a **space** or a **being**. Ables can scope to positions (you're a coder at the coders space) OR to relationships (you're friends-with @alice). The auth walker checks both.

```js
Being.qualities.ablesGranted = [
  { able: "coder",  anchorSpaceId: "<spaceId>",  anchorBeingId: null, grantedBy: ..., grantedAt: ... },
  { able: "friend", anchorSpaceId: null,         anchorBeingId: "alice", grantedBy: "alice", grantedAt: ... },
]
```
...
### The `global` able — baseline for every being

A foundational able granted to **every authenticated being at birth** by their parent (cherub for new humans; the operator-being for sub-children). Carries the actions every being in this story should have by default. Arrival is excluded — anonymous callers stay on their implicit read-only floor.

```js
{
  name: "global",
  scope: "anchored",
  description: "Baseline able every authenticated being carries. Customize per story.",
  canDo: [
...
Anchored at the place root with story-wide reach via descendants. **Customizable per story** — operators edit the global able's canDo to define their baseline. Default seed-shipped global is conservative (move + see-self + release); operators expand it as they decide what "everyone here can do."

Term-collision note: the able NAME is `global` (carries the "every being gets this" meaning) but its SCOPE field is `"anchored"` (the grant goes at place root). The `scope: "global"` distinction is about how a grant reaches (no anchor required, story-wide intrinsic); the `global` able uses normal anchored grants.
...
The reach of a able comes from ONE of two mechanisms, depending on scope:

- **Anchored able** — the grant's `anchorSpaceId` (or `anchorBeingId`) IS the reach. A `coder` able granted at `/coders/` reaches `/coders/` and its descendants. The canX entries don't carry patterns — the anchor is the single source of truth for "where this able works." Asking each canX entry to also declare a pattern would create the "what if canDo's pattern disagrees with canSee's pattern" problem.

- **Global able** — by default reaches everywhere (no anchor). Optionally constrained by a top-level `reach` field on the able spec that lists where the able can act. Absent → true-global (e.g. the `angel` able). Present → reach restricted to those paths/IDs.

The canX entries stay simple: action names, op names, being-name patterns, operation names. They describe WHAT the able permits; the anchor or `reach` describes WHERE.

#### Able spec — final shape
...
Small. Just enough for "this global able works at these specific places":
...
No regex. No wildcards in names. No per-canX patterns. First match in the `reach` list means the able can act at the target.
...
authorize({verb, target, action, intent, operation, identity}):
  ...
  for each grant in identity.ablesGranted:
    able = getAble(grant.able)
    if not able: skip

    // Reach check — does this able reach the target?
    if able.scope === "anchored":
      // The grant's anchor is the reach
      if not targetReachableFromAnchor(target, grant): continue
    else if able.scope === "global":
      if Array.isArray(able.reach) && able.reach.length > 0:
        if not matchesAny(able.reach, target): continue
      // else: true-global, applies anywhere

    // canX check (action-only; no patterns inside canX)
    if verb === "see"    && permitsSee(able, target):                  return ok
    if verb === "do"     && permitsDo(able, action):                   return ok
    if verb === "call"   && permitsSummon(able, target.being, intent): return ok
    if verb === "be"     && permitsBe(able, operation):                return ok
...
For anchored ables, the grant's `anchorSpaceId` (or `anchorBeingId`) IS the reach. For global ables, the able's optional `reach` field is the constraint and the grant's anchor (typically the place root) is auxiliary.

In both cases the anchor also serves:

- **Identity for revoke** — `revoke-able` targets a specific `(able, anchor, grantor)` tuple. The anchor distinguishes one grant from another when the same able is granted to the same being from multiple grantors at different places.
...
cherub.canDo = [
  { action: "grant-able:human",   description: "register a new human at the place root" },
  { action: "grant-able:arrival", description: "..." },
  ...
]

// Angel (I-Am's able) can grant any able.
angel.canDo = [
  { action: "grant-able:*", description: "I-Am grants anything" },
  ...
]

// Coder can peer-grant coder.
coder.canDo = [
  { action: "grant-able:coder", description: "promote a peer to coder" },
]
```
...
This closes the hierarchy: the I-Am can grant anything (angel canDo: `grant-able:*`); seed delegates can grant within their domain; humans can grant the ables they authored or that some other able permits them to hand out. The chain back to I-Am is structural.

The same pattern applies to `revoke-able:<ableName>` — revocation is governed by the symmetric canDo entry. Cherub can revoke human (canDo: `revoke-able:human`); angels can revoke anything (canDo: `revoke-able:*`).
...
- **(a) travel + join** — the being moves to the coder space; an existing coder runs `grant-able(target=<this being>, able="coder", at=<here>)`. Now the being has `coder` granted at that space.
- **(b) travel + ask** — same shape; the request goes through a CALL to a coder; the coder decides whether to grant.
- **(c) author own** — the being runs `set-able(name="my-coder", canDo=[...], scope="anchored")` at a space they own; they're automatically granted that able at that space.

No special machinery needed beyond grant-able + the existing set-able op.
...
async function authorize({
  ...
  for (const grant of reachable) {
    const able = getAble(grant.able);
    if (!able) continue;
    if (permits(able, verb, action, intent, operation)) {
...
The `matchAny` helper does the wildcard + prefix matching from today's `scoreKey`. The four-layer authorize collapses to one walk over the actor's granted ables.

## Schema changes

### Being

- **Add:** `qualities.ablesGranted: Array<{able, anchorSpaceId, grantedBy, grantedAt}>`
- **Retire:** `defaultAble` field stays for now (still drives flow's fallback clause) but doesn't gate auth. Future cleanup: replace with first granted able.
- **Retire:** `qualities.flow` keeps its current shape; the active-able pick at moment-time still walks the flow. The flow's chosen able must appear in `ablesGranted` or it falls back.
...
The `angel` memberClass retires. Replaced by `angel` ABLE granted at heaven. Seed delegates and human operators register the angel able at heaven via `grant-able`. Same result, one mechanism.

### qualities.permissions retires

The `qualities.permissions.<verb>.<keyParts> = { requires: ... }` namespace retires. Existing rules get translated to able definitions during the migration. After the migration, this namespace is empty and the read path is removed from authorize.
...
```js
do(targetBeing, "grant-able", { able: "coder", anchorSpaceId: "<id>" })
```

- Caller must hold a able at `anchorSpaceId` (or above) that includes `canDo: [{action: "grant-able", description: "..."}]`.
- Target being's `qualities.ablesGranted` gains the entry (via `set-being` fact under the hood — the audit fact is the grant record).
- If the grantor doesn't hold the able they're granting, the op refuses. (Exception: if the able's `canBe: ["grant"]` permits this grantor to lend it out — but that's a finer point; default refuses.)

### `revoke-able`

```js
do(targetBeing, "revoke-able", { able: "coder", anchorSpaceId: "<id>" })
```

- Caller must be the original grantor OR hold a strictly-broader able at the same anchor.
- Appends a `revoke-able` fact; the fold drops the matching grant from `ablesGranted`.

### `set-able` (existing — gains `scope`)

```js
do(target, "set-able", { name, canSee, canDo, canSummon, canBe, prompt, scope: "anchored" })
```

The op runs an **explicit canDo check** before authoring — no back door:

1. Resolve the target space (where the able is being authored).
2. Walk the caller's `ablesGranted` to find ables reachable at that target.
3. **Required:** at least one of those ables must have `canDo: [{action: "set-able", ...}]`.
...
Without this gate a coder could author `super-admin` with canDo: `["*"]` and self-grant; the gate enforces that able authorship needs an existing able to back it.
...
- `Being.defaultAble = "<name>"` + `qualities.flow = [...]`
- `Space.qualities.permissions = {...}` rules (stance-based)
- `Space.members.angel = [<beingIds>]` (heaven angel class)

The migration pass:

1. **Seed-being grants.** Genesis already births I-Am + seed delegates. Append a grant moment: I-Am grants the `angel` able at heaven to every seed delegate (cherub, birther, llm-assigner, etc.) at the moment of birth. Same chain shape, slightly different facts.

2. **Heaven angel class → angel able grants.** A one-shot script reads `Space.members.angel` on heaven, emits a `grant-able(angel, anchor=heaven)` per member, then unsets the angel class. After this lands, the `memberClasses` derivation path can stop reading the angel class.

3. **defaultAble → ablesGranted seed.** For each being with `defaultAble`, emit a `grant-able(<defaultAble>, anchor=<homeSpace>, grantedBy=I-Am)` so their existing default able appears in their granted list. Without this they wake up with zero granted ables.

4. **qualities.permissions translation.** For each `qualities.permissions.<verb>.<keyParts> = { requires: { memberClasses: { includes: "X" } } }` rule, ensure a corresponding able exists with `canX: [<keyParts>]` and that members of class X are granted that able at the space. Most existing rules are seed defaults; a handful per story. The migration script enumerates and converts.
...
The able section (above the existing DO actions form) gains:

- **Granted ables list** — render `ablesGranted[]` with anchor space + grantor + when. Self-only edit affordances appear when `session.beingId === b.beingId`.
- **Grant a able** — when the session being holds `canDo: ["grant-able"]` reachable from the current position, show a form to grant one of their ables to this being at this position.
- **Revoke** — for grants the session being made, a revoke button.

### Able manager panel

- **Existing ables list** gets a `scope` column (global / anchored) + the anchor space for anchored ables.
- **Create new able** form gains a `scope` selector.
- **Granted-to view** — show, for a selected able, the list of beings currently holding it (read from being projections with anchor-filter).

### Chat panel

Already shows the LLM that'll respond. After this lands, it can also show:

- **The able under which the receiver will run** (derived from their flow + grants intersection at the asker's stance).
...
- Add `Being.qualities.ablesGranted` shape (no schema change; Mixed accepts it).
- Add `scope` to able registry. Default to `"anchored"`. Heaven-bound ables (angel) default to `"global"`.
- Add `grant-able` + `revoke-able` DO ops. The grantor's own grant chain is the auth gate.
- Genesis: after birthing seed delegates, emit `grant-able(angel, anchor=heaven)` per delegate. The I-Am is the grantor on every one of these.

### Pass 2 — authorize swap

- Add a feature flag: `useAbleAuth: bool` on `internalConfig`. When false, current authorize stands; when true, the new able-walk runs.
- Implement the able-walk authorize alongside the existing one. Both produce `{ok, reason, able?}`.
...
- Convert existing realities (seed migration step): translate `qualities.permissions` rules + `members.angel` into `ablesGranted` entries.
- Flip `useAbleAuth: true`.
...
- Delete `members.angel` / memberClass derivation. Stance properties keep `arrival` / `owner` / `contributor` for use as INPUTS into able-grant decisions (cherub still reads "did this human just register" to anoint them angel).
- Update FACTORY.md / PERMISSIONS.md / CLAUDE.md.

### Pass 5 — frontend

- Update able-manager-panel: scope picker, grants list, revoke affordance.
- Update being inspector: granted-ables section.
- Update chat panel: receiver-able indicator.

### Pass 6 — verifier + final regression

- New verifier `verify-ables-are-auth.js`: bootstrap an I-Am → cherub → human → coder grant chain; verify each step's authorize result; verify revoke removes the grant; verify scope (anchored vs global) reaches the right spaces.
- Run full regression: 283-ish suites should pass.

## Out of scope

- **Cross-world able propagation** — a granted able doesn't automatically transfer when the being walks into a foreign story. Deferred to the cross-world doctrine.
...
> **Ables are auth.** A being acts under a able; the able's canSee / canDo / canSummon / canBe IS the permission gate. There is no parallel "permissions" namespace. The able registry is authoritative.
>
> **Grants flow down the being-tree, rooted at I-Am.** Every granted able carries `{able, anchorSpaceId, grantedBy, grantedAt}`. The grant chain back to I-Am is the proof of authority.
>
> **Ables are spatial.** Each grant is anchored at a space. The able's reach is that space and its descendants (or story-wide if the able's scope is `"global"`).
>
> **Acquiring a able is an act, not a status check.** A being holds a able because someone granted it OR because they authored it themselves at a space they owned. Authorization never derives ables from stance properties.

## Verification criteria

- ✓ I-Am can do anything anywhere (bootstrap angel able at the place root, scope:global).
- ✓ Cherub can do BE birth/connect/release because the I-Am granted it the angel able at heaven.
- ✓ A new human registers via cherub; cherub grants them the `human` able at the place root.
- ✓ Human authors `coder` able at `<story>/coders/`; they get auto-granted coder at that space.
- ✓ Human births a child being; the child has zero granted ables by default. They can SEE the place root (default human able's canSee includes "place") but cannot DO anything until granted a able.
- ✓ Parent grants child the human able at place root; child can now do everything a human can.
- ✓ Coder grants a child being the coder able at the coders space; the child can do coder things in coders/ but not outside.
- ✓ A being not granted angel able cannot grant angel able to anyone else (the grant op's own canDo check refuses).
- ✓ Revoke removes the grant; subsequent calls under that able refuse.
