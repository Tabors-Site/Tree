...
export async function listReelChildren({ limit = 100 } = {}) {
  ...
  // The head fact's ord (the clock-free global append ordinal, monotonic
  // ACROSS reels) is the recency key; its date is kept only as an inert
  // display witness ("last active when"), never sorted on. Per-reel seq is
  // not cross-reel comparable, so ord is the right global order. The
  // file-native head fact IS the reel's fact at seq === head, read through
  // the curated getFactsOnReelWhere (history-aware, per-reel). One small
  // read per reel; nothing new stored.
  ...
  for (const h of heads) {
    ...
    try {
      ...
      if (headFact)
        byHash.set(String(h.headHash), {
          ord: typeof headFact.ord === "number" ? headFact.ord : 0,
          date: headFact.date || null,
        });
    } catch {
      /* best-effort recency; a missing reel just yields no ord/date */
    }
  }

  return heads
    .map((h) => {
      const hf = h.headHash ? byHash.get(String(h.headHash)) : null;
      return {
        kind: h.type,
        id: String(h.id),
        history: "0",
        ...
        // Clock-free recency order: the head fact's global append ordinal.
        headOrd: hf ? hf.ord : 0,
        // Inert display witness only; nothing sorts/compares on this.
        lastFactAt: hf ? hf.date : null,
      };
    })
    // Newest-active first by ord (clock-free), headSeq as a stable tiebreak.
    .sort((a, b) => b.headOrd - a.headOrd || b.headSeq - a.headSeq)
    .slice(0, cap);
}
