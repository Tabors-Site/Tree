...
async function getBeingOrThrow(beingId, history = "0") {
  if (!beingId) throw new Error("beingId is required");
  // loadOrFold so a being created in main is visible from any history
  // before it's been touched there. Without this, the user's first
  // create-space on a fresh history throws "Being not found" because
  // their being slot hasn't been cold-folded onto the history yet.
...
export async function createSpace({
  ...
  // History the create runs on. Threaded from the moment ctx so a
...
// Sibling order is derived from bornOrd by listSpaceChildren; an
 ...
 */
export async function resolveSpaceAccess(spaceId, beingId, history) {
  if (!spaceId) {
    return {
      ok: false,
  ...
  const ancestors = await getAncestorChain(String(spaceId), history);
  if (!ancestors) {
    return {
      ok: false,
...
export async function listSpaceChildren(
  ...
  if (history === "0") {
    const rows = findByParent("0", parentId, "space")
      ...
      .map(toRow);
    // Sibling order is birth order: bornOrd is the birth fact's append
    // ordinal (clock-free), seeded into space state by applyCreateSpace
    // and surfaced top-level by toRow. Ascending = earliest-created
    // first. No wall-clock (new Date(createdAt)) anywhere.
    rows.sort((a, b) => (a.bornOrd ?? 0) - (b.bornOrd ?? 0));
  ...
  // Same birth-order sort as the main path: bornOrd (clock-free birth
  // ordinal), ascending = earliest-created first. No wall-clock.
  all.sort((a, b) => (a.bornOrd ?? 0) - (b.bornOrd ?? 0));
