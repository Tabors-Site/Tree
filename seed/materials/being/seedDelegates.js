...
// ───────────────────────────────────────────────────────────────────
// The grants moved to genesis.word (the creation sequence).
// ───────────────────────────────────────────────────────────────────
//
// grantAngelToSeedDelegates (the JS that granted each delegate the angel able at heaven + its own
// able at the story root) is now the I's acts, read and run from story/seed/store/genesis.word by
// genesis.js's readAndRunGenesisWord — one grant = one moment. The grant reducer dedupes by
// (able, anchor, grantor), so the Word run is idempotent across Awakenings, exactly as the JS was.
// SEED_DELEGATES above stays the roster the host reader resolves to being ids (the lookup is floor;
// the grant is the Word).
