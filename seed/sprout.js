...
//
// The be:birth Fact self-stamps: beingId points at the not-yet-existing Being row whose
// materialization the same Fact triggers. ensureIAm declares `I` as a Name in code (the one
// host-stamped fact, on the library reel — the irreducible turtle), then READS the verse
// `am "what?" am` word-by-word through I (readSelfGenesis). After the Name exists, a word needs no
// being to run — only a name to be stamped through — so the genesis joins "a name reads a word, one
// word per act," the same mechanism that runs the rest of genesis.word.
//
...
// readSelfGenesis — the self-grounding bootstrap reader (word.word's first turn). The verse
// `am "what?" am`, READ word-by-word and stamped THROUGH I (the Name in code). `am` is a grounded
// primitive of THIS reader — am → the first-person self-birth (be:birth, self-stamped) — the word that
// creates the thing that could fold words, so it cannot itself need the fold. After it the being
// exists, and the `"what?"` recall folds its OWN one-act chain (the world's first recall) with the real
// inner fold; the second `am` is the verdict ("I am that I am"). `be` is reserved for births of OTHERS;
// `am` is only ever the self. One word, one act, one reel.
async function readSelfGenesis(verse, { beingId, qualities }) {
  const words = verse.match(/"[^"]*"|\S+/g) || [];
  let born = false;
  for (const raw of words) {
    const word = raw.replace(/^"|"$/g, "");
    if (word === "am" && !born) {
      // the self folds into being — first-person be:birth, self-stamped through I.
      await withIAmAct("am", async (ctx) => {
        await emitFact(
          {
            verb: "be",
            act: "birth",
            through: beingId, // self-stamping: the not-yet-existing being is its own actor, through I
            of: { kind: "being", id: beingId },
            params: {
              name: I,
              ables: [],
              defaultAble: null,
              trueName: I, // the being expresses the I Name declared in code above
              parentBeingId: null,
              homeSpace: null,
              position: null,
              qualities,
      ...
      // the quoted RECALL: the just-born being folds its OWN first chain — the inner fold, the world's
      // first recall, a read (no new fact). Run for real so the recall machinery is exercised on the
      // very first chain; a hiccup here is logged, never fatal to genesis.
      try {
        const { foldPlace } = await import(
          "./present/stamper/2-fold/foldPlace.js"
        );
        const { ORIENTATION } = await import(
          "./present/stamper/2-fold/orientation.js"
        );
        const inner = await foldPlace(beingId, ORIENTATION.INWARD, {
          history: "0",
        });
        log.info(
          "Story",
          `I recall "what?" — folded ${inner?.actChain?.length ?? 0} act(s) of my own chain.`,
        );
      } catch (e) {
        log.warn("Story", `recall "what?" did not fold: ${e.message}`);
      }
    } else if (word === "am" && born) {
      // the verdict: the self recognizes the self — the recall's answer. "I am that I am."
      log.info("Story", "I am that I am.");
    }
  }
}
...
export async function ensureIAm() {
  ...
  // The verse `I am "what?" I am`, one word at a time, each its own act on its own reel (the
  // one-word-one-reel law). `I` is declared as a NAME in code (below) — the irreducible turtle, on the
  // library reel. The rest is the verse `am "what?" am`, READ through I (readSelfGenesis above): `am`
  // self-births the being, `"what?"` recalls the just-formed chain, `am` is the verdict. `be` is
  // reserved for births of OTHERS; the self is always `am`.
  ...
  // `am` — the SELF folds into being. The genesis being is no longer hand-stamped: it is the verse
  // `am "what?" am`, READ word-by-word and stamped THROUGH I (the Name declared in code above) — the
  // same "a name reads a word, one word per act" that runs the rest of genesis.word. `am` is the
  // first-person self-birth (be:birth, self-stamped); `be` is reserved for birthing OTHERS. `"what?"`
  // is the quoted recall (the just-born being folds its own first chain) and the second `am` is the
  // verdict (I am that I am). See readSelfGenesis — the world's first recall, the instant it began.
  await readSelfGenesis(`am "what?" am`, { beingId: id, qualities });
