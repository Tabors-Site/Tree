...
fn word_to_fact_pipe_pure_rust() {
    ...
    assert_eq!((kind.as_str(), id.as_str()), ("space", "garden"), "I make garden -> create-space on (space, garden)");
    ...
    assert_eq!(get_str(&facts[0], "act"), Some("create-space"), "the fact carries the word's act");
