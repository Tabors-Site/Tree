...
//       create-space -> the new space reel exists + verifies, and the being woke exactly once.
...
//   (F-seal) the FILE-flow SEAL leg: the SAME parameterized flow deed (`do create-space on the space
//       <ref> with { name, type }`), decided by the conductor and SEALED end-to-end over a real
//       genesis-born world (the vocabulary coined so `create-space` resolves as an op word). The deed
...
//       create-space fact — which lands on a real space reel and chain-VERIFIES. Node-free, clock-free.
...
/// `do create-space on the space <id> with { name, type }` is parsed AND rendered by treeword (the
...
fn plant_flowfile_able(ables_dir: &std::path::Path, space: &str) {
    ...
    let body = [
        ...
        "A builder can do create-space.".to_string(),
        ...
        format!("  do create-space on the space {space} with {{ name: \"{space}\", type: \"home-territory\" }}."),
        String::new(),
    ]
    .join("\n");
    std::fs::write(ables_dir.join("builder.word"), body).unwrap();
...
/// built create-space node that renders to "I make <space>.".
...
fn summoned_being_wakes_decides_acts_chain_advances() {
    ...
    assert_eq!(get_str(stamped, "act"), Some("create-space"), "the decided Word was create-space");
    ...
    println!("  treeos present-loop Phase 1 (A): summon -> wake -> scripted decide -> ACT stamps create-space -> chain verifies (Node-free, clock-free)  OK");
}
...
fn scripted_word_file_flow_with_a_parameterized_deed_drives_the_loop() {
    ...
    // (`do create-space on the space <id> with { name, type }`). The conductor PARSES the able's flows off
    // disk (no injected decision) and RENDERS the matched effect to the spoken decision Word. The renderer
    // no longer DROPS the deed's target/params (the of.id genesis guard keeps a flow `do create-space`
    // deed from collapsing to the genesis "I make ."), so the being decides the FULL deed
    // `do create-space on the space <id> with { name: ..., type: ... }`; end-to-end FROM THE FILE, Node-free.
    //
    ...
    // SEAL of a create-space DEED is the op-word path's job; a flow deed targets `{ref}`, resolved by the
    ...
    assert!(content.starts_with("do create-space on the space "), "spoke the targeted deed (not the genesis stub): {content:?}");
    ...
    let _ = std::fs::remove_dir_all(&base);
    std::env::remove_var("TREE_ABLES_DIR");
    println!("  treeos present-loop Phase 1 (F): scripted .word FILE flow (parameterized deed) -> load-from-file -> decide -> ACT speaks the FULL `do create-space ... with {{ name, type }}` (target + params carried, not dropped), Node-free  OK");
}
...
/// here is the real one (every op declared on chain, `create-space` resolving as a kind:"op" word).
...
/// Plant a scratch scripted "builder" able whose `.word` flow holds a parameterized create-space deed
...
fn plant_seal_flow_able(ables_dir: &std::path::Path, space_ref: &str) {
    std::fs::create_dir_all(ables_dir).unwrap();
    let body = [
        "# scratch scripted builder whose flow SEALS a real create-space (valid space type).".to_string(),
        "A builder is an able.".to_string(),
        "A builder can do create-space.".to_string(),
        "A builder can see place.".to_string(),
        "A builder reaches /**.".to_string(),
        ...
        String::new(),
        "When the sky is summoned:".to_string(),
        format!("  do create-space on the space {space_ref} with {{ name: \"{space_ref}\", type: \"place\" }}."),
        String::new(),
    ]
    .join("\n");
    std::fs::write(ables_dir.join("builder.word"), body).unwrap();
...
fn scripted_word_file_flow_deed_seals_a_real_fact_end_to_end() {
    ...
    // `do create-space on the space <ref> with { name, type }` is SUMMONED -> the conductor WAKES it,
    ...
    // (op_word_via_fold resolves `create-space` from the chain word-fold; derive_trigger's literal-ref
    // recovery supplies the `{ref}` id; the create-space `.word` body runs through the host see-op and
    // enriches the fact). The proof: a REAL create-space fact stamps + its space reel chain-VERIFIES.
    //
    // The vocabulary must be coined on chain for `create-space` to fold as a kind:"op" word, so this
    ...
    // 1. BIRTH a real world: the vocabulary coined (so `create-space` resolves as an op word), the
    ...
    if let treecognition::Cognition::Act { content } = &r.decision {
        assert!(content.starts_with("do create-space on the space "), "the decided Word is the targeted deed: {content:?}");
        assert!(content.contains(space_ref), "the decided Word carries its `{{ref}}`: {content:?}");
    }

    // ── the SEAL stamped a real create-space fact (NOT just rendered) ────────────────────────────────
    ...
    assert_eq!(get_str(stamped, "act"), Some("create-space"), "the SEAL is a create-space fact");
    ...
    // the create-space op DERIVES the new space's reel id (id-from-nature); read it off the sealed fact.
    ...
    assert_eq!(get_str(&new_reel[0], "act"), Some("create-space"), "the reel's first fact is the sealed create-space");
    let verdict = verify_fact_chain(&new_reel);
    assert!(ok_true(&verdict), "the sealed space chain verifies: {}", treehash::stringify(&verdict));

    let _ = std::fs::remove_dir_all(&base);
    std::env::remove_var("TREE_ABLES_DIR");
    println!("  treeos present-loop Phase 1 (F-seal): scripted .word FILE flow deed (`do create-space on the space {{ref}} with {{ name, type }}`) -> summon -> wake -> decide -> SEAL via the op-word path (op_word_via_fold + literal-ref recovery) -> a REAL create-space fact lands + chain-verifies (Node-free, clock-free)  OK");
}
