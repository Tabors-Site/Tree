...
fn full_seam_real_act_drives_the_emit_hook() {
    ...
    // B subscribes to "create-space" everywhere (the event = the sealed fact's act name).
    let b = "watcher3";
    assert!(subscriptions::apply_fact(&sub_registered(b, "s2", "create-space", "INTERACTIVE")));

    // a sealed create-space fact (as it would land off a reel: act + of=space + ord) drives the hook.
    let mut woke: Vec<String> = Vec::new();
    let sealed = emitted_fact("builderA", "create-space", "space", "meadow", 7.0);
    let n = subscriptions::emit_for_fact(&sealed, |being, _entry| woke.push(being.to_string()));
    assert_eq!(n, 1, "the create-space seal woke the subscriber");
    assert_eq!(woke, vec![b.to_string()]);

    subscriptions::reset_all();
    println!("  treeos present-loop Phase 2 (E): a real sealed fact (create-space) drives the emit hook -> wake  OK");
}
