...
fn authorized_act_stamps_and_denied_act_does_not() {
    ...
    let builder_spec = pj(r#"{"canDo":["create-space"],"reach":["/**"]}"#).unwrap();
    ...
    // 4. AUTHORIZED PATH — "I make garden." -> do:create-space; builder permits it.
    let ir = treeword::parse("I make garden.");
    let act = &ir[0];
    let decision = authorize_act(act, &b1, &grants, &builder_spec, "garden");
    assert!(ok_true(&decision), "b1 (holding builder) is authorized to create-space");
    ...
    // 5. DENIED PATH — a stranger with no grant cannot create-space; nothing is stamped.
