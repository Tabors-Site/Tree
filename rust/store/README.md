...
- Coined name = the file stem (`kill.word` coins `kill`). Exception (transitional): `create.word` /
  `index.js`-style names coin as their bundle dir — dying with the M1C rename to verb-truth
  (`create-space`→`make` on a space).
