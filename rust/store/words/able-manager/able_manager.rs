...
// do:create-space / do:end-space chain write on the .ables heaven space reel) AND hot-(un)register the
...
//   - the manifest WRITE (the do:create-space / do:end-space on .ables) -> caller-side seal, the SAME
...
//     `able-exists` / create-matter's extension-type gate make). `hotRegistered: true` is preserved as
