...
// ── resolve-birth-space (create-space) ──────────────────────────────────────────────────────────────
...
/// "no caller". Returns the enriched birth spec the do:create-space fact records.
pub fn resolve_create_space(
    ...
    let caller = match caller {
        ...
        _ => {
            return Err(HostError::unauthorized(
                "create-space requires an identified actor",
            ))
        }
    };
    ...
    // the caller's name-floor, matching how create-matter's name floor stays caller-assistable).
    ...
    let name = match &raw_name {
        ...
        _ => {
            return Err(HostError::invalid(
                "create-space: `name` must be a string",
            ))
        }
    };
    ...
    if !name.is_empty() {
        ...
        if !name_unique(root, &history, "space", &name, &scope, None)? {
            return Err(HostError::name_taken("create-space", &name, &history));
        }
    }
