// The HOST SEE-OP SEAM, END-TO-END: a materials `.word` (set-being / create-space) runs through
...
// ── create-space: the birth see-op enriches the spec, the fact stamps; the coord/size gate refuses ───
#[test]
fn host_seam_create_space_end_to_end_and_size_gate() {
    ...
    let word = store_word("create-space/create.word");
    // run AS the maker being (a real caller - create-space's resolver requires an identified actor);
    // run the authorize as I-less is fine because we attribute through the maker but authorize via I?
    // No: create-space authorize needs a grant. To keep this test on the SEE-OP seam (not re-test the
    ...
    // spaceId, the Return synthesizes the do:create-space fact (params = the enriched spec), it stamps.
    let trigger = obj(vec![
    ...
    let out = run_op_word(&word, &i, &trigger, &dir, "0", no_spec, None, None);
    assert_eq!(out.len(), 1, "one do:create-space fact");
    let fact = match &out[0] {
        Outcome::Authorized(f) => f,
        Outcome::Denied(r) => panic!("create-space end-to-end denied (the seam did not run): {r}"),
    };
    assert_eq!(get_str(fact, "act"), Some("create-space"), "the create-space op");
    ...
    assert!(
        sp_facts.iter().any(|f| get_str(f, "act") == Some("create-space")),
        "the create-space fact is on the new space's reel"
    );
    ...
    match denied.first() {
        Some(Outcome::Denied(reason)) => {
            ...
            println!("    create-space size gate refused with: {reason}");
        }
