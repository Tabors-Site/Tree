...
fn builder_spec_of(name: &str) -> Option<Json> {
    if name == "builder" {
        Some(pj(r#"{"canDo":["create-space"],"canSee":["*"],"reach":["/**"]}"#).unwrap())
    } else {
        None
    }
}
...
fn ibp_moment_seal_writes_act_then_fact() {
    ...
    assert_eq!(get_str(&facts[0], "act"), Some("create-space"), "the fact carries the op");
...
fn ibp_multi_act_bindings_propagate() {
    ...
    let acts = [
        pj(r#"{"kind":"act","verb":"do","act":"create-space","by":"I","of":{"kind":"space","id":"home"},"bind":"h"}"#).unwrap(),
...
fn ibp_fold_able_spec_authorizes() {
    ...
    // scribe has no canDo -> do:create-space is denied
    let v2 = treeibp::authorize("do", Some("create-space"), Some("root"), None, &b1, &dir, "0", &spec_of);
    assert!(!ok_true(&v2), "scribe denied do:create-space (not in its can-set)");
