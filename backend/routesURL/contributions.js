...
router.get("/node/:nodeId/:version/contributions", urlAuth, async (req, res) => {
  try {
      ...
      switch (c.action) {
        ...
        case "updateChildNode": {
          ...
          return uc.action === "added"
            ? `Added child ${link(uc.childId)}`
            : `Removed child ${link(uc.childId)}`;
        }
        ...
        case "rawIdea": {
          ...
          if (ri.action === "placed") {
            const target = ri.targetNodeId ? link(ri.targetNodeId) : "node";
    ...
    const items = contributions.map((c) => {
      ...
      const actionHtml = renderAction(c);
