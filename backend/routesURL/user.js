...
router.get("/user/:userId", urlAuth, async (req, res) => {
  try {
    ...
    return res.send(renderUserProfile({ userId, user, roots, profileType, energy, extraEnergy: user.additionalEnergy, queryString, resetTimeLabel, storageUsedKB }));
  } catch (err) {
...
router.get("/user/:userId/contributions", urlAuth, async (req, res) => {
  try {
    ...
    return res.send(await renderUserContributions({ userId, contributions, username, getNodeName, token }));
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
  }
...
router.get("/user/:userId/raw-ideas", urlAuth, async (req, res) => {
  try {
    ...
    return res.send(renderRawIdeasList({ userId, user, rawIdeas, query, statusFilter, tabs, tabUrl, token, AUTO_PLACE_ELIGIBLE }));
  } catch (err) {
  ...
  try {
    ...
    if (req.query.html !== undefined && process.env.ENABLE_FRONTEND_HTML === "true") {
      ...
      if (rawIdea.contentType === "text") {
        return res.send(renderRawIdeaText({ userId, rawIdea, back, backText, userLink, hasToken, token }));
      }

      // ---------- FILE ----------

      return res.send(renderRawIdeaFile({ userId, rawIdea, back, backText, userLink, hasToken, token }));
    }
...
router.post("/user/:userId/api-keys", authenticate, async (req, res) => {
  ...
  try {
    ...
    return res.status(201).send(renderApiKeyCreated({ userId, safeName, rawKey, token }));
  } catch (err) {
...
router.get("/user/:userId/api-keys", authenticate, async (req, res) => {
  try {
    ...
    return res.send(renderApiKeysList({ userId, user, apiKeys, token, errorParam }));
  } catch (err) {
...
router.get("/user/:userId/energy", urlAuth, async (req, res) => {
  try {
    ...
    return res.send(renderEnergy({ userId, user, energyAmount, additionalEnergy, profileType, planExpiresAt, llmConnections, mainAssignment, rawIdeaAssignment, activeConn, hasLlm, connectionCount, isBasic, qs }));
  } catch (err) {
...
router.get("/user/:userId/chats", urlAuth, async (req, res) => {
  try {
    ...
    return res.send(renderChats({ userId, chats, sessions, username, token, sessionId }));
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
  }
