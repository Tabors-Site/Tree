...
import { createSession, endSession, setSessionAbort, clearSessionAbort, abortSessionsByScope, SESSION_TYPES } from "../ws/sessionRegistry.js";
import { resolveTreeAccess } from "./authenticate.js";

const BUSY_MESSAGE = "I'm already processing your last 2 messages. Please send again later.";
 ...
 */
export async function processGatewayMessage(channelId, { senderName, senderPlatformId, messageText }) {
  ...
  if (lowerTrimmed === "cancel" || lowerTrimmed === "stop") {
    ...
    console.log(`Gateway: ${lowerTrimmed} command for channel ${channelId}, aborted ${abortCount} in-flight message(s)`);
    ...
    try {
      ...
      console.error("Gateway: failed to finalize AIChats on cancel:", err.message);
    }
  ...
  // 6. Check LLM access
  var rootCheck = await Node.findById(channel.rootId).select("rootOwner llmAssignments").lean();
  var hasUserLlm = await userHasLlm(channel.userId);
  var hasRootLlm = !!rootCheck?.llmAssignments?.placement;
  ...
  try {
    ...
    aiChat = await startAIChat({
      ...
      source: "gateway",
      modeKey: "tree:" + (channel.mode === "write" ? "place" : channel.mode === "read" ? "query" : "chat"),
      llmProvider: {
  ...
  var result = await enqueue(queueKey, async () => {
    var timedOut = false;
    ...
    var timer = setTimeout(() => {
      ...
      if (aiChat) {
        finalizeAIChat({ chatId: aiChat._id, content: "Error: Request timed out", stopped: false }).catch(() => {});
      }
    }, TIMEOUT_MS);

    try {
      var internalJwt = jwt.sign(
        { userId: channel.userId.toString(), username: user.username, visitorId },
        JWT_SECRET,
        { expiresIn: "1h" },
      );
      ...
      if (aiChat) {
        ...
        var answer = wasAborted ? "Cancelled by user" : (orchResult?.answer || orchResult?.reason || null);
        finalizeAIChat({
          chatId: aiChat._id,
          content: answer,
          stopped: wasAborted,
          modeKey: orchResult?.modeKey || "tree:orchestrator",
        }).catch((err) => console.error("Gateway: AIChat finalize failed:", err.message));
      }

      return orchResult || { success: false, answer: "Could not process your message." };
    } catch (err) {
      clearTimeout(timer);
      if (timedOut) return { success: false, answer: "Request timed out." };
      ...
      if (aiChat) {
        finalizeAIChat({
          chatId: aiChat._id,
          content: abort.signal.aborted ? "Cancelled by user" : "Error: " + err.message,
