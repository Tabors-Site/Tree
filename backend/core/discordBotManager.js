...
export async function connectBot(botToken, channelId, discordChannelId, rootId) {
  await loadDiscord();
  ...
  if (entry) {
    ...
    console.log(`Discord bot manager: added channel ${channelId} to existing bot (${entry.channelMap.size} channels)`);
    return;
  }
  ...
  client.on("messageCreate", async (message) => {
    ...
    for (var [gwChannelId, config] of channelMap) {
      ...
      var senderName = message.author.displayName || message.author.username || "Unknown";
      var senderPlatformId = message.author.id;
      var messageText = message.content;

      if (!messageText || !messageText.trim()) continue;

      console.log(`Discord bot: message on gw channel ${gwChannelId} from ${senderName}: "${messageText.slice(0, 80)}"`);
      ...
      try {
        ...
        console.error(`Discord bot: error processing message for channel ${gwChannelId}:`, err.message);
      }
    }
  });
  ...
  try {
    ...
    console.log(`Discord bot manager: bot connected as ${client.user?.tag}, ${channelMap.size} channel(s)`);
  } catch (err) {
...
export async function disconnectBot(channelId) {
  for (var [key, entry] of activeBots) {
    if (entry.channelMap.has(channelId)) {
      ...
      console.log(`Discord bot manager: removed channel ${channelId} (${entry.channelMap.size} remaining)`);
      ...
      if (entry.channelMap.size === 0) {
        ...
        console.log("Discord bot manager: bot client destroyed (no channels left)");
      }
      return;
    }
  }
...
export async function startupScan() {
  try {
    var GatewayChannel = (await import("../db/models/gatewayChannel.js")).default;
    ...
    console.log(`Discord bot manager: found ${channels.length} Discord input channel(s) to connect`);

    for (var channel of channels) {
      try {
        var full = await getChannelWithSecrets(channel._id);
        if (!full?.config?.decryptedSecrets?.botToken) {
          console.error(`Discord bot manager: no bot token for channel ${channel._id}`);
          continue;
        }
        ...
        console.error(`Discord bot manager: failed to connect channel ${channel._id}:`, err.message);
      }
    }
  } catch (err) {
