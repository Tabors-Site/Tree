  ...
  switch (type) {
    ...
    case "discord": {
      if (hasInput) {
        ...
        if (!config.discordChannelId || typeof config.discordChannelId !== "string") {
...
export async function addGatewayChannel(userId, rootId, { name, type, direction, mode, config, notificationTypes, queueBehavior }) {
  ...
  if (type === "discord" && hasInput) {
    ...
    if (!user || !PAID_TIERS.includes(user.profileType)) {
      throw new Error("Discord input channels require a Standard, Premium, or God tier subscription");
    }
  }
  ...
  if (hasInput && channel.enabled) {
    registerInputChannel(channel).catch((err) =>
      console.error(`Gateway: failed to register input for channel ${channel._id}:`, err.message),
    );
  }
...
export async function updateGatewayChannel(userId, channelId, updates) {
  ...
  if (updates.queueBehavior !== undefined) {
    channel.queueBehavior = updates.queueBehavior === "silent" ? "silent" : "respond";
  }
  ...
  if (updates.config !== undefined) {
    ...
    channel.config = buildEncryptedConfig(channel.type, updates.config, channel.direction);
  }
  ...
  if (hasInput) {
    if (!wasEnabled && channel.enabled) {
      ...
      unregisterInputChannel(channel).catch((err) =>
        console.error(`Gateway: failed to unregister input for channel ${channel._id}:`, err.message),
      );
    }
  }
...
async function registerInputChannel(channel) {
  if (channel.type === "telegram") {
    ...
    console.log(`Gateway: Telegram webhook registered for channel ${channel._id}`);
...
async function unregisterInputChannel(channel) {
  if (channel.type === "telegram") {
    try {
      var secrets = JSON.parse(decrypt(channel.config.encryptedPayload));
      await fetch(`https://api.telegram.org/bot${secrets.botToken}/deleteWebhook`, {
        method: "POST",
      });
      console.log(`Gateway: Telegram webhook removed for channel ${channel._id}`);
    } catch (err) {
      console.error(`Gateway: failed to remove Telegram webhook for ${channel._id}:`, err.message);
    }
  } else if (channel.type === "discord") {
    try {
      var { disconnectBot } = await import("./discordBotManager.js");
      await disconnectBot(channel._id);
      console.log(`Gateway: Discord bot disconnected for channel ${channel._id}`);
    } catch (err) {
      console.error(`Gateway: failed to disconnect Discord bot for ${channel._id}:`, err.message);
    }
  }
}
