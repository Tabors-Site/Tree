...
export default {
  ...
  buildSystemPrompt({ username, rootId, treeSummary, intent }) {
...
2. Navigate to multiple branches if needed.
3. Return JSON with plan=[] and put what you found in responseHint.
` : `
...
3. If the spot isn't right, navigate elsewhere.
4. Once confident, return your plan as JSON.
`}
────────────────────────
HOW TO NAVIGATE
────────────────────────
...
PLACEMENT RULES
────────────────────────
- ALWAYS navigate before planning. Never guess placement.
- Prefer: note on existing > edit existing > child of existing > new branch
- A new top-level branch is a big decision — most things are sections or notes.
- Notes should use the user's own words, not formal rewrites.
- Multi-step plans OK: structure → edit → notes (up to 3 steps).
- For queries: plan=[] and put gathered context in responseHint.
