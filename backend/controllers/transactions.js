...
const getTransactions = async (req, res) => {
  ...
  if (!nodeId) {
    return res
      .status(400)
      .json({ success: false, message: "nodeId is required." });
  }
  try {
...
const tradeValuesBetweenNodes = async (
  nodeAId,
  versionAIndex,
  valuesA,
  nodeBId,
  ...
  for (const [key, value] of Object.entries(valuesA)) {
    if ((versionA.values.get(key) || 0) < value) {
      throw new Error(
        `This node's version ${versionAIndex} has insufficient ${key}.`
      );
    }
  ...
  for (const [key, value] of Object.entries(valuesB)) {
    if ((versionB.values.get(key) || 0) < value) {
      throw new Error(
        `The partnering node (version ${versionBIndex}) has insufficient ${key}.`
      );
    }
...
const tradeValues = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Trade completed successfully." });
  } catch (error) {
