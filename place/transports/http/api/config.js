...
router.get("/place/root", authenticateOptional, async (req, res) => {
  try {
    const placeRootCached = await getPlaceRoot();
    if (!placeRootCached) {
    ...
    const placeRoot = await Space.findById(placeRootCached._id).select("_id name children").lean();

    const beingId = req.beingId;
    const isAnon = !beingId;

    // Fetch all place-root children with the fields needed to filter.
    const children = await Space.find({ _id: { $in: placeRoot.children } })
