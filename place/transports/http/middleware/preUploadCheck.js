...
export default function preUploadCheck(req, res, next) {
  ...
  // Master switch
  const enabled = getPlaceConfigValue("uploadEnabled");
  if (enabled === false || enabled === "false") {
    return sendError(res, 403, IBP_ERR.UPLOAD_DISABLED, "Uploads are disabled on this place");
  }

  // Size ceiling
  const maxBytes = Number(getPlaceConfigValue("maxUploadBytes")) || 104857600;
  ...
  // MIME filter
  const allowed = getPlaceConfigValue("allowedMimeTypes");
