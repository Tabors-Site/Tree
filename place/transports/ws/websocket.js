...
export function initWebSocketServer(httpServer, originPolicy) {
  ...
  io = new Server(httpServer, {
    ...
    maxHttpBufferSize:
      Number(getPlaceConfigValue("socketMaxBufferSize")) || 1048576,
    pingTimeout: Number(getPlaceConfigValue("socketPingTimeout")) || 30000,
    pingInterval: Number(getPlaceConfigValue("socketPingInterval")) || 25000,
    connectTimeout: Number(getPlaceConfigValue("socketConnectTimeout")) || 10000,
  });

  // Per-IP connection limit
  const ipCounts = new Map();
  const MAX_PER_IP = Number(getPlaceConfigValue("maxConnectionsPerIp")) || 20;
