...
// (which it has cached as a PlacePeer) and runs the verb locally.
//
// On the receive side, `verifyIncoming` reads the X-Canopy-Sender +
// X-Canopy-Signature headers, looks up the sender in PlacePeer, and
 ...
 * domain or null.
 */
export function extractTargetPlace(address) {
...
export function getForeignTargetDomain(address) {
  const local = getPlaceDomain();
  const target = extractTargetPlace(address);
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: {
        ...
        "X-Canopy-Sender":      getPlaceDomain(),
