...
//
// Run: node --test --experimental-test-module-mocks place/test/doTriggerSubstrate.test.js
...
describe("DO-trigger substrate — single subscriber", () => {
  test("emit → inbox → scheduler → echoEmbodiment fires", async () => {
    ...
    //   2. appendToInbox at place root (subscriber-side, b1's bucket)
    //   3. wake(b1, place-root-id) — the scheduler picks it up
...
describe("DO-trigger substrate — priority field propagates to the inbox", () => {
  test("subscription's priority places on the appended inbox entry", async () => {
    ...
    // both entries place before processing starts. Here we verify the
