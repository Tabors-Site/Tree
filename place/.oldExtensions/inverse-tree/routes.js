...
// GET /user/:beingId/inverse - profile as the AI sees it
router.get("/user/:beingId/inverse", (req, res, next) => {
  if ("html" in req.query) return next("route");
  next();
}, authenticate, async (req, res) => {
  try {
    // Users can only read their own inverse profile
    if (req.params.beingId !== req.beingId) {
      return sendError(res, 403, ERR.FORBIDDEN, "Can only view your own inverse profile");
    }
    const data = await getProfile(req.beingId);
...
// POST /user/:beingId/inverse/correct - manual correction
router.post("/user/:beingId/inverse/correct", authenticate, async (req, res) => {
  try {
    if (req.params.beingId !== req.beingId) {
    ...
    const corrections = await addCorrection(req.beingId, text);
...
// POST /user/:beingId/inverse/reset - wipe profile
router.post("/user/:beingId/inverse/reset", authenticate, async (req, res) => {
  try {
    if (req.params.beingId !== req.beingId) {
      return sendError(res, 403, ERR.FORBIDDEN, "Can only reset your own inverse profile");
    }
    await resetProfile(req.beingId);
