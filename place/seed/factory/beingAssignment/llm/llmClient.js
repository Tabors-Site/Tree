 ...
 *   5. noLlm sentinel
 *
 * A future cross-place step ("summoner's home being") slots between 4
...
export async function getClientForBeing(beingId, slot, overrideConnectionId) {
  ...
  // 4. Place-level default (operator-configured fallback).
  try {
    ...
    if (placeLlmId) {
      ...
      const placeEntry = await resolveConnection(placeLlmId, placeCacheKey);
