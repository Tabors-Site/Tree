...
// in the place's uploads/ directory. When that Matter row is deleted
...
function maxDeletionsPerCycle() { return Math.max(10, Math.min(Number(getPlaceConfigValue("uploadCleanupBatchSize")) || 1000, 50000)); }
...
export function startUploadCleanup({
  intervalMs = Number(getPlaceConfigValue("uploadCleanupInterval")) || DEFAULT_INTERVAL_MS,
  graceMs    = Number(getPlaceConfigValue("uploadGracePeriodMs"))   || DEFAULT_GRACE_MS,
} = {}) {
