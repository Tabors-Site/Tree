...
// cascading. When a Matter write places at a space whose
// qualities.cascade.enabled is true, and the place has cascadeEnabled
...
// across places through Canopy, queue downstream work, write results
// to .flow.
//
// Two gates only, both substrate:
//   1. qualities.cascade.enabled on this space.
//   2. cascadeEnabled in place config.
...
export async function checkCascade(spaceId, writeContext) {
  // Check global enable
  const enabled = getPlaceConfigValue("cascadeEnabled");
  ...
  const rateLimit = parseInt(getPlaceConfigValue("cascadeRateLimit") || "60", 10);
...
export async function deliverCascade({ spaceId, signalId, payload = {}, source, depth = 0 }) {
  ...
  const maxDeliveriesPerSignal = parseInt(getPlaceConfigValue("cascadeMaxDeliveriesPerSignal") || "500", 10);
  if (signalId) {
  ...
  const maxPayloadBytes = parseInt(getPlaceConfigValue("cascadeMaxPayloadBytes") || "51200", 10);
  let payloadBytes;
  try {
  ...
  // Check depth limit
  const maxDepth = parseInt(getPlaceConfigValue("cascadeMaxDepth") || "50", 10);
  if (depth > maxDepth) {
  ...
  if (space.seedSpace) {
    const result = { status: CASCADE.REJECTED, source: spaceId, payload: { reason: "place seed spaces do not cascade" }, timestamp: new Date(), signalId };
    await writeResult(signalId, result);
    return result;
  }
 ...
 * any write places. To prevent exceeding the 16MB BSON limit:
...
async function writeResult(signalId, result) {
  try {
    ...
    if (!partitionId) {
      log.error("Cascade", "No .flow place seed space found. Cannot write result.");
      return;
    }
    ...
    const maxPerDay = parseInt(getPlaceConfigValue("flowMaxResultsPerDay") || "10000", 10);
    const results = partition.qualities instanceof Map
      ? partition.qualities.get("results") || {}
      : partition.qualities?.results || {};
...
export async function cleanupExpiredResults() {
  const ttl = parseInt(getPlaceConfigValue("resultTTL") || "604800", 10);
