...
function MAX_ENTRIES() {
  return Number(getPlaceConfigValue("ancestorCacheMaxEntries")) || 50000;
}
function MAX_DEPTH() {
  return Math.max(
    10,
    Math.min(Number(getPlaceConfigValue("ancestorCacheMaxDepth")) || 100, 500),
  );
}
...
function getTTL() {
  const configured = getPlaceConfigValue("ancestorCacheTTL");
 ...
 * The array is ordered from the space itself to root (or the last non-place seed space).
 *
