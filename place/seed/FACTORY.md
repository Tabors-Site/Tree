...
`place/` is the project — the Tree's root, named for what it produces.
Inside it, `seed/` is the factory: the whole apparatus that produces
reality. The factory holds three tenses.
...
└── system/          the host floor — runtime, wire, the world outside
```

`ibp/` carries the verbs — the universal currency every act speaks.
`system/` is the host floor that knows nothing of the world.
