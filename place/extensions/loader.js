...
async function runNpmInstall(extDir, npmDeps, extName, opts = {}) {
  ...
  try {
    ...
    const configured = getPlaceConfigValue("npmInstallTimeout");
...
export async function registerExtensionManagementOps() {
  ...
  registerOperation("disable-extension", {
    ...
    handler: async ({ params }) => {
      ...
      const current = getPlaceConfigValue("disabledExtensions") || [];
      if (!current.includes(name)) {
        current.push(name);
        syncDisabledFile(current);
        await setPlaceConfigValue("disabledExtensions", current);
      }
  ...
  registerOperation("enable-extension", {
    ...
    handler: async ({ params }) => {
      ...
      await setPlaceConfigValue("disabledExtensions", updated);
...
export async function disableExtension(name) {
  ...
  try {
    ...
    if (!dbList.includes(name)) {
      dbList.push(name);
      await setPlaceConfigValue("disabledExtensions", dbList);
    }
  } catch {
 ...
 *
 * @param {Function} [configFn] - optional config reader (getPlaceConfigValue)
 * @returns {Set<string>}
 */
