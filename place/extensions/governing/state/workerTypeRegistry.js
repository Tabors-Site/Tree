 ...
 * Pass 1 where one workspace is active per place in practice. If two
 ...
 * insertion-order first-match wins, which means places with multiple
 ...
 *   2. NO workspace extensions are registered in this place at all
 *      (governing-alone case). Places that install governing without
 *      any workspace are coordination-only places; every message at a
 *      tree position goes through the Ruler.
 *
 * Returns false when workspaces exist in the place but none is active
...
export async function shouldGovernAtScope(spaceId) {
  ...
  // Case 2: no workspaces registered → governing-alone place. Every
