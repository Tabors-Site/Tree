...
// Query: Summon.find({ sessionId }).sort({ chainIndex: 1 }) -> full chain
...
//
// Tool-call → summonId correlation used to flow through a global Map
// keyed by aiSessionKey (`activeAiContext`). Slice 2 of the
// per-being / per-Portal-Address refactor deleted that Map: callers
// now thread `summonId` and `sessionId` through processMessage's ctx,
...
export async function finalizeOpenSummon(socket) {
  ...
  try {
    ...
    log.debug("AI", `Finalized orphaned chat ${active.summonId} for ${socket.aiSessionKey}`);
  } catch (err) {
 ...
 */
export async function startSummon({
  ...
  beingOut = null,
  // Stance positions used to compute this chat's IBP Address.
  // The IBP Address is `<askerStance> :: <addresseeStance>` (sorted
  ...
  // When either resolves to null the IBP Address is left null —
  ...
  // Planner chainstep), pass the parent summonId. We resolve the
  // parent's rootSummonId so the spawned chat sits in the same chain
  ...
  // Resolve rootSummonId: when there's a parent, the new chat inherits
  // its rootSummonId so audit walks see the whole chain rooted at the
  // user's original message. Without a parent (top-level chat), the
  // chat IS its own root.
  const summonId = uuidv4();
  let resolvedRootChatId = summonId;
  let resolvedChainIndex = 0;
  if (parentSummonId) {
    try {
      const parent = await Summon.findById(parentSummonId).select("rootSummonId chainIndex").lean();
      if (parent) {
        resolvedRootChatId = parent.rootSummonId || parentSummonId;
        // Increment chainIndex from parent so ordering reflects depth.
        resolvedChainIndex = (parent.chainIndex || 0) + 1;
      } else {
        // Parent not found — fall back to using parentSummonId as root.
        // The link is still useful for audit walks even if the parent
        // record is gone.
        resolvedRootChatId = parentSummonId;
      }
    } catch {
      resolvedRootChatId = parentSummonId;
    }
  ...
  // Compute the canonical IBP Address from the stance pair. Both
  ...
  // the IBP Address stays null and that's by design.
  ...
  const chat = await Summon.create({
    _id: summonId,
    ...
    rootSummonId: resolvedRootChatId,
...
export async function finalizeSummon({
  ...
  // Collect AI contributions linked to this chat by summonId (capped)
  const contributions = await Did.find({ summonId })
    .select("_id")
...
export async function getLatestActiveChainstepForBeing(beingOut) {
  ...
  try {
    return await Summon.findOne({
      ...
      .select("_id startMessage toolCalls aiContext treeContext parentSummonId rootSummonId chainIndex beingIn beingOut")
...
export async function appendToolCall(summonId, { tool, args, result, success, error, ms }) {
  ...
  try {
    ...
    log.debug("ChatTracker", `appendToolCall failed for ${summonId}: ${err.message}`);
  }
}
 ...
 */
export async function appendModeSwitch(summonId, { modeKey, reason = null }) {
  if (!summonId || !modeKey) return;
  try {
    await Summon.updateOne(
      { _id: summonId },
      {
    ...
    log.debug("ChatTracker", `appendModeSwitch failed for ${summonId}: ${err.message}`);
  }
}
...
export function trackChainStep({
  ...
  beingId,
  beingOut = null,
  // Pre-computed IBP Address (canonical sorted stance::stance).
  ...
  // by IBP Address simply won't surface this row.
  ibpAddress = null,
  sessionId,
  chainIndex,
  rootSummonId = null,
  ...
  // Fire and forget. Don't await, don't block the chain. ibpAddress
...
export async function startChainStep({
  ...
  beingOut = null,
  // Stance positions used to compute this chat's IBP Address.
  // See startSummon for semantics. The awaited variant auto-resolves
