...
//
// The land beings. My first delegates.
//
// Every land has a small set of beings I plant at the land root
...
//   land-manager  Conversational interface for land-level admin
//                 (extensions, config, peers). LLM cognition.
//
// They exist as real Being rows so the descriptor can surface them
// and the address grammar resolves `<land>/@auth` (etc.) to them.
//
...
//
// Being-tree parenting. Every land has exactly one I-Am at the root
// of the being-tree (the only Being with `parentBeingId: null`),
// planted during `ensureLandRoot()`. The land beings (auth,
// llm-assigner, land-manager) and every human are my children.
// Walking parentBeingId from any being eventually reaches me, then
// `null`.
//
// `ensureLandBeings(landRootId)` runs at genesis after my Being row
// exists. Idempotent: safe to call every boot. The drift reconciler
// also keeps existing land beings' parentBeingId pointed at me.
//
// Each land being's password is auto-generated random bytes,
...
/**
 * Find the I_AM: the land's first Being row, the root of the being-
 * tree, identified by `parentBeingId: null`. Every other being on the
 * land chains back to it. Created during `ensureLandRoot()`; absent
...
// calls. The I_AM has universal authority on its land; kernel-internal
 ...
 * (land-LLM config, root-only operations); use `findIAm()` for
 ...
 * land). ensureLandRoot() creates my row first and then calls this.
  ...
  for (const spec of LAND_BEINGS) {
    try {
      ...
      if (existingBeing) {
        ...
        // The being-tree chain land-being → me → null must hold.
      ...
      // because land beings don't create their own child spaces — they
      // live at the land root itself. parentBeingId = me, so the
      // being-tree chain land-being → me → null is intact.
