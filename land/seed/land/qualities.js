...
function validateKey(key) {
  ...
  if (key.length > MAX_KEY_LENGTH) {
    throw new Error(`Quality namespace "${key.slice(0, 20)}..." exceeds ${MAX_KEY_LENGTH} character limit`);
  }
...
function validateData(key, data) {
  ...
  try {
    ...
    throw new Error(`Quality "${key}" data is not serializable (circular reference, BigInt, or non-JSON type)`);
  }
  if (size > MAX_VALUE_BYTES()) {
    throw new Error(`Quality "${key}" data exceeds ${MAX_VALUE_BYTES() / 1024}KB limit (${Math.round(size / 1024)}KB)`);
  }
  const depth = measureDepth(data);
  if (depth > maxNestingDepth()) {
    throw new Error(`Quality "${key}" data exceeds max nesting depth of ${maxNestingDepth()} (found ${depth})`);
  }
  if (hasDangerousKeys(data)) {
    throw new Error(`Quality "${key}" data contains forbidden keys (__proto__, constructor, or prototype)`);
  }
  return size;
}
...
function createQualityPrimitives({ Model, documentType, enforceOwnership = false }) {
  return {
    getQuality(doc, key) {
      if (!doc || !doc.qualities) return {};
      const data = doc.qualities instanceof Map
    ...
    async setQuality(doc, key, data, opts) {
      validateKey(key);
      if (enforceOwnership && opts?.callerExtName && key !== opts.callerExtName) {
        throw new Error(`Namespace violation: "${opts.callerExtName}" cannot write to "${key}". Extensions can only write to their own quality namespace.`);
      }
      validateData(key, data);

      if (typeof doc === "object" && doc !== null && doc.qualities !== undefined) {
      ...
      hooks.run("afterQualityWrite", { documentType, [`${documentType}Id`]: id, key, data }).catch(() => {});
      return true;
    },
    ...
    async mergeQuality(doc, key, partial, opts) {
      validateKey(key);
      if (enforceOwnership && opts?.callerExtName && key !== opts.callerExtName) {
        throw new Error(`Namespace violation: "${opts.callerExtName}" cannot write to "${key}".`);
      }
      ...
      hooks.run("afterQualityWrite", { documentType, [`${documentType}Id`]: id, key, data: safePartial }).catch(() => {});
      return true;
    },
    ...
    async pushQuality(doc, key, field, item, maxLength = 100) {
      ...
      await Model.updateOne(
        { _id: id },
        { $push: { [`qualities.${key}.${field}`]: { $each: [item], $slice: -safeCap } } },
      );
      return true;
    },
...
export const qualities = Object.freeze({
  being:  createQualityPrimitives({ Model: Being,  documentType: "being",  enforceOwnership: false }),
  space:  createQualityPrimitives({ Model: Space,  documentType: "space",  enforceOwnership: true  }),
  matter: createQualityPrimitives({ Model: Matter, documentType: "matter", enforceOwnership: true  }),
});
