...
// Three shapes:
//
//   emitReplyToAsker      reply lands in the inbox of whoever
//                         summoned this being (reads originalMessage.from).
//                         Default for sub-beings reporting up.
//
//   emitReplyToStance     reply lands at an explicitly named stance.
...
// scheduler, address parsing, and land domain — no extension-specific
...
// Parse a stance string of the form `<land>/<spaceId>@<qualifier>` into
...
function parseAskerStance(stance) {
  ...
  if (!m) return null;
  return { land: m[1], spaceId: m[2], qualifier: m[3] };
}
