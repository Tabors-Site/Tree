...
export const authBeing = Object.freeze({
  ...
  async register(payload, ctx) {
    ...
    // The I_AM already exists (created by ensureLandRoot at
    // boot). The very first human registration parents under the
    // I_AM, becoming the land's root operator. Every subsequent
    ...
    if (first) {
      const { findIAm } = await import("../../land/being/landBeings.js");
