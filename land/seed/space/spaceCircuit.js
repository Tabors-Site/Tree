...
/**
 * Is the tree-circuit feature enabled on this land?
 */
function isEnabled() {
...
export async function isTreeAlive(treeId) {
  ...
  const meta = anchor.metadata instanceof Map
    ? anchor.metadata.get("circuit")
    : anchor.metadata?.circuit;
...
export async function checkTreeHealth(treeId) {
  ...
  if (sampleSize > 0) {
    ...
    for (const s of sample) {
      try {
        const meta = s.metadata instanceof Map ? Object.fromEntries(s.metadata) : (s.metadata || {});
...
export async function reviveTree(treeId, beingId) {
  ...
  const circuit = anchor.metadata instanceof Map
    ? anchor.metadata.get("circuit")
    : anchor.metadata?.circuit;
