...
export async function handleBe(socket, env, ack) {
  ...
  try {
    ...
    const ctx = {
      ...
      identity: identityForAuth,
      // HTTP carrier from the HTTP adapter / legacy auth routes stashes
      // the express req here. The auth-being's hooks (beforeRegister,
      // afterRegister) read it. Null for WS-arrival sockets.
