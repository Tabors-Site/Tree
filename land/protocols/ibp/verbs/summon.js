...
export async function handleSummon(socket, env, ack) {
  ...
  try {
    ...
    // SUMMON's right side is a stance with `@qualifier`. The stance
    // must resolve to a node — the position where the receiving being
    // is summoned. Zone-aware special-casing retired 2026-05-18.
    if (!resolved.nodeId) {
    ...
    // Resolve inbox-attach node. The stance always resolves to a node
    // (already enforced above); the being's homePositionId is the
    // fallback when the stance carries no explicit position.
    ...
    const summonCtx = {
      ...
      resolved,
      identity:   { beingId: socket.beingId, name: socket.name },
    };
...
async function runSummoning(role, ctx) {
  ...
  return {
    from:        `${ctx.resolved.being ? `${pathOfResolved(ctx.resolved)}@${ctx.toBeing.name}` : ctx.being}`,
