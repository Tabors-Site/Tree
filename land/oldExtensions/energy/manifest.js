export default {
  ...
  provides: {
    ...
    sessionTypes: {},
    cli: [
      { command: "energy", scope: ["home"], description: "Show your energy balance and reset time", method: "GET", endpoint: "/user/:beingId/energy" },
    ],

    hooks: {
      fires: [],
      listens: ["beforeArtifact", "beforeStatusChange", "afterNodeCreate", "beforeNodeDelete"],
    },

    // Documented exports (available via core.energy or getExtension("energy")?.exports)
    //
    // core.energy.useEnergy({ beingId, action })  - Deduct energy for an action. Throws EnergyError if insufficient.
