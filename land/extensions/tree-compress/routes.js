...
router.post("/root/:rootId/compress", authenticate, async (req, res) => {
  try {
    ...
    let result;
    if (targetSizeBytes) {
      result = await compressToBudget(req.params.rootId, req.beingId, user?.username, targetSizeBytes);
    } else {
      result = await compressTree(req.params.rootId, req.beingId, user?.username);
    }
...
router.post("/node/:nodeId/compress", authenticate, async (req, res) => {
  try {
    const user = await Being.findById(req.beingId).select("username").lean();
    const result = await compressBranch(req.params.nodeId, req.beingId, user?.username);
    sendOk(res, result);
  } catch (err) {
    sendError(res, 500, ERR.INTERNAL, err.message);
  }
...
router.post("/node/:nodeId/decompress", authenticate, async (req, res) => {
  try {
    const result = await decompressNode(req.params.nodeId, req.beingId);
    sendOk(res, result);
  } catch (err) {
    sendError(res, 500, ERR.INTERNAL, err.message);
  }
...
router.post("/root/:rootId/compress/budget", authenticate, async (req, res) => {
  try {
    ...
    const user = await Being.findById(req.beingId).select("username").lean();
    const result = await compressToBudget(req.params.rootId, req.beingId, user?.username, size);
    sendOk(res, result);
  } catch (err) {
    sendError(res, 500, ERR.INTERNAL, err.message);
  }
