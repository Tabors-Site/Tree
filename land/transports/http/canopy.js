...
import { sendOk, sendError, ERR, SYSTEM_OWNER } from "../../seed/core/protocol.js";
import express from "express";
import { getLandInfoPayload, getLandIdentity, signCanopyToken } from "../../protocols/canopy/identity.js";
import {
  registerPeer,
  removePeer,
  blockPeer,
  ...
  getPeerBaseUrl,
  runHeartbeat,
} from "../../protocols/canopy/peers.js";
import {
  addCanopyHeaders,
  authenticateCanopy,
  checkRateLimit as checkCanopyRateLimit,
} from "../../protocols/canopy/middleware.js";
import {
  validateCanopyRequest,
  isCompatibleVersion,
} from "../../protocols/canopy/protocol.js";
import { compareSemver } from "../../protocols/canopy/semver.js";
import { proxyToRemoteLand } from "../../protocols/canopy/proxy.js";
import {
  queueCanopyEvent,
  getPendingEventCount,
  getFailedEvents,
  retryEvent,
} from "../../protocols/canopy/events.js";
import Being from "../../seed/models/being.js";
import RemoteUser from "../../protocols/canopy/models/remoteUser.js";
import LandPeer from "../../protocols/canopy/models/landPeer.js";
import Node from "../../seed/models/node.js";
import authenticate from "./middleware/authenticate.js";
import { getExtension } from "../../extensions/loader.js";
import { lookupLandByDomain, searchLands, searchPublicTrees } from "../../protocols/canopy/horizon.js";
import { isPrivateHost } from "../../protocols/canopy/security.js";
...
/**
 * Middleware: gate canopy admin endpoints. The previous implementation
 * checked `being.isAdmin`; that flag retired 2026-05-18. Stance
 * authorization ([[project_stance_authorization]]) replaces it with
 * per-stance grant rules. Until that lands the gate is a no-op (any
 * authenticated being passes); operators should keep these endpoints
 * behind external auth (reverse proxy, IP allowlist) for now.
 */
    ...
    const results = await Promise.all(
      trees.map(async (tree) => {
        ...
        return {
          rootId: tree._id,
          name: tree.name || "",
          ownerUsername: owner?.name || "unknown",
...
router.post("/canopy/llm/proxy", authenticateCanopy, async (req, res) => {
  try {
    ...
    const { getClientForBeing } = await import("../../seed/llm/llmClient.js");
...
router.post("/canopy/notify", authenticateCanopy, async (req, res) => {
  try {
    ...
    const user = await Being.findById(targetUserId).select("_id name isRemote").lean();
    if (!user || user.isRemote) {
      return sendError(res, 404, ERR.USER_NOT_FOUND, "Target user not found on this land");
    }

    log.verbose("Canopy", `Notification for ${user.name}: ${notificationType}`);
