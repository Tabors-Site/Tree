// ables-panel.js — the Place > Ables tab.
//
// Per seed/AblesAreAuth.md, ables ARE the permission gate. This panel
...
import "../styles/ables-panel.css";

export async function renderAblesPanel(body, action, opByName, { refreshView } = {}) {
  body.innerHTML = "";
  ...
  try { hostedHere = await collectAblesInEffect(desc, story); } catch { /* surface in section */ }
  ...
  // ── 4. Ables hosted here (collapsed) ────────────────────────────
  renderHostedHere(body, { hostedHere });

  // ── 5. Author a able here (collapsed) ───────────────────────────
  ...
  // ── 6. Grant a able (only when you have grants reaching here) ──
  if (activeGrants.length > 0) {
    renderGrantForm(body, {
      heldAbles: activeGrants,
      story,
      onResult: (err) => { if (!err && typeof refreshView === "function") refreshView(); },
    });
  }
...
// be} as Maps of token → Set<source-able-names>. Owner gets a sentinel
...
function computeEffective(activeGrants, claim, viewerName) {
  ...
  for (const g of activeGrants) {
    addCan(out.see,    g.spec?.canSee,    "name",      g.grant.able);
    addCan(out.do,     g.spec?.canDo,     "action",    g.grant.able);
    addCan(out.summon, g.spec?.canSummon, "pattern",   g.grant.able);
    addCan(out.be,     g.spec?.canBe,     "operation", g.grant.able);
  }
  return out;
}
...
function renderEffective(parent, { effective, claim, viewerName, isAnonymous }) {
  ...
  if (totalSize === 0) {
    sec.appendChild(noteRow("No granted able reaches this position."));
    return;
  }
...
function addCan(map, list, key, ableName) {
...
function renderYourGrants(parent, { annotated, viewerName, isAnonymous, lineage, story }) {
  ...
  if (isAnonymous) {
    sec.appendChild(noteRow("Sign in to see your held ables."));
    return;
  }
  if (annotated.length === 0) {
    sec.appendChild(noteRow("(you hold no granted ables)"));
  } else {
...
function grantCard(entry, viewerName, story) {
  ...
  name.textContent = entry.grant.able;
...
// ──────────────────────────────────────────────────────────────────
// Section 4 — Ables hosted here (collapsed)
// ──────────────────────────────────────────────────────────────────

function renderHostedHere(parent, { hostedHere }) {
  const sec = collapsibleSection(parent, `Ables hosted here & inherited (${hostedHere.length})`);
  if (hostedHere.length === 0) {
    sec.body.appendChild(noteRow("(no ables hosted on this position or any ancestor)"));
    return;
  }
  ...
  const cards = hostedHere.map((e) => hostedAbleCard(e));
...
// ──────────────────────────────────────────────────────────────────
// Section 5 — Author a able here (collapsed)
// ──────────────────────────────────────────────────────────────────

function renderAuthorForm(parent, { positionAddress, effective, hostedHere, onResult }) {
  const sec = collapsibleSection(parent, "Author / edit a able here");
  sec.body.appendChild(noteRow(
    "Pickers below show ONLY canX you currently hold (so you can't author a able with abilities you can't grant). " +
    "Owner of this space? You can pick from any of your held tokens regardless of source able."
  ));
  const form = document.createElement("div");
  form.className = "compact-form";

  // Name input (also doubles as edit selector: same name overwrites).
  const nameField = textInput("name", "Able name (kebab-case)");
  form.appendChild(nameField.wrapper);

  // Edit-existing: pick a hosted able to load its spec into the form.
  // Same path as authoring — set-able overwrites by name.
  if (hostedHere && hostedHere.length > 0) {
    const editField = document.createElement("div");
    editField.className = "field-row";
    const lbl = document.createElement("label");
    lbl.textContent = "Load existing able to edit (optional)";
  ...
  submit.textContent = "Install / replace able at this space";
  submit.addEventListener("click", async () => {
    ...
    if (!name) {
      ...
      result.textContent = "Able name is required.";
    ...
    try {
      await flat.doOp(positionAddress, "set-able", { name, spec });
...
// ──────────────────────────────────────────────────────────────────
// Section 6 — Grant a able to a being
// ──────────────────────────────────────────────────────────────────

function renderGrantForm(parent, { heldAbles, story, onResult }) {
  const sec = collapsibleSection(parent, "Grant a able to a being");
  sec.body.appendChild(noteRow(
  ...
  const ableField = document.createElement("div");
  ableField.className = "field-row";
  const ableLabel = document.createElement("label");
  ableLabel.textContent = "Able to grant";
  const ableSelect = document.createElement("select");
  ableSelect.className = "op-input";
  for (const entry of heldAbles) {
    const opt = document.createElement("option");
    opt.value = entry.grant.able;
    opt.textContent = entry.grant.able;
    ableSelect.appendChild(opt);
  }
  ableField.appendChild(ableLabel);
  ableField.appendChild(ableSelect);
  form.appendChild(ableField);
  ...
  submit.type = "button";
  submit.className = "btn-primary";
  submit.textContent = "Grant able";
  submit.addEventListener("click", async () => {
    submit.disabled = true;
    result.textContent = "";
    const able = ableSelect.value;
    ...
    try {
      await flat.doOp(target, "grant-able", {
        able,
        anchorSpaceId,
        anchorBeingId: null,
      });
      result.className = "action-result action-ok";
      result.textContent = `granted "${able}" to ${target}`;
      onResult?.(null);
    } catch (err) {
      result.className = "action-result action-err";
      result.textContent = err?.message || String(err);
...
async function collectAblesInEffect(desc, story) {
  const client = flat.state?.client;
  if (!client) return [];
  ...
  const out = [];
  const path = desc.address?.pathByNames || "/";
  await harvestAblesAt(client, story, path, seen, out, false);

  const segs = path.replace(/^\/+|\/+$/g, "").split("/").filter(Boolean);
  for (let i = segs.length - 1; i >= 0; i--) {
    const ancestorPath = "/" + segs.slice(0, i).join("/");
    await harvestAblesAt(client, story, ancestorPath || "/", seen, out, true);
  }
  return out;
}

async function harvestAblesAt(client, story, path, seen, out, viaInheritance) {
  let snap = null;
  try {
    snap = await client.see(`${story}${path === "/" ? "/" : path}`);
  } catch { return; }
  const ables = snap?.space?.qualities?.ables || snap?.qualities?.ables || null;
  if (!ables || typeof ables !== "object") return;
  const hostSpaceName = snap?.space?.name || snap?.address?.spaceName
                       || (path === "/" ? "story root" : path);
  const hostSpaceId = snap?.space?._id || snap?.address?.spaceId || null;
  for (const [name, spec] of Object.entries(ables)) {
...
async function collectYourGrantsAndLineage(viewerName, story, positionSpaceId) {
  ...
  // carry ablesGranted + lineage. The identity block doesn't carry
  ...
  const granted = Array.isArray(qualities?.ablesGranted) ? qualities.ablesGranted : [];
...
async function annotateGrants(grants, hostedHere, positionSpaceId, story) {
  ...
  for (const grant of grants) {
    let host  = null;
    let spec  = null;
    const cached = byName.get(grant.able);
    if (cached) {
      spec = cached.spec;
      host = { spaceId: cached.hostSpaceId, name: cached.hostSpaceName };
    } else if (grant.anchorSpaceId) {
      const walked = await walkForAbleSpec(grant.anchorSpaceId, grant.able, story);
      spec = walked.spec;
      host = walked.host;
    }
...
async function walkForAbleSpec(anchorSpaceId, ableName, story) {
  const client = flat.state?.client;
  ...
  while (cursor && safety < 12) {
    ...
    const ables = snap?.space?.qualities?.ables || snap?.qualities?.ables || {};
    const found = ables?.[ableName];
    if (found) {
      return {
        spec: found,
        host: {
