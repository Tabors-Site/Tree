...
- **Human being** (`@<username>` — usually at land root `/`). The human brings *everything they are* — their full life experience, all the modes and tools available to them as a person, plus any account-level settings (LLM config, identity roster, permissions on this land). The default left-side stance for a signed-in human is `<land>/@<username>` — the human inhabits the Land at its root. A human CAN appear at deeper positions (`treeos.ai/flappybird@tabor`); the path then provides context about *where on the land they're operating from*, beyond just "user." For Pass 1 the focus is the default `/` case.
